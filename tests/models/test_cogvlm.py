...
from paddlemix.models.cogvlm.configuration import CogVLMConfig
from paddlemix.models.cogvlm.modeling import CogVLMForCausalLM
...
class CogVLMForCausalLMTester:
    ...
    def get_config(self):
        ...
        return CogVLMConfig(**test_config)
    ...
    def create_and_check_model(self, images, input_ids, attention_mask, token_type_ids, position_ids):
        model = CogVLMForCausalLM(config=self.get_config())
        model.eval()
        with paddle.no_grad():
            result = model(
                images=images,
...
class CogVLMForCausalLMTest(ModelTesterMixin, unittest.TestCase):
    all_model_classes = (CogVLMForCausalLM,)
    fx_compatible = False
    ...
    def setUp(self):
        self.model_tester = CogVLMForCausalLMTester(self)
    ...
    def test_model_from_pretrained(self):
        model = CogVLMForCausalLM.from_pretrained("THUDM/cogvlm-chat")
