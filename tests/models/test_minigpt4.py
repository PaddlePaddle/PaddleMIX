...
from paddlemix.models.minigpt4 import (
    MiniGPT4Config,
...
class MiniGPT4QFormerModelTester:
    ...
    def create_and_check_model(self, config, query_embeds, encoder_hidden_states, encoder_attention_mask):
        model = MiniGPT4QFormerModel(config=config)
        model.eval()
        result = model(
...
class MiniGPT4TextModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        first_input_ids = ids_tensor([self.batch_size, self.seq_length], self.vocab_size, dtype="int64").clip(
            3,
        )
        first_input_ids[:, -1] = self.eos_token_id  # Eos Token

        first_attention_mask = first_input_ids.not_equal(paddle.to_tensor([self.pad_token_id], dtype="int64")).cast(
            "int64"
        )
        second_input_ids = ids_tensor([self.batch_size, self.seq_length], self.vocab_size, dtype="int64").clip(
            3,
        )
        second_input_ids[:, -1] = self.eos_token_id  # Eos Token

        second_attention_mask = second_input_ids.not_equal(paddle.to_tensor([self.pad_token_id], dtype="int64")).cast(
            "int64"
        )
        return config, first_input_ids, second_input_ids, first_attention_mask, second_attention_mask
...
class MiniGPT4ModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        return config, first_input_ids, second_input_ids, first_attention_mask, second_attention_mask, pixel_values
