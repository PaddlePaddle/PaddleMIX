...
class InternVLChatModelTester:
    ...
    def prepare_config_and_inputs_for_common(self):
        ...
        pixel_values = paddle.to_tensor(np.random.rand(14, 3, 448, 448), dtype='bfloat16')  
    ...
    def create_and_check_model(self, pixel_values):
        model = InternVLChatModel.from_pretrained(self.model_name_or_path, dtype="bfloat16")
        model.eval()
...
class InternVLChatModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    def test_save_load(self):
        ...
        for model_class in self.all_model_classes:
            model = InternVLChatModel.from_pretrained(self.model_tester.model_name_or_path, dtype="bfloat16")
            model.eval()
            img_context_token_id = self.model_tester.tokenizer.convert_tokens_to_ids("<IMG_CONTEXT>")
            model.img_context_token_id = img_context_token_id
            model.eval()
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                ...
                model = model_class.from_pretrained(tmpdirname, dtype="bfloat16")
                model.img_context_token_id = img_context_token_id
                model.eval()
                with paddle.no_grad():
                    second = model(**self._prepare_for_class(inputs_dict, model_class))[0]
