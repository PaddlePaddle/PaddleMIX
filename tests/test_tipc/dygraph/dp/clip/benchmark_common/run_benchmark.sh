...
function _set_params(){
    model_item=${1:-"clip"}   # (必选) 模型 item |fastscnn|segformer_b0| ocrnet_hrnetw48
...
function _train(){
    ...
    export MODEL_NAME="paddlemix/CLIP/CLIP-ViT-L-14-laion2B-s32B-b82K"

    train_cmd="../paddlemix/examples/clip/run_pretrain_dist.py \
    ...
    DP) if [[ ${device_num} = "N1C1" ]];then
            echo "run ${run_mode} "
            train_cmd="python3.8 ${train_cmd}" 
        else
            rm -rf ./mylog   # 注意执行前删掉log目录
            train_cmd="python3.8 -m paddle.distributed.launch --log_dir=./mylog --gpus=$CUDA_VISIBLE_DEVICES \
                  ${train_cmd}" 
        fi
        ;;
