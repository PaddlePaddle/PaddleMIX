...
from paddlevlp.processors.tokenizer import tokenize
...
def get_autocast(precision):
    if precision == 'float16':
        return paddle.amp.autocast
    elif precision == 'bfloat16':
        return lambda: paddle.amp.autocast(dtype='bfloat16')
    else:
...
def run(model, classifier, dataloader, args):
    ...
    with paddle.no_grad():
        ...
        for images, target in tqdm(
            ...
            with autocast(dtype=args.fp16_opt_level):
                if hasattr(model, '_layers'):
