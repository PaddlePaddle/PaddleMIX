...
def _get_window(
    ...
    if dither != 0.0:
        x = paddle.maximum(
            x=epsilon, y=paddle.rand(shape=strided_input.shape, dtype=dtype)
        )
...
def vtln_warp_freq(
    ...
    outside_low_high_freq = paddle.less_than(
        x=freq, y=paddle.to_tensor(low_freq)
    ) | paddle.greater_than(x=freq, y=paddle.to_tensor(high_freq))
...
def get_mel_banks(
    ...
    if vtln_warp_factor == 1.0:
        bins = paddle.maximum(
            x=paddle.zeros(shape=[1]), y=paddle.minimum(x=up_slope, y=down_slope)
        )
    else:
        bins = paddle.zeros_like(x=up_slope)
        up_idx = paddle.greater_than(
            x=mel, y=paddle.to_tensor(left_mel)
        ) & paddle.less_equal(x=mel, y=paddle.to_tensor(center_mel))
        down_idx = paddle.greater_than(
            x=mel, y=paddle.to_tensor(center_mel)
        ) & paddle.less_than(x=mel, y=paddle.to_tensor(right_mel))
...
def fbank(
    ...
    if use_log_fbank:
        mel_energies = paddle.maximum(
            x=mel_energies, y=_get_epsilon(device, dtype)
        ).log()
    if use_energy:
