...
_EVAL14_336 = dict(
    ...
    eva_clip_224to336=_pcfg(
        hf_hub='QuanSun/EVA-CLIP/EVA02_CLIP_L_psz14_224to336.pt'),
    eva02_clip_224to336=_pcfg(
        hf_hub='QuanSun/EVA-CLIP/EVA02_CLIP_L_psz14_224to336.pt'))
_VITH14 = dict(laion2b_s32b_b79k=_pcfg(
...
_EVAg14_PLUS = dict(
    ...
    eva01=_pcfg(hf_hub='QuanSun/EVA-CLIP/EVA01_g_psz14.pt'),
    eva_clip=_pcfg(
        hf_hub='QuanSun/EVA-CLIP/EVA01_CLIP_g_14_plus_psz14_s11B.pt'),
    eva01_clip=_pcfg(
        hf_hub='QuanSun/EVA-CLIP/EVA01_CLIP_g_14_plus_psz14_s11B.pt'))
_VITbigG14 = dict(laion2b_s39b_b160k=_pcfg(
...
def list_pretrained(as_str: bool=False):
    ...
    """
    return [(':'.join([k, t]) if as_str else (k, t))
            for k in _PRETRAINED.keys() for t in _PRETRAINED[k].keys()]
...
def download_pretrained_from_url(url: str, cache_dir: Union[str, None]=None):
    ...
    if expected_sha256 and not hashlib.sha256(
            open(download_target, 'rb').read()).hexdigest().startswith(
                expected_sha256):
        raise RuntimeError(
...
def download_pretrained_from_hf(model_id: str,
...
def download_pretrained(cfg: Dict,
