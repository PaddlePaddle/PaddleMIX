...
def _natural_key(string_):
    return [(int(s) if s.isdigit() else s)
            for s in re.split('(\\d+)', string_.lower())]
...
def get_model_config(model_name):
    if model_name in _MODEL_CONFIGS:
        print("model name in _MODEL_CONFIGS", model_name)
...
def get_tokenizer(model_name):
    ...
    tokenizer = HFTokenizer(config['text_cfg'][
        'hf_tokenizer_name']) if 'hf_tokenizer_name' in config[
...
def load_state_dict(checkpoint_path: str,
    ...
    if is_openai:
        raise ValueError(
            "paddle do not have a function to load torch jit model")
...
def load_checkpoint(model,
    ...
    state_dict = load_state_dict(
        checkpoint_path, model_key=model_key, is_openai=False)
...
def load_clip_visual_state_dict(checkpoint_path: str,
                                map_location: str='cpu',
                                is_openai: bool=False,
                                skip_list: list=[]):
    state_dict = load_state_dict(
...
def load_clip_text_state_dict(checkpoint_path: str,
                              map_location: str='cpu',
                              is_openai: bool=False,
                              skip_list: list=[]):
    state_dict = load_state_dict(
...
def load_pretrained_checkpoint(model,
    ...
    logging.info(
        f'num of model state_dict keys: {len(model.state_dict().keys())}')
    ...
    if text_checkpoint_path:
        if text_tag == 'eva_clip' or text_tag == 'open_clip':
            ...
            text_state_dict = load_state_dict(
                ...
                is_openai=False,
                skip_list=skip_list)
        text_incompatible_keys = model.text.load_state_dict(text_state_dict)
        logging.info(
...
def create_model(args,
    ...
    if pretrained and pretrained.lower() == 'openai':
        ...
        print("model_cfg:{}".format(model_cfg))
        ...
        custom_clip = model_cfg.pop(
            'custom_text',
            False) or force_custom_clip or 'hf_model_name' in model_cfg[
                'text_cfg']
        if custom_clip:
            if 'hf_model_name' in model_cfg.get('text_cfg', {}):
                model_cfg['text_cfg']['hf_model_pretrained'] = pretrained_hf
            if 'coca' in model_name:
                model = CoCa(args, **model_cfg, cast_dtype=cast_dtype)
            else:
                model = CustomCLIP(args, **model_cfg, cast_dtype=cast_dtype)
        else:
            model = CLIP(args, **model_cfg, cast_dtype=cast_dtype)
        pretrained_cfg = {}
        if pretrained:
            checkpoint_path = ''
            ...
            if checkpoint_path:
                ...
                error_str = (
                    f'Pretrained weights ({pretrained}) not found for model {model_name}.Available pretrained tags ({list_pretrained_tags_by_model(model_name)}.'
                )
            ...
            if pretrained_text:
                ...
                pretrained_text_cfg = get_pretrained_cfg(pretrained_text_model,
                                                         pretrained_text)
        ...
        model.visual.image_std = pretrained_cfg.get('std',
                                                    None) or OPENAI_DATASET_STD
    return model
...
def create_model_and_transforms(args,
    ...
    # preprocess_train = image_transform(model.visual.image_size, is_train=
    #     True, mean=image_mean, std=image_std)
    # preprocess_val = image_transform(model.visual.image_size, is_train=
    #     False, mean=image_mean, std=image_std)
...
def create_model_from_pretrained(args,
    ...
    if not is_pretrained_cfg(model_name,
                             pretrained) and not os.path.exists(pretrained):
        raise RuntimeError(
