...
def _get_sinc_resample_kernel(
    ...
    for i in range(new_freq):
        ...
        if resampling_method == 'sinc_interpolation':
            ...
            window = paddle.i0(
                beta_tensor * paddle.sqrt(x=1 - (t / lowpass_filter_width) ** 2)
            ) / paddle.i0(beta_tensor)
        t *= math.pi
        # breakpoint()
        kernel = paddle.where(
            condition=t == 0, x=paddle.to_tensor(data=1.0), y=paddle.sin(x=t) / t
        )
...
def _apply_sinc_resample_kernel(
    waveform, orig_freq: int, new_freq: int, gcd: int, kernel, width: int
):
    ...
    waveform = paddle.nn.functional.pad(
        waveform.unsqueeze(1), (width, width + orig_freq), data_format="NCL"
    ).squeeze()
    if waveform.dim() == 1:
