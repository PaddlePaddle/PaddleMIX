...
class MultiheadAttention(paddle.nn.MultiHeadAttention):
    ...
    def compute_kv(self, key, value):
        ...
        if self.add_bias_kv:
            k = paddle.concat(
                [k, paddle.repeat_interleave(self.bias_k, bsz, axis=0)], axis=1
            )
            v = paddle.concat(
                [v, paddle.repeat_interleave(self.bias_v, bsz, axis=0)], axis=1
            )
...
class SimpleTransformer(paddle.nn.Layer):
    def __init__(
        ...
        if drop_path_type == 'progressive':
            dpr = [
                x.item()
                for x in paddle.linspace(start=0, stop=drop_path_rate, num=num_blocks)
            ]
