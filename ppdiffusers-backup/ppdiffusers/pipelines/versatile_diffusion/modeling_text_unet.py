...
class UNetFlatConditionModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
                 ...
                 down_block_types: Tuple[str]=(
                     ...
                     'CrossAttnDownBlockFlat', 'DownBlockFlat'),
        ...
        if encoder_hid_dim_type == 'text_proj':
            self.encoder_hid_proj = paddle.nn.Linear(
                in_features=encoder_hid_dim, out_features=cross_attention_dim)
        ...
        if class_embed_type is None and num_class_embeds is not None:
            self.class_embedding = paddle.nn.Embedding(
                num_embeddings=num_class_embeds, embedding_dim=time_embed_dim)
        elif class_embed_type == 'timestep':
            self.class_embedding = TimestepEmbedding(
                timestep_input_dim, time_embed_dim, act_fn=act_fn)
        elif class_embed_type == 'identity':
            self.class_embedding = paddle.nn.Identity(time_embed_dim,
                                                      time_embed_dim)
            ...
            self.class_embedding = paddle.nn.Linear(
                in_features=projection_class_embeddings_input_dim,
    ...
    def set_attention_slice(self, slice_size):
        ...
        def fn_recursive_set_attention_slice(module: paddle.nn.Layer,
                                             slice_size: List[int]):
            if hasattr(module, 'set_attention_slice'):
    ...
    def forward(self,
        ...
        if self.config.addition_embed_type == 'text':
            ...
            if ('image_embeds' not in added_cond_kwargs or
                    'hint' not in added_cond_kwargs):
                raise ValueError(
        ...
        if (self.encoder_hid_proj is not None and
                self.config.encoder_hid_dim_type == 'text_proj'):
...
class LinearMultiDim(paddle.nn.Linear):
    def __init__(self,
                 in_features,
                 out_features=None,
                 second_dim=4,
                 *args,
                 **kwargs):
        in_features = [in_features, second_dim, 1] if isinstance(
            in_features, int) else list(in_features)
        if out_features is None:
            out_features = in_features
        out_features = [out_features, second_dim, 1] if isinstance(
            out_features, int) else list(out_features)
...
class ResnetBlockFlat(paddle.nn.Layer):
    def __init__(self,
                 *,
                 in_channels,
                 out_channels=None,
        ...
        self.pre_norm = True
        in_channels = [in_channels, second_dim, 1] if isinstance(
            in_channels, int) else list(in_channels)
        self.in_channels_prod = np.array(in_channels).prod()
        self.channels_multidim = in_channels
        if out_channels is not None:
            out_channels = [out_channels, second_dim, 1] if isinstance(
                out_channels, int) else list(out_channels)
        ...
        self.norm1 = paddle.nn.GroupNorm(
            num_groups=groups,
            num_channels=self.in_channels_prod,
            epsilon=eps,
            weight_attr=True,
            bias_attr=True)
        self.conv1 = paddle.nn.Conv2D(
            in_channels=self.in_channels_prod,
            out_channels=out_channels_prod,
            kernel_size=1,
            padding=0)
        if temb_channels is not None:
            self.time_emb_proj = paddle.nn.Linear(
                in_features=temb_channels, out_features=out_channels_prod)
        else:
            self.time_emb_proj = None
        self.norm2 = paddle.nn.GroupNorm(
            num_groups=groups_out,
            num_channels=out_channels_prod,
            epsilon=eps,
            weight_attr=True,
            bias_attr=True)
        self.dropout = paddle.nn.Dropout(p=dropout)
        self.conv2 = paddle.nn.Conv2D(
            in_channels=out_channels_prod,
            out_channels=out_channels_prod,
            kernel_size=1,
            padding=0)
    ...
    def forward(self, input_tensor, temb):
        ...
        input_tensor = input_tensor.reshape(*shape[0:-n_dim],
                                            self.in_channels_prod, 1, 1)
...
class DownBlockFlat(paddle.nn.Layer):
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
                 temb_channels: int,
...
class CrossAttnDownBlockFlat(paddle.nn.Layer):
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
                 temb_channels: int,
...
class UpBlockFlat(paddle.nn.Layer):
    def __init__(self,
                 in_channels: int,
...
class CrossAttnUpBlockFlat(paddle.nn.Layer):
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
                 prev_output_channel: int,
...
class UNetMidBlockFlatCrossAttn(paddle.nn.Layer):
    def __init__(self,
                 in_channels: int,
                 temb_channels: int,
                 dropout: float=0.0,
...
class UNetMidBlockFlatSimpleCrossAttn(paddle.nn.Layer):
    def __init__(self,
                 in_channels: int,
                 temb_channels: int,
                 dropout: float=0.0,
