...
class VaeImageProcessorLDM3D(VaeImageProcessor):
    ...
    def numpy_to_depth(self, images):
        ...
        if images.shape[-1] == 6:
            ...
            pil_images = [
                Image.fromarray(
                    self.rgblike_to_depthmap(image_depth), mode='I;16')
            ...
            pil_images = [
                Image.fromarray(
                    image_depth, mode='I;16') for image_depth in images_depth
            ]
        else:
    ...
    def postprocess(self,
        ...
        image = paddle.stack(x=[(self.denormalize(image[i])
                                 if do_denormalize[i] else image[i])
                                for i in range(image.shape[0])])
        ...
        if output_type == 'np':
            if image.shape[-1] == 6:
                image_depth = np.stack(
                    [self.rgblike_to_depthmap(im[:, :, 3:]) for im in image],
                    axis=0)
            else:
                image_depth = image[:, :, :, 3:]
