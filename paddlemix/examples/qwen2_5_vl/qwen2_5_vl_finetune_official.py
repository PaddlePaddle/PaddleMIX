...
from paddlemix.models.qwen2_5_vl.dataset import (
    DataCollatorForSupervisedDataset,
    FlattenedDataCollatorForSupervisedDataset,
    LazySupervisedDatasetQwen2_5VL,
)
...
from paddlemix.processors.qwen2_5_vl_processing import Qwen2_5_VLImageProcessor
...
class ModelArguments(ProcessorArguments):
    ...
    tune_mm_mlp: Optional[str] = field(default=False, metadata={"help": "whether to tune mm mlp parameters."})
...
class DataTrainingArguments:
    ...
    dataset_use: Optional[str] = field(
        default="",
        metadata={"help": "used dataset"},
    )
    ...
    data_flatten: Optional[bool] = field(default=None, metadata={"help": "Set to use data flatten"})
...
def make_supervised_data_module(tokenizer, data_args, image_processor) -> Dict:
    """Make dataset and collator for supervised fine-tuning."""
    train_dataset = LazySupervisedDatasetQwen2_5VL(
...
def main():
    ...
    model = Qwen2_5_VLForConditionalGeneration.from_pretrained(
        MODEL_NAME, dtype=dtype, attn_implementation="flash_attention_2"
    )
    image_processor = Qwen2_5_VLImageProcessor()
    tokenizer = MIXQwen2_5_Tokenizer.from_pretrained(MODEL_NAME)
    # processor = Qwen2_5_VLProcessor(
    #     image_processor, tokenizer, image_max_pixels=model_args.max_pixels, image_min_pixels=model_args.min_pixels
    # )
    ...
    if model_args.freeze_vit:
        ...
        # merger enable gradient
        for name, param in model.visual.named_parameters():
            if name.startswith("visual.merger"):
                print(f"{name} use gradient")
