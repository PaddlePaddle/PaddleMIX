...
from paddlenlp.transformers import AutoModelForCausalLM, Qwen2Tokenizer
...
from paddlemix.models.internvl2.constants import IMG_CONTEXT_TOKEN
...
class ModelArguments:
    ...
    use_backbone_lora: int = field(
        default=0, metadata={"help": "Set the LoRA adapter rank for the backbone model. Default is 0."}
    )
...
class DataTrainingArguments:
    ...
    conv_style: Optional[str] = field(default="internlm2-chat", metadata={"help": "Prompt style for a conversation."})
...
def main():
    ...
    if "qwen" in tokenizer_path.lower() or "1B" in tokenizer_path or "InternVL2_5-4B" in tokenizer_path:
        tokenizer = Qwen2Tokenizer.from_pretrained(tokenizer_path, add_eos_token=False, trust_remote_code=True)
    else:
    ...
    if model_args.model_name_or_path is not None:
        logger.info("Loading MiniMonkeyChatModel...")
        ...
        model = MiniMonkeyChatModel.from_pretrained(model_args.model_name_or_path, config=config, dtype=dtype)
        # TODO: fix dtype for layernorm
    else:
        logger.info("Loading ViT-6B...")
        vision_config = InternVisionConfig.from_pretrained(model_args.vision_path)
        vision_config.drop_path_rate = model_args.drop_path_rate
        vision_model = InternVisionModel.from_pretrained(model_args.vision_path, config=vision_config, dtype=dtype)
        ...
        llm = model_type.from_pretrained(model_args.llm_path, dtype=dtype, config=llm_config, trust_remote_code=True)
        ...
        logger.info("Building MiniMonkeyChatModel...")
        model = MiniMonkeyChatModel(internvl_chat_config, vision_model, llm)
