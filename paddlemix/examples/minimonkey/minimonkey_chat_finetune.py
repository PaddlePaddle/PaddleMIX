...
class LazySupervisedDataset(Dataset):
    ...
    def multi_modal_get_item(self, data_item):
        ...
        if self.dynamic_image_size:  # If dynamic image size is enabled, preprocess the image dynamically
            images, target_aspect_ratio = dynamic_preprocess(image, min_num=self.min_dynamic_patch, max_num=self.max_dynamic_patch,
                                        image_size=self.image_size, use_thumbnail=self.use_thumbnail, return_target_aspect_ratio=True)
            images = images[:-1] + dynamic_preprocess2(image, min_num=1, max_num=4,
                                            image_size=self.image_size, use_thumbnail=False, prior_aspect_ratio=target_aspect_ratio) + images[-1:]
        else:  # Otherwise, use the original image as a single patch
            images = [image]
