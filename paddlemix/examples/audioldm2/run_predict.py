...
def get_vits_phoneme_ids_no_padding(phonemes):
    ...
    return {
        "phoneme_idx": paddle.to_tensor(_pad_phonemes(sequence), dtype="int64").unsqueeze(0).expand([batchsize, -1])
    }
...
def make_batch_for_text_to_audio(text, transcription="", waveform=None, fbank=None, batchsize=1):
    ...
    if fbank is None:
        fbank = paddle.zeros((batchsize, 1024, 64))  # Not used, here to keep the code format
    else:
...
def save_wave(waveform, savepath, name="outwav", samplerate=16000):
    ...
    for i in range(waveform.shape[0]):
        if waveform.shape[0] > 1:
            fname = "%s_%s.wav" % (
                os.path.basename(name[i]) if (".wav" not in name[i]) else os.path.basename(name[i]).split(".")[0],
                i,
            )
        else:
            fname = (
...
class DataArguments:
    ...
    text_list: str = field(
        default="",
        metadata={"help": "A file (utf-8 encoded) that contains text prompt to the model for audio generation."},
    )
