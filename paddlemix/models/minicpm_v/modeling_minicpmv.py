...
class MiniCPMV(MiniCPMVPreTrainedModel):
    ...
    def forward(self, data, **kwargs):
        ...
        return self.llm(input_ids=None, position_ids=position_ids,
            inputs_embeds=vllm_embedding, **kwargs)
    ...
    def _decode(self, inputs_embeds, tokenizer, attention_mask, decode_text=False, **kwargs):
        ...
        if decode_text:
            return self._decode_text(output[0], tokenizer) #注意这里的output是list
        return output
    ...
    def chat(self, image, msgs, tokenizer, processor=None,
        ...
        inputs = processor(prompts_lists, input_images_lists,
            max_slice_nums=max_slice_nums, use_image_id=use_image_id,
            return_tensors='pt', max_length=max_inp_length)

        if sampling:
            generation_config = {'top_p': 0.01, 'top_k': 100, 'temperature':
                0.7, 'do_sample': True, 'repetition_penalty': 1.05}
        else:
        ...
        with paddle.no_grad():
            res = self.generate(**inputs, tokenizer=tokenizer,
                max_new_tokens=max_new_tokens, vision_hidden_states=
                vision_hidden_states, stream=stream, decode_text=True, **
                generation_config)
        if stream:
            def stream_gen():
                for text in res:
