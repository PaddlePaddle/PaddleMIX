...
try:
    from nn.functional.flash_attention import flash_attention as flash_attn_func
    from nn.functional.flash_attention import flash_attn_unpadded as flash_attn_varlen_func
    print("modeling_minicpm-v-2_6 has_flash_attn is True.")
    has_flash_attn = True
except:
    flash_attn_func, flash_attn_varlen_func = None, None
    print("modeling_minicpm-v-2_6 has_flash_attn is False.")
...
class PaddleAttentionMaskConverter:
    ...
    def _expand_mask(mask: paddle.Tensor, dtype: str, tgt_len: Optional[int] = None):
        ...
        inverted_mask = paddle.where(
            inverted_mask.astype('bool'),
            min_value,
            paddle.zeros_like(inverted_mask)
        )
...
class SigLipVisionConfig(PretrainedConfig):
    ...
    def __init__(self, hidden_size=768, intermediate_size=3072,
        num_hidden_layers=12, num_attention_heads=12, num_channels=3,
        image_size=224, patch_size=16, hidden_act='gelu',
        layer_norm_eps=1e-06, attention_dropout=0.0, **kwargs):
...
def _get_unpad_data(attention_mask):
    ...
    cu_seqlens = nn.functional.pad(x=paddle.cumsum(x=
...
def trunc_normal_tf_(tensor: paddle.Tensor, mean: float=0.0, std: float=1.0,
    a: float=-2.0, b: float=2.0) ->paddle.Tensor:
...
class SiglipVisionEmbeddings(nn.Layer):
    ...
    def forward(self, pixel_values: paddle.Tensor, patch_attention_mask:
        paddle.Tensor, tgt_sizes: Optional[paddle.Tensor]=None
        ) ->paddle.Tensor:
        ...
        boundaries = paddle.arange(start=1 / self.num_patches_per_side, end
            =1.0, step=1 / self.num_patches_per_side)
        position_ids = paddle.full(shape=(batch_size, max_nb_patches_h *
        ...
        for batch_idx, p_attn_mask in enumerate(patch_attention_mask):
            ...
            fractional_coords_h = paddle.arange(start=0, end=1 - 1e-06,
                step=1 / nb_patches_h)
            fractional_coords_w = paddle.arange(start=0, end=1 - 1e-06,
                step=1 / nb_patches_w)
            bucket_coords_h = paddle.bucketize(x=fractional_coords_h,
                sorted_sequence=boundaries, right=True)
            bucket_coords_w = paddle.bucketize(x=fractional_coords_w,
                sorted_sequence=boundaries, right=True)
            pos_ids = (bucket_coords_h[:, None] * self.num_patches_per_side +
                bucket_coords_w).flatten()
            position_ids[batch_idx].scatter_(paddle.nonzero(p_attn_mask.reshape([-1]))[:, 0], pos_ids.astype(position_ids.dtype))
...
class SiglipFlashAttention2(SigLipAttention):
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        ...
        ) ->Tuple[paddle.Tensor, Optional[paddle.Tensor], Optional[Tuple[
        paddle.Tensor]]]:
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, SiglipVisionEmbeddings):
            ...
            if hasattr(module.fc1, 'bias') and module.fc1.bias is not None:
                module.fc1.bias.set_value(
                    paddle.normal(shape=module.fc1.bias.shape, mean=0.0, std=1e-06))
            if hasattr(module.fc2, 'bias') and module.fc2.bias is not None:
                module.fc2.bias.set_value(
                    paddle.normal(shape=module.fc2.bias.shape, mean=0.0, std=1e-06))
...
class SigLipEncoder(nn.Layer):
    ...
    def __init__(self, config: SigLipVisionConfig):
        super().__init__()
        self.config = config
        self.layers = nn.LayerList(sublayers=[SigLipEncoderLayer(
            config) for _ in range(config.num_hidden_layers)])
        self.gradient_checkpointing = False

    def forward(self, inputs_embeds, attention_mask: Optional[paddle.Tensor
        ]=None, output_attentions: Optional[bool]=None,
        output_hidden_states: Optional[bool]=None, return_dict: Optional[
        bool]=True) ->Union[Tuple, BaseModelOutput]:
        """
        Args:
        ...
        for encoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(encoder_layer
                    .__call__, hidden_states, attention_mask, output_attentions
                    )
            else:
...
class SigLipVisionTransformer(SigLipPreTrainedModel):
    ...
    def __init__(self, config: SigLipVisionConfig):
        ...
        self.post_layernorm = nn.LayerNorm(normalized_shape=
    ...
    def forward(self, pixel_values, patch_attention_mask: Optional[paddle.
        Tensor]=None, tgt_sizes: Optional[paddle.Tensor]=None,
        output_attentions: Optional[bool]=None, output_hidden_states:
        Optional[bool]=None, return_dict: Optional[bool]=True) ->Union[Tuple, BaseModelOutputWithPooling]:
        """
        Returns:
        """
        output_attentions = (output_attentions if output_attentions is not
        ...
        if patch_attention_mask is None:
            patch_attention_mask = paddle.ones(shape=(batch_size, 
                pixel_values.shape[2] // self.config.patch_size, 
                pixel_values.shape[3] // self.config.patch_size), dtype='bool')
        
        hidden_states = self.embeddings(pixel_values=pixel_values,
            patch_attention_mask=patch_attention_mask, tgt_sizes=tgt_sizes)
        ...
        encoder_outputs = self.encoder(inputs_embeds=hidden_states,
            attention_mask=attention_mask, output_attentions=
