...
def compute_spec_paddle(hps, filename, specname):
    audio, sampling_rate = utils.load_wav_to_paddle(filename)
    ...
    spec = spectrogram.spectrogram_paddle(
        audio_norm, n_fft, sampling_rate, hop_size, win_size, center=False)
    spec = paddle.squeeze(spec, 0)

    # print(f"paddle {spec.mean().item()} {spec.std().item()}")
    paddle.save(spec, specname)

# def compute_spec_torch(hps, filename, specname):
...
#     # print(f"torch {spec.mean().item()} {spec.std().item()}")
#     torch.save(spec, specname)

def process_file(file):
    if file.endswith(".wav"):
        file = file[:-4]
        compute_spec_paddle(hps.data, f"{wavPath}/{spks}/{file}.wav", f"{spePath}/{spks}/{file}.pt")
        # compute_spec_torch(hps.data, f"{wavPath}/{spks}/{file}.wav", f"{spePath}/{spks}/{file}.pt")
