...
def infer(frames, model='full', embed=False):
    ...
    if not hasattr(infer, 'model') or not hasattr(infer, 'capacity') or \
       (hasattr(infer, 'capacity') and infer.capacity != model):
        crepe.load.model(frames.place, model)

    # Move model to correct device (no-op if devices are the same)
    # infer.model = infer.model.to(frames.place)
...
def postprocess(probabilities,
    ...
    minidx = crepe.convert.frequency_to_bins(fmin)
    maxidx = crepe.convert.frequency_to_bins(fmax, math.ceil)
...
def preprocess(audio,
    ...
    for i in range(0, total_frames, batch_size):
        ...
        frames /= paddle.maximum(paddle.to_tensor(1e-10),
...
def periodicity(probabilities, bins):
    ...
    periodicity = probs_stacked.take_along_axis(bins_stacked, axis=1)
