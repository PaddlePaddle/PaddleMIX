...
class InternLMXComposer2ForCausalLM(InternLM2PretrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        self.output = paddle.nn.Linear(in_features=config.hidden_size,
        ...
        self.vis_processor = paddle.vision.transforms.Compose([
            ...
            paddle.vision.transforms.Normalize((0.48145466, 0.4578275, 0.40821073), (0.26862954, 0.26130258, 0.27577711))])
    ...
    def encode_text(self, text, add_special_tokens=False):
        token = self.tokenizer(text, return_tensors='pd',
    ...
    def prompt_wrap(self, img_embeds, prompt):
        ...
        wrapped_img_embeds = paddle.concat(x=[p_before_embeds, img_embeds],
            axis=1)
        wrapped_atts_img = paddle.ones(shape=wrapped_img_embeds.shape[:-1],
            dtype='int64')
        wrapped_target = paddle.ones(shape=[batch_size, wrapped_img_embeds.
    ...
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, im_mask=None, **kwargs):
        if past_key_values is not None:
        ...
        model_inputs.update({'position_ids': position_ids,
            ...
            'use_cache'), 'attention_mask': attention_mask, 'im_mask': im_mask}
            )
        return model_inputs
    ...
    def prepare_input_ids_for_generation(bos_token_id, encoder_output=None):
        ...
        # return paddle.ones([batch_size, 0], dtype="int64") * bos_token_id
        return paddle.ones([batch_size, 1], dtype="int64") * bos_token_id


    def build_inputs(self, tokenizer, query: str, history: List[Tuple[str,
        str]]=[], meta_instruction=''):
        prompt = ''
        if meta_instruction:
    ...
    @paddle.no_grad()
    def stream_chat(self, tokenizer, query: str, history: List[Tuple[str,
        str]]=[], max_new_tokens: int=1024, do_sample: bool=True,
        temperature: float=0.8, top_p: float=0.8, **kwargs):
