...
class Attention(paddle.nn.Layer):
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        B, N, C = x.shape
        if self.deepnorm or self.subln:
            # x [100, 257, 192]  q.sumsum() 282649.21875000
            q = self.q_proj(x)
            k = self.k_proj(x)
            v = self.v_proj(x)
    ...
    def forward(self, x, rel_pos_bias=None, attn_mask=None):
        if self.gamma_1 is None:
            if 1:
                ...
                with get_rng_state_tracker().rng_state("global_seed"):
                    x_drop2 = self.drop_path(x_mlp)
                x = x_ori2 + x_drop2
        return x

    def forward222(self, x, rel_pos_bias=None, attn_mask=None):
        if self.gamma_1 is None:
            if self.postnorm:
                x = x + self.drop_path(
                    self.norm1(
...
def _mask_1d_rel_pos_index(seq_len):
    ...
    return index.reshape([1, seq_len]) - index.reshape(
        [seq_len, 1]) + seq_len - 1


def _add_cls_to_index_matrix(index, num_tokens, offset):
    index = index.reshape([num_tokens, num_tokens])
    new_index = paddle.zeros(
        shape=(num_tokens + 1, num_tokens + 1), dtype=index.dtype)
    new_index[1:, 1:] = index
...
class DecoupledRelativePositionBias(nn.Layer):
    def __init__(self, window_size, num_heads):
        super().__init__()
        self.window_size = window_size
        self.num_relative_distance = 2 * window_size[0] + 2, 2 * window_size[
            1] + 2
        ...
        h_index = _mask_1d_rel_pos_index(window_size[0]).reshape(
            [window_size[0], 1, window_size[0], 1]).expand(
                [-1, window_size[1], -1, window_size[1]])
        h_index = _add_cls_to_index_matrix(h_index, num_tokens,
                                           2 * window_size[0] - 1)
        self.register_buffer("relative_position_high_index", h_index)

        w_index = _mask_1d_rel_pos_index(window_size[1]).reshape(
            [1, window_size[1], 1, window_size[1]]).expand(
                [window_size[0], -1, window_size[0], -1])
        w_index = _add_cls_to_index_matrix(w_index, num_tokens,
                                           2 * window_size[1] - 1)
...
class EVA02VisionTransformerPretrainedModel(PretrainedModel):
    """
...
class EVA02VisionTransformer(EVA02VisionTransformerPretrainedModel):
    ...
    def forward_features(self, x, return_patch_tokens=False):
        ...
        for blk in self.blocks:
            ...
            #np.save("pd/blocks_{}.npy".format(cnt), x.numpy())
    ...
    def forward(self, x, return_patch_tokens=False):
        ...
        x = self.head(x)
        #np.save("pd/head.npy", x.detach().cpu().numpy())
        return x
