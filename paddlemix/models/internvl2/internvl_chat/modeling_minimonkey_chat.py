    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        if use_scm:
            ...
            tmp_idx = tmp_attn.mean(1).topk(int(tmp_attn.shape[-1] * 0.5))[1] + self.language_model.model.img_idx[0][0] # value, indices = topk 
            top_attention_rank_index = tmp_idx.sort()[0]
            top_attention_rank_index = paddle.concat((paddle.arange(self.language_model.model.img_idx[0][0]), top_attention_rank_index, paddle.arange(self.language_model.model.img_idx[0][0]+self.language_model.model.high_token+1, input_embeds.shape[1])))
