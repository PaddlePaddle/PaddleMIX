...
from paddlenlp.generation import GenerationConfig,LogitsProcessorList, StoppingCriteriaList, validate_stopping_criteria, get_unfinished_flag
from paddlenlp.transformers.utils import cached_file
from paddlenlp.transformers.model_utils import PretrainedModel
from paddlenlp.transformers.model_outputs import ModelOutput, BaseModelOutput, BaseModelOutputWithPast
...
from .configuration_qwen2_5_omni import Qwen2_5OmniAudioEncoderConfig, Qwen2_5OmniBigVGANConfig, Qwen2_5OmniConfig, Qwen2_5OmniDiTConfig, Qwen2_5OmniTalkerConfig, Qwen2_5OmniTextConfig, Qwen2_5OmniThinkerConfig, Qwen2_5OmniToken2WavConfig, Qwen2_5OmniVisionEncoderConfig


def compute_eager_attn(
    self,
    query,
    key,
    value,
...
__all__ = ['Qwen2_5OmniPreTrainedModel']


def kaiser_window(window_length, periodic=True, beta=12.0, *, dtype=None, layout=None, device=None, stop_gradient=True):
    # 计算窗函数的采样位置
...
class Qwen2_5OmniPreTrainedModelForConditionalGeneration(
    Qwen2_5OmniPreTrainedModel):

    def _prepare_4d_causal_attention_mask_with_cache_position(self,
        attention_mask: paddle.Tensor, sequence_length: int, target_length:
        int, dtype: paddle.dtype, min_dtype: float, cache_position: paddle.Tensor,
        batch_size: int):
        """
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            causal_mask = paddle.full(shape=(sequence_length, target_length
                ), fill_value=min_dtype, dtype=dtype)
            if sequence_length != 1:
                causal_mask = paddle.triu(x=causal_mask, diagonal=1)
            causal_mask *= paddle.arange(end=target_length
                ) > cache_position.reshape([-1, 1])
            causal_mask = causal_mask[None, None, :, :].expand(shape=[
                batch_size, 1, -1, -1])
            if attention_mask is not None:
                ...
                causal_mask[:, :, :, :mask_length] = causal_mask[:, :, :, :
                    mask_length].masked_fill(mask=padding_mask, value=min_dtype
                    )
        return causal_mask

    def get_llm_pos_ids_for_vision(self, start_idx: int, vision_idx: int,
        spatial_merge_size: int, t_index: List[int], grid_hs: List[int],
        grid_ws: List[int]):
        llm_pos_ids_list = []
        llm_grid_h = grid_hs[vision_idx] // spatial_merge_size
        llm_grid_w = grid_ws[vision_idx] // spatial_merge_size
        h_index = paddle.arange(end=llm_grid_h).view([1, -1, 1]).expand(shape
            =[len(t_index), -1, llm_grid_w]).flatten()
        w_index = paddle.arange(end=llm_grid_w).view([1, 1, -1]).expand(shape
            =[len(t_index), llm_grid_h, -1]).flatten()
        t_index = paddle.to_tensor(data=t_index).view([-1, 1]).expand(shape=[
            -1, llm_grid_h * llm_grid_w]).flatten().astype(dtype='int64')
    ...
    def get_chunked_index(self, llm_pos_ids, t_ntoken_per_chunk, st_idx):
        ...
        def _iter():
            ...
            while i < tuple(llm_pos_ids.shape)[1]:
                if llm_pos_ids[0][i
                    ] - st_idx >= current_chunk * t_ntoken_per_chunk:
                    yield start_idx, i
                    start_idx = i
                    current_chunk += 1
                i += 1
    ...
    def get_rope_index(self, input_ids: Optional[paddle.Tensor]=None,
        ...
        Optional[paddle.Tensor]=None, attention_mask: Optional[paddle.
        Tensor]=None, use_audio_in_video: bool=False, audio_seqlens:
        Optional[paddle.Tensor]=None, second_per_grids: Optional[paddle.
        Tensor]=None) ->Tuple[paddle.Tensor, paddle.Tensor]:
        """
        ...
        if input_ids is not None and (image_grid_thw is not None or 
            ...
            for i, input_ids in enumerate(total_input_ids):
                ...
                multimodal_nums = (image_nums + audio_nums if
                    use_audio_in_video else image_nums + video_nums +
                    audio_nums)
                for _ in range(multimodal_nums):
                    ...
                    if min_ed == ed_audio:
                        ...
                        if text_len != 0:
                            ...
                            llm_pos_ids_list.append(paddle.arange(end=
                                text_len).view(1, -1).expand(shape=[3, -1]) +
                                st_idx)
                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(
                            llm_pos_ids_list) > 0 else 0
                        bos_len = 1
                        llm_pos_ids_list.append(paddle.arange(end=bos_len).
                            view(1, -1).expand(shape=[3, -1]) + st_idx)
                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(
                            llm_pos_ids_list) > 0 else 0
                        audio_len = ((audio_seqlens[audio_idx] - 1) // 2 + 
                            1 - 2) // 2 + 1
                        llm_pos_ids = paddle.arange(end=audio_len).view(1, -1
                            ).expand(shape=[3, -1]) + st_idx
                        llm_pos_ids_list.append(llm_pos_ids)
                        st_idx = llm_pos_ids_list[-1].max() + 1 if len(
                            llm_pos_ids_list) > 0 else 0
                        eos_len = 1
                        llm_pos_ids_list.append(paddle.arange(end=eos_len).
                            view(1, -1).expand(shape=[3, -1]) + st_idx)
                        ...
                        t_index = (paddle.arange(end=grid_t) * 1 *
                            position_id_per_seconds).astype(dtype='int64')
                        llm_pos_ids = self.get_llm_pos_ids_for_vision(st_idx,
                        ...
                        t_index = (paddle.arange(end=grid_t) *
                            second_per_grids[video_idx].cpu().astype(dtype=
                            'float32') * position_id_per_seconds).astype(dtype
                            ='int64')
                        llm_pos_ids = self.get_llm_pos_ids_for_vision(st_idx,
                        ...
                        audio_len = ((audio_seqlens[audio_idx] - 1) // 2 + 
                            1 - 2) // 2 + 1
                        audio_llm_pos_ids = paddle.arange(end=audio_len).view(
                            [1, -1]).expand(shape=[3, -1]) + st_idx
                        grid_t = video_grid_thw[video_idx][0]
                        grid_hs = video_grid_thw[:, 1]
                        grid_ws = video_grid_thw[:, 2]
                        t_index = (paddle.arange(end=grid_t) *
                        ...
                        llm_pos_ids_list.append(paddle.arange(end=eos_len).
                            view([1, -1]).expand(shape=[3, -1]) + st_idx)
                        st += (text_len + bos_len * 2 + audio_len +
                            video_len + eos_len * 2)
                        audio_idx += 1
                        video_idx += 1
                        remain_videos -= 1
                ...
                if st < len(input_tokens):
                    ...
                    llm_pos_ids_list.append(paddle.arange(end=text_len).
                        view([1, -1]).expand(shape=[3, -1]) + st_idx)
                llm_positions = paddle.concat(x=llm_pos_ids_list, axis=1
                    ).reshape([3, -1])
                position_ids[..., i, attention_mask[i] == 1
                    ] = llm_positions.to(position_ids.place)
                mrope_position_deltas.append(llm_positions.max() + 1 -
                    len(input_ids))
            mrope_position_deltas = paddle.to_tensor(data=
                mrope_position_deltas, place=input_ids.place).unsqueeze(axis=1)
            return position_ids, mrope_position_deltas
        else:
            position_ids = attention_mask.astype(dtype='int64').cumsum(axis=-1
                ) - 1
            position_ids.masked_fill_(mask=attention_mask == 0, value=1)
            position_ids = position_ids.unsqueeze(axis=0).expand(shape=[3, 
                -1, -1]).to(attention_mask.place)
            max_position_ids = position_ids.max(0, keepdim=False)[0
                ].max(-1, keepdim=True)[0]
            mrope_position_deltas = max_position_ids + 1 - paddle.sum(x=
                attention_mask, axis=-1, keepdim=True)
            return position_ids, mrope_position_deltas
...
class Qwen2_5OmniAudioAttention(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniAudioEncoderConfig):
        ...
        self.k_proj = paddle.nn.Linear(in_features=self.embed_dim,
            out_features=self.embed_dim, bias_attr=False)
        self.v_proj = paddle.nn.Linear(in_features=self.embed_dim,
            out_features=self.embed_dim, bias_attr=True)
        self.q_proj = paddle.nn.Linear(in_features=self.embed_dim,
            out_features=self.embed_dim, bias_attr=True)
        self.out_proj = paddle.nn.Linear(in_features=self.embed_dim,
            out_features=self.embed_dim, bias_attr=True)

    def forward(self, hidden_states: paddle.Tensor, cu_seqlens: Optional[
        ...
        attention_mask = paddle.full(shape=[1, seq_length, tuple(key_states
            .shape)[1]], fill_value=paddle.finfo(dtype=query_states.dtype).
            min, dtype=query_states.dtype)
        for i in range(1, len(cu_seqlens)):
            attention_mask[..., cu_seqlens[i - 1]:cu_seqlens[i], cu_seqlens
                [i - 1]:cu_seqlens[i]] = 0
        attn_weights = attn_weights + attention_mask
        attn_weights = paddle.nn.functional.softmax(x=attn_weights, axis=-1
            ).astype(query_states.dtype)
        attn_output = paddle.matmul(x=attn_weights, y=value_states).transpose([1,0,2]).reshape([seq_length, self.embed_dim])
...
class Qwen2_5OmniAudioFlashAttention2(Qwen2_5OmniAudioAttention):
    ...
    def forward(self, hidden_states: paddle.Tensor, cu_seqlens: Optional[
        ...
        attn_output = paddle.nn.functional.flash_attention.flash_attn_unpadded(
            ...
            cu_seqlens_q=cu_seqlens, cu_seqlens_k=cu_seqlens, max_seqlen_q=
            max_seqlen, max_seqlen_k=max_seqlen, dropout=0.0, scale=paddle.
            utils.try_import('math').sqrt(query_states.shape[-1]))[0]
...
class Qwen2_5OmniAudioSdpaAttention(Qwen2_5OmniAudioAttention):
    ...
    def forward(self, hidden_states: paddle.Tensor, cu_seqlens: Optional[
        ...
        attention_mask = paddle.zeros(shape=[1, seq_length, tuple(
            key_states.shape)[0]], dtype='bool')
        for i in range(1, len(cu_seqlens)):
            attention_mask[..., cu_seqlens[i - 1]:cu_seqlens[i], cu_seqlens
                [i - 1]:cu_seqlens[i]] = paddle.finfo(query_states.dtype).min
        ...
        attn_output = paddle.nn.functional.scaled_dot_product_attention(query
            ...
            attention_mask, dropout_p=self.dropout if self.training else 0.0)
...
class Qwen2_5OmniAudioEncoderLayer(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniAudioEncoderConfig):
        ...
        self.self_attn_layer_norm = paddle.nn.LayerNorm(normalized_shape=
            self.embed_dim)
        self.dropout = config.dropout
        self.activation_fn = ACT2FN[config.activation_function]
        self.activation_dropout = config.activation_dropout
        self.fc1 = paddle.nn.Linear(in_features=self.embed_dim,
            out_features=config.encoder_ffn_dim)
        self.fc2 = paddle.nn.Linear(in_features=config.encoder_ffn_dim,
            out_features=self.embed_dim)
        self.final_layer_norm = paddle.nn.LayerNorm(normalized_shape=self.
            embed_dim)

    def forward(self, hidden_states: paddle.Tensor, cu_seqlens: paddle.Tensor
        ) ->paddle.Tensor:
        """
        Args:
        ...
        hidden_states = residual + hidden_states
        if hidden_states.dtype == paddle.float16 and (paddle.isinf(x=
            hidden_states).astype('bool').any() or paddle.isnan(x=
            hidden_states).astype('bool').any()):
            clamp_value = paddle.finfo(dtype=hidden_states.dtype).max - 1000
            hidden_states = paddle.clip(x=hidden_states, min=-clamp_value,
                max=clamp_value)
...
class SinusoidsPositionEmbedding(paddle.nn.Layer):
    ...
    def __init__(self, length, channels, max_timescale=10000):
        ...
        inv_timescales = paddle.exp(x=-log_timescale_increment * paddle.
            arange(end=channels // 2))
        scaled_time = paddle.arange(end=length,dtype="float32")[:, np.newaxis
            ] * inv_timescales[np.newaxis, :]
        self.register_buffer(name='positional_embedding', tensor=paddle.
            concat(x=[paddle.sin(x=scaled_time), paddle.cos(x=scaled_time)],
            axis=1), persistable=False)
...
class Qwen2_5OmniAudioEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniAudioEncoderConfig,**kwargs):
        ...
        self.conv1 = paddle.nn.Conv1D(in_channels=self.num_mel_bins,
            out_channels=embed_dim, kernel_size=3, padding=1)
        self.conv2 = paddle.nn.Conv1D(in_channels=embed_dim, out_channels=
            embed_dim, kernel_size=3, stride=2, padding=1)
        self.positional_embedding = SinusoidsPositionEmbedding(self.
            max_source_positions, embed_dim)
        self.audio_bos_eos_token = paddle.nn.Embedding(num_embeddings=2,
            embedding_dim=config.output_dim)
        self.layers = paddle.nn.LayerList(sublayers=[
            Qwen2_5OmniAudioEncoderLayer(config) for _ in range(config.
            encoder_layers)])
        self.ln_post = paddle.nn.LayerNorm(normalized_shape=config.d_model)
        self.avg_pooler = paddle.nn.AvgPool1D(kernel_size=2, stride=2,
            exclusive=False)
        self.proj = paddle.nn.Linear(in_features=config.d_model,
    ...
    def forward(self, input_features, feature_lens=None, aftercnn_lens=None):
        chunk_num = paddle.ceil(x=feature_lens / (self.n_window * 2)).astype(
            dtype='int64')
        chunk_lengths = paddle.to_tensor(data=[self.n_window * 2] * int(chunk_num.sum()), dtype='int64', place=feature_lens.place)
        tail_chunk_index = paddle.nn.functional.pad(x=chunk_num, pad=(1, 0),
            value=-1, pad_from_left_axis=False).cumsum(axis=0)[1:]
        chunk_lengths[tail_chunk_index] = feature_lens % (self.n_window * 2)
        chunk_lengths = paddle.where(condition=chunk_lengths == 0, x=self.
            n_window * 2, y=chunk_lengths)
        chunk_list = input_features.split(chunk_lengths.tolist(), axis=1)
        padded_feature, padded_mask, padded_mask_after_cnn = (self.
            padded_and_mask_function(chunk_list, chunk_lengths,
            padding_value=0, padding_side='right'))
        padded_embed = paddle.nn.functional.gelu(x=self.conv1(padded_feature).astype("float32")* padded_mask.astype("float32")).cast(dtype=self.dtype) 
        padded_embed = paddle.nn.functional.gelu(x=self.conv2(padded_embed).astype("float32")).transpose([0,2,1]).cast(dtype=self.dtype)
        padded_embed = (padded_embed + self.positional_embedding.
            positional_embedding[:tuple(padded_embed.shape)[1], :].
            unsqueeze(axis=0).to(padded_embed.dtype))
        hidden_states = padded_embed[padded_mask_after_cnn]
        cu_seqlens = paddle.concat(x=(paddle.zeros(shape=[1], dtype='int64'), padded_mask_after_cnn.sum(axis=1).cumsum(axis=0))).astype('int32')
        tmp_hidden_states = []
        ...
        for each_audio_states in hidden_states_list:
            each_audio_states = self.avg_pooler(each_audio_states.transpose([1,0]).unsqueeze(0)).squeeze(0).transpose([1,0])
    ...
    def padded_and_mask_function(self, tensor_list, tensor_len,
        padding_value=0, padding_side='right'):
        max_len = tensor_len.max()
        dim = tuple(tensor_list[0].shape)[0]
        padded_tensor = paddle.full(shape=(len(tensor_list), dim, max_len),
            fill_value=padding_value, dtype=self.dtype)
        batch_mask = paddle.zeros(shape=(len(tensor_len), max_len), dtype=
            'int64')
        ...
        batch_mask_after_cnn = paddle.zeros(shape=(len(tensor_len),
            max_len_after_cnn), dtype='int64')
        for i, length in enumerate(feature_lens_after_cnn):
            batch_mask_after_cnn[i, :length] = 1
        return padded_tensor, batch_mask.unsqueeze(axis=1
            ), batch_mask_after_cnn.astype(dtype='bool')
...
def apply_rotary_pos_emb_vision(tensor: paddle.Tensor, freqs: paddle.Tensor
    ) ->paddle.Tensor:
    orig_dtype = tensor.dtype
    tensor = tensor.astype(dtype='float32')
    cos = freqs.cos()
    sin = freqs.sin()
    cos = cos.unsqueeze(axis=1).tile(repeat_times=[1, 1, 2]).unsqueeze(axis=0
        ).astype(dtype='float32')
    sin = sin.unsqueeze(axis=1).tile(repeat_times=[1, 1, 2]).unsqueeze(axis=0
        ).astype(dtype='float32')
...
class Qwen2_5OmniVisionAttention(paddle.nn.Layer):
    ...
    def __init__(self, dim: int, num_heads: int=16) ->None:
        ...
        self.q = paddle.nn.Linear(in_features=dim, out_features=dim,
            bias_attr=True)
        self.k = paddle.nn.Linear(in_features=dim, out_features=dim,
            bias_attr=True)
        self.v = paddle.nn.Linear(in_features=dim, out_features=dim,
            bias_attr=True)
        self.proj = paddle.nn.Linear(in_features=dim, out_features=dim)

    def forward(self, hidden_states: paddle.Tensor, cu_seqlens: paddle.
        Tensor, rotary_pos_emb: paddle.Tensor=None) ->paddle.Tensor:
        ...
        q = apply_rotary_pos_emb_vision(q.unsqueeze(axis=0), rotary_pos_emb
            ).squeeze(axis=0)
        k = apply_rotary_pos_emb_vision(k.unsqueeze(axis=0), rotary_pos_emb
            ).squeeze(axis=0)
        attention_mask = paddle.full(shape=[1, seq_length, seq_length],
            fill_value=paddle.finfo(dtype=q.dtype).min, dtype=q.dtype)
        for i in range(1, len(cu_seqlens)):
            attention_mask[..., cu_seqlens[i - 1]:cu_seqlens[i], cu_seqlens
                [i - 1]:cu_seqlens[i]] = 0
        q = q.transpose([1,0,2])
...
class Qwen2_5OmniVisionFlashAttention2(paddle.nn.Layer):
    ...
    def _apply_rotary_pos_emb_flashatt(self, tensor: paddle.Tensor, freqs:
        paddle.Tensor) ->paddle.Tensor:
        tensor_ = tensor.astype(dtype='float32')
        cos = freqs.cos()
        sin = freqs.sin()
        # apply rotary emb
        output = flash_attn.layers.rotary.apply_rotary_emb(tensor_, cos, sin
            ).astype(dtype=tensor.dtype)
        return output
    ...
    def forward(self, hidden_states: paddle.Tensor, cu_seqlens: paddle.
        ...
        k = self.k(hidden_states).reshape(seq_length, self.num_heads, -1)
        v = self.v(hidden_states).reshape(seq_length, self.num_heads, -1)
        q = self._apply_rotary_pos_emb_flashatt(q.unsqueeze(axis=0),
            rotary_pos_emb).squeeze(axis=0)
        k = self._apply_rotary_pos_emb_flashatt(k.unsqueeze(axis=0),
            rotary_pos_emb).squeeze(axis=0)
        max_seqlen = (cu_seqlens[1:] - cu_seqlens[:-1]).max().item()
        assert paddle.device.cuda.get_device_capability()[0
            ] >= 8, 'Fault: Your device computational capabilities less 8'
        attn_output = paddle.nn.functional.flash_attention.flash_attn_unpadded(
            query=q, key=k, value=v, cu_seqlens_q=cu_seqlens, cu_seqlens_k=
            cu_seqlens, max_seqlen_q=max_seqlen, max_seqlen_k=max_seqlen,
            scale=paddle.utils.try_import('math').sqrt(q.shape[-1]))[0
...
class Qwen2_5OmniVisionSdpaAttention(paddle.nn.Layer):
    ...
    def forward(self, hidden_states: paddle.Tensor, cu_seqlens: paddle.
        ...
        # attention_mask = paddle.zeros(shape=[1, seq_length, seq_length],
        #     dtype=q.dtype)
        # for i in range(1, len(cu_seqlens)):
        #     attention_mask[..., cu_seqlens[i - 1]:cu_seqlens[i], cu_seqlens
        #         [i - 1]:cu_seqlens[i]] = paddle.finfo(q.dtype).min
        attn_output = paddle.nn.functional.scaled_dot_product_attention(query
            =q, key=k, value=v, attn_mask=None, dropout_p=0.0)
        attn_output = attn_output.reshape([seq_length, -1])
        attn_output = self.proj(attn_output)
        return attn_output
...
class Qwen2RMSNorm(paddle.nn.Layer):
    ...
    def extra_repr(self):
        return (
            f'{tuple(tuple(self.weight.shape))}, eps={self.variance_epsilon}')


QWEN2_5_OMNI_VISION_ATTENTION_CLASSES = {'eager':
    Qwen2_5OmniVisionAttention, 'flash_attention_2':
    Qwen2_5OmniVisionFlashAttention2, 'sdpa': Qwen2_5OmniVisionSdpaAttention}
...
class Qwen2_5OmniVisionBlock(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniVisionEncoderConfig) ->None:
        ...
        self.attn = QWEN2_5_OMNI_VISION_ATTENTION_CLASSES[config.get('_attn_implementation','eager')](config.hidden_size, num_heads=config.
            num_heads)
...
class Qwen2_5_VisionPatchEmbed(paddle.nn.Layer):
    ...
    def __init__(self, patch_size: int=14, temporal_patch_size: int=2,
        ...
        self.proj = paddle.nn.Conv3D(in_channels=in_channels, out_channels=
            embed_dim, kernel_size=kernel_size, stride=kernel_size,
            bias_attr=False)

    def forward(self, hidden_states: paddle.Tensor) ->paddle.Tensor:
        target_dtype = self.proj.weight.dtype
        hidden_states = hidden_states.view([-1, self.in_channels, self.
            temporal_patch_size, self.patch_size, self.patch_size])
        hidden_states = self.proj(hidden_states.astype(target_dtype)).view([
            -1, self.embed_dim])
        return hidden_states
...
class Qwen2_5_VisionRotaryEmbedding(paddle.nn.Layer):
    ...
    def __init__(self, dim: int, theta: float=10000.0) ->None:
        super().__init__()
        inv_freq = 1.0 / theta ** (paddle.arange(start=0, end=dim, step=2,
            dtype='float32') / dim)
...
class Qwen2_5OmniPatchMerger(paddle.nn.Layer):
    ...
    def __init__(self, dim: int, context_dim: int, spatial_merge_size: int=2
        ...
        self.mlp = paddle.nn.Sequential(paddle.nn.Linear(in_features=self.
            hidden_size, out_features=self.hidden_size), paddle.nn.GELU(),
...
class Qwen2_5OmniVisionEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniVisionEncoderConfig, **
        ...
        self.spatial_merge_unit = (self.spatial_merge_size * self.
            spatial_merge_size)
        self.patch_embed = Qwen2_5_VisionPatchEmbed(patch_size=config.
            patch_size, temporal_patch_size=config.temporal_patch_size,
            in_channels=config.in_channels, embed_dim=config.hidden_size)
        head_dim = config.hidden_size // config.num_heads
        self.rotary_pos_emb = Qwen2_5_VisionRotaryEmbedding(head_dim // 2)
        self.blocks = paddle.nn.LayerList(sublayers=[Qwen2_5OmniVisionBlock
            (config) for _ in range(config.depth)])
        self.merger = Qwen2_5OmniPatchMerger(dim=config.out_hidden_size,
            context_dim=config.hidden_size, spatial_merge_size=config.
            spatial_merge_size)
        self.gradient_checkpointing = False
    ...
    def rot_pos_emb(self, grid_thw):
        ...
        for t, h, w in grid_thw:
            hpos_ids = paddle.arange(end=h).unsqueeze(axis=1).expand(shape=
                [-1, w])
            hpos_ids = hpos_ids.reshape([h // self.spatial_merge_size, self.
                spatial_merge_size, w // self.spatial_merge_size, self.
                spatial_merge_size])
            hpos_ids = hpos_ids.transpose(perm=[0, 2, 1, 3])
            hpos_ids = hpos_ids.flatten()
            wpos_ids = paddle.arange(end=w).unsqueeze(axis=0).expand(shape=
                [h, -1])
            wpos_ids = wpos_ids.reshape([h // self.spatial_merge_size, self.
                spatial_merge_size, w // self.spatial_merge_size, self.
                spatial_merge_size])
            wpos_ids = wpos_ids.transpose(perm=[0, 2, 1, 3])
            wpos_ids = wpos_ids.flatten()
            pos_ids.append(paddle.stack(x=[hpos_ids, wpos_ids], axis=-1).
                tile(repeat_times=[t, 1]))
    ...
    def get_window_index(self, grid_thw):
        ...
        window_index_id = 0
        vit_merger_window_size = (self.window_size // self.
            spatial_merge_size // self.patch_size)
        for grid_t, grid_h, grid_w in grid_thw:
            llm_grid_h, llm_grid_w = (grid_h // self.spatial_merge_size, 
                grid_w // self.spatial_merge_size)
            index = paddle.arange(end=grid_t * llm_grid_h * llm_grid_w
                ).reshape([grid_t, llm_grid_h, llm_grid_w])
            pad_h = (vit_merger_window_size - llm_grid_h %
                vit_merger_window_size)
            pad_w = (vit_merger_window_size - llm_grid_w %
                vit_merger_window_size)
            ...
            index_padded = index_padded.transpose(perm=[0, 1, 3, 2, 4]
                ).reshape([grid_t, num_windows_h * num_windows_w,
    ...
    def forward(self, hidden_states: paddle.Tensor, grid_thw: paddle.Tensor
        ...
        cu_window_seqlens = paddle.to_tensor(data=cu_window_seqlens, dtype=
            'int32',
        ...
        hidden_states = hidden_states.reshape([seq_len // self.
            spatial_merge_unit, self.spatial_merge_unit, -1])
        hidden_states = hidden_states[window_index, :, :]
        hidden_states = hidden_states.reshape([seq_len, -1])
        rotary_pos_emb = rotary_pos_emb.reshape([seq_len // self.
            spatial_merge_unit, self.spatial_merge_unit, -1])
        rotary_pos_emb = rotary_pos_emb[window_index, :, :]
        rotary_pos_emb = rotary_pos_emb.reshape([seq_len, -1])
        cu_seqlens = paddle.repeat_interleave(x=grid_thw[:, 1] * grid_thw[:,
            2], repeats=grid_thw[:, 0]).cumsum(axis=0, dtype='int32')
        cu_seqlens = paddle.nn.functional.pad(x=cu_seqlens, pad=(1, 0),
...
class Qwen2_5OmniRotaryEmbedding(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig, device=None):
        ...
        if hasattr(config, 'rope_scaling') and config.rope_scaling is not None:
            self.rope_type = config.rope_scaling.get('rope_type', config.
    ...
    def _dynamic_frequency_update(self, position_ids, device):
        ...
        if seq_len > self.max_seq_len_cached:
            inv_freq, self.attention_scaling = self.rope_init_fn(self.
                config, device, seq_len=seq_len, **self.rope_kwargs)
            self.register_buffer(name='inv_freq', tensor=inv_freq,
                persistable=False)
            self.max_seq_len_cached = seq_len
        if (seq_len < self.original_max_seq_len and self.max_seq_len_cached >
            self.original_max_seq_len):
            self.register_buffer(name='inv_freq', tensor=self.
                original_inv_freq, persistable=False)
    ...
    def forward(self, x, position_ids):
        ...
        inv_freq_expanded = self.inv_freq[None, None, :, None].astype(dtype
            ='float32').expand(shape=[3, tuple(position_ids.shape)[1], -1, 1])
        position_ids_expanded = position_ids[:, :, None, :].astype(dtype=
            'float32')
...
def apply_multimodal_rotary_pos_emb(q, k, cos, sin, mrope_section,
    ...
    cos = paddle.concat(x=[m[i % 3] for i, m in enumerate(cos.split(
        mrope_section, axis=-1))], axis=-1).unsqueeze(axis=unsqueeze_dim)
    sin = paddle.concat(x=[m[i % 3] for i, m in enumerate(sin.split(
        mrope_section, axis=-1))], axis=-1).unsqueeze(axis=unsqueeze_dim)
...
class Qwen2_5OmniAttention(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniConfig, layer_idx: Optional[int]=None
        ...
        self.head_dim = getattr(config, 'head_dim', self.hidden_size //
            self.num_heads)
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        ...
        query_states, key_states = apply_multimodal_rotary_pos_emb(query_states
        ...
        attn_weights = paddle.matmul(x=query_states, y=key_states.transpose(perm=[0,1,3,2])) / math.sqrt(self.head_dim)
        if attention_mask is not None:
            causal_mask = attention_mask[:, :, :, :tuple(key_states.shape)[-2]]
            attn_weights = attn_weights + causal_mask
        if query_states.dtype == 'float16':
            attn_weights = paddle.where(condition=paddle.isinf(x=
                attn_weights), x=paddle.zeros_like(x=attn_weights), y=
                attn_weights)
        attn_weights = paddle.nn.functional.softmax(x=attn_weights, axis=-1,
            dtype='float32').astype(query_states.dtype)
...
class Qwen2_5OmniFlashAttention2(Qwen2_5OmniAttention):
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        ...
        query_states = query_states.view(bsz, q_len, -1, self.head_dim
            ).transpose(perm=dim2perm(query_states.view(bsz, q_len, -1,
            self.head_dim).ndim, 1, 2))
        key_states = key_states.view(bsz, q_len, -1, self.head_dim).transpose(
            perm=dim2perm(key_states.view(bsz, q_len, -1, self.head_dim).
            ndim, 1, 2))
        value_states = value_states.view(bsz, q_len, -1, self.head_dim
            ).transpose(perm=dim2perm(value_states.view(bsz, q_len, -1,
            self.head_dim).ndim, 1, 2))
        cos, sin = position_embeddings
        query_states, key_states = apply_multimodal_rotary_pos_emb(query_states
            , key_states, cos, sin, self.rope_scaling['mrope_section'])
        ...
        query_states = query_states.transpose(perm=dim2perm(query_states.
            ndim, 1, 2))
        key_states = key_states.transpose(perm=dim2perm(key_states.ndim, 1, 2))
        value_states = value_states.transpose(perm=dim2perm(value_states.
            ndim, 1, 2))
        if self.config.use_sliding_window and getattr(self.config,
            'sliding_window', None
            ) is not None and self.layer_idx >= self.config.max_window_layers:
        ...
        attn_output = _flash_attention_forward(query_states, key_states,
            ...
            sliding_window=sliding_window, is_causal=self.is_causal,
...
class Qwen2_5OmniSdpaAttention(Qwen2_5OmniAttention):
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        Optional[paddle.Tensor]=None, position_ids: Optional[paddle.Tensor]
        =None, past_key_value: Optional[paddle.Tensor]=None, output_attentions:
        bool=False, use_cache: bool=False, cache_position: Optional[paddle.
        Tensor]=None, position_embeddings: Optional[Tuple[paddle.Tensor,
        paddle.Tensor]]=None) ->Tuple[paddle.Tensor, Optional[paddle.Tensor
        ], Optional[Tuple[paddle.Tensor]]]:
        ...
        attn_output = paddle.nn.functional.scaled_dot_product_attention(query
            =query_states, key=key_states, value=value_states, attn_mask=
            None, dropout_p=self.attention_dropout if self.training else
            0.0, is_causal=is_causal)
        # attn_output = attn_output.transpose(perm=dim2perm(attn_output.ndim,
        #     1, 2)).contiguous()
...
class Qwen2_5OmniDecoderLayer(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniConfig, layer_idx: int):
        ...
        if (config.use_sliding_window and self._attn_implementation !=
            'flash_attention_2'):
            logger.warning_once(
        ...
        self.self_attn = QWEN2_5_OMNI_ATTENTION_CLASSES[self.
            _attn_implementation](config, layer_idx)
    ...
    def forward(self, hidden_states: paddle.Tensor, attention_mask:
        ...
        False, cache_position: Optional[paddle.Tensor]=None,
        position_embeddings: Optional[Tuple[paddle.Tensor, paddle.Tensor]]=
        None, **kwargs) ->Tuple[paddle.Tensor, Optional[Tuple[paddle.Tensor,
        paddle.Tensor]]]:
        """
        Args:
            hidden_states (`torch.FloatTensor`): input to the layer of shape `(batch, seq_len, embed_dim)`
        ...
        hidden_states, self_attn_weights, present_key_value = self.self_attn(
            ...
            cache_position=cache_position, position_embeddings=
            position_embeddings)
        hidden_states = residual + hidden_states
        residual = hidden_states
...
class Qwen2_5OmniThinkerTextModel(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniTextConfig,**kwargs):
        ...
        self.embed_tokens = paddle.nn.Embedding(num_embeddings=config.
            ...
            padding_idx)
        self.layers = paddle.nn.LayerList(sublayers=[
            Qwen2_5OmniDecoderLayer(config, layer_idx) for layer_idx in
    ...
    def forward(self, input_ids: paddle.Tensor=None, attention_mask:
        ...
        inputs_embeds: Optional[paddle.Tensor]=None, use_cache: Optional[
        bool]=None, output_attentions: Optional[bool]=None,
        output_hidden_states: Optional[bool]=None, return_dict: Optional[
        bool]=None, cache_position: Optional[paddle.Tensor]=None) ->Union[
        ...
        if cache_position is None:
            ...
            cache_position = paddle.arange(start=past_seen_tokens, end=
                past_seen_tokens + tuple(inputs_embeds.shape)[1])
        if position_ids is None:
            position_ids = cache_position.view([1, 1, -1]).expand(shape=[3,
                tuple(inputs_embeds.shape)[0], -1])
        elif position_ids.dim() == 2:
            position_ids = position_ids[None, ...].expand(shape=[3, tuple(
        ...
        for decoder_layer in self.layers:
            ...
            if self.gradient_checkpointing and self.training:
                layer_outputs = self._gradient_checkpointing_func(decoder_layer
                    .__call__, hidden_states, causal_mask, position_ids,
                    past_key_values, output_attentions, use_cache,
                    cache_position, position_embeddings)
            else:
                layer_outputs = decoder_layer(hidden_states, attention_mask
                    =causal_mask, position_ids=position_ids, past_key_value
    ...
    def _update_causal_mask(self, attention_mask: paddle.Tensor,
        input_tensor: paddle.Tensor, cache_position: paddle.Tensor,
        past_key_values: Tuple[paddle.Tensor], output_attentions: bool=False):
        if self._attn_implementation == 'flash_attention_2':
            if attention_mask is not None and past_key_values is not None:
                is_padding_right = attention_mask[:, -1].sum().item() != tuple(
        ...
        #     SlidingWindowCache)
        if self._attn_implementation == 'sdpa' and not (
            using_static_cache or using_sliding_window_cache
            ) and not output_attentions:
            if self._ignore_causal_mask_sdpa(attention_mask,
                inputs_embeds=input_tensor, past_key_values_length=
                past_seen_tokens, sliding_window=self.config.sliding_window,
        ...
        if using_sliding_window_cache or using_static_cache:
            ...
            target_length = tuple(attention_mask.shape)[-1] if isinstance(
                attention_mask, paddle.Tensor
                ) else past_seen_tokens + sequence_length + 1
        causal_mask = (self._prepare_4d_causal_attention_mask_with_cache_position(attention_mask, sequence_length=sequence_length, target_length=target_length, dtype=dtype, cache_position=cache_position, batch_size=tuple(input_tensor.shape)[0], config=self.config, past_key_values=past_key_values))
    ...
    def _ignore_causal_mask_sdpa(
        ...
        if attention_mask is None:
            if (
                is_training and (query_length == 1 or key_value_length == query_length)
    ...
    @staticmethod
    def _prepare_4d_causal_attention_mask_with_cache_position(attention_mask:
        paddle.Tensor, sequence_length: int, target_length: int, dtype:
        paddle.dtype,cache_position: paddle.Tensor, batch_size: int, config:
        Qwen2_5OmniConfig, past_key_values: Tuple[paddle.Tensor]):
        """
        Creates a causal 4D mask of shape `(batch_size, 1, query_length, key_value_length)` from a 2D mask of shape
        `(batch_size, key_value_length)`, or if the input `attention_mask` is already 4D, do nothing.
        ...
        if attention_mask is not None and attention_mask.dim() == 4:
            ...
            diagonal_attend_mask = paddle.arange(end=target_length
                ) > cache_position.reshape([-1, 1])
            if config.sliding_window is not None:
                ...
                if sequence_length > target_length:
                    sliding_attend_mask = paddle.arange(end=target_length
                        ) <= cache_position.reshape([-1, 1]
            ...
            if attention_mask is not None:
                ...
                mask_length = tuple(attention_mask.shape)[-1]
                padding_mask = causal_mask[:, :, :, :mask_length
                    ] + attention_mask[:, None, None, :].to(causal_mask.place).astype(causal_mask.dtype)
                padding_mask = padding_mask == 0
                causal_mask[:, :, :, :mask_length] = causal_mask[:, :, :, :
                    mask_length].masked_fill(mask=padding_mask, value=min_dtype
                    )
    ...
    def _unmask_unattended(
        ...
        return expanded_mask.multiply((~paddle.all(expanded_mask == min_dtype, axis=-1, keepdim=True)).astype(expanded_mask.dtype))
...
class Qwen2_5OmniThinkerForConditionalGeneration(
    ...
    def __init__(self, config: Qwen2_5OmniThinkerConfig):
        ...
        self.audio_tower = Qwen2_5OmniAudioEncoder._from_config(config.audio_config, attn_implementation=config.get('_attn_implementation',"eager"))
        self.visual = Qwen2_5OmniVisionEncoder._from_config(config.vision_config, attn_implementation=config.get('_attn_implementation',"eager"))
        self.vocab_size = config.text_config.vocab_size
        self.model = Qwen2_5OmniThinkerTextModel._from_config(config.text_config, attn_implementation=config.get('_attn_implementation',"eager"))
        self.lm_head = paddle.nn.Linear(in_features=config.text_config.
            hidden_size, out_features=config.text_config.vocab_size,
            bias_attr=False)
        self.pad_token_id = (self.config.pad_token_id if self.config.
            pad_token_id is not None else -1)
    ...
    def forward(self, input_ids: Optional[paddle.Tensor]=None,
        input_features: Optional[paddle.Tensor]=None, pixel_values:
        Optional[paddle.Tensor]=None, pixel_values_videos: Optional[paddle.
        Tensor]=None, image_grid_thw: Optional[paddle.Tensor]=None,
        video_grid_thw: Optional[paddle.Tensor]=None, attention_mask:
        Optional[paddle.Tensor]=None, feature_attention_mask: Optional[
        paddle.Tensor]=None, audio_feature_lengths: Optional[paddle.Tensor]
        =None, position_ids: Optional[paddle.Tensor]=None, past_key_values:
        Optional[List[paddle.Tensor]]=None, inputs_embeds: Optional[paddle.
        Tensor]=None, rope_deltas: Optional[paddle.Tensor]=None, labels:
        Optional[paddle.Tensor]=None, use_cache: Optional[bool]=None,
        output_attentions: Optional[bool]=None, output_hidden_states:
        Optional[bool]=None, return_dict: Optional[bool]=None,
        use_audio_in_video: Optional[bool]=None, cache_position: Optional[
        paddle.Tensor]=None, video_second_per_grid: Optional[paddle.Tensor]
        =None) ->Union[Tuple, Qwen2_5OmniThinkerCausalLMOutputWithPast]:
        """
        Args:
        ...
        if feature_attention_mask is not None:
            ...
            input_features = input_features.transpose(perm=[0, 2, 1])[feature_attention_mask.astype(dtype='bool')].transpose(perm=[1, 0])
        else:
            audio_feature_lengths = None
        # print("attention_mask:",attention_mask.shape)
        # import pdb;pdb.set_trace()
        if attention_mask is not None and position_ids is None:
            if (cache_position is None or cache_position is not None and 
                cache_position[0] == 0 or self.rope_deltas is None):
                ...
                position_ids, rope_deltas = self.get_rope_index(input_ids,
                    ...
                    use_audio_in_video, audio_feature_lengths,
                ...
                delta = cache_position[0
                    ] + self.rope_deltas if cache_position is not None else 0
                position_ids = paddle.arange(end=seq_length)
                position_ids = position_ids.view([1, -1]).expand(shape=[
                    batch_size, -1])
                position_ids = position_ids.add(delta)
                position_ids = position_ids.unsqueeze(axis=0).expand(shape=
                    [3, -1, -1])
        if inputs_embeds is None:
        ...
        if tuple(input_ids.shape)[1] != 1:
            if input_features is not None:
                audio_feat_lengths, audio_output_lengths = (self.
                    audio_tower._get_feat_extract_output_lengths(
                    audio_feature_lengths if audio_feature_lengths is not
                    None else feature_attention_mask.sum(axis=-1)))
                feature_lens = (audio_feature_lengths if 
                    audio_feature_lengths is not None else
                    feature_attention_mask.sum(axis=-1))
                audio_outputs = self.audio_tower(input_features,
                    feature_lens=feature_lens, aftercnn_lens=audio_feat_lengths
                    )
                audio_features = audio_outputs.last_hidden_state
                if tuple(audio_features.shape)[0] != sum(audio_output_lengths
                    .tolist()):
                    raise ValueError(
                        'length of audio_features should match audio_output_lengths'
                        )
                audio_mask = (input_ids == self.config.audio_token_index
                    ).unsqueeze(axis=-1).expand_as(y=inputs_embeds).to(
                    inputs_embeds.place)
                audio_features = audio_features.to(inputs_embeds.place).astype(inputs_embeds.dtype)
                # TODO
                """Not Support auto convert *.masked_scatter, please judge whether it is Pytorch API and convert by yourself"""
                inputs_embeds = inputs_embeds.masked_scatter(audio_mask,
                    audio_features)
                """Not Support auto convert *.masked_scatter, please judge whether it is Pytorch API and convert by yourself"""
                embeds_to_talker = embeds_to_talker.masked_scatter(audio_mask,
                    paddle.zeros_like(x=audio_features))
            if pixel_values is not None:
                ...
                image_mask = (input_ids == self.config.image_token_index
                    ...
                    inputs_embeds.place)
                image_embeds = image_embeds.astype(inputs_embeds.place,
                    inputs_embeds.dtype)
                """Not Support auto convert *.masked_scatter, please judge whether it is Pytorch API and convert by yourself"""
                inputs_embeds = inputs_embeds.masked_scatter(image_mask,
                    image_embeds)
                """Not Support auto convert *.masked_scatter, please judge whether it is Pytorch API and convert by yourself"""
                embeds_to_talker = embeds_to_talker.masked_scatter(image_mask,
                    paddle.zeros_like(x=image_embeds))
            if pixel_values_videos is not None:
                pixel_values_videos = pixel_values_videos.astype(self.
                    visual.get_dtype())
                ...
                """Not Support auto convert *.masked_scatter, please judge whether it is Pytorch API and convert by yourself"""
                inputs_embeds = inputs_embeds.masked_scatter(video_mask,
                    video_embeds)
                """Not Support auto convert *.masked_scatter, please judge whether it is Pytorch API and convert by yourself"""
                embeds_to_talker = embeds_to_talker.masked_scatter(video_mask,
                    paddle.zeros_like(x=video_embeds))
            if attention_mask is not None:
                attention_mask = attention_mask.to(inputs_embeds.place)
        outputs = self.model(attention_mask=attention_mask, position_ids=
        ...
        return Qwen2_5OmniThinkerCausalLMOutputWithPast(loss=loss, logits=
            logits, past_key_values=outputs.past_key_values, hidden_states=
            (embeds_to_talker, outputs.hidden_states), attentions=outputs.
            attentions, attention_mask=attention_mask, rope_deltas=self.
            rope_deltas)

    def prepare_inputs_for_generation(self, input_ids, past_key_values=None,
        attention_mask=None, inputs_embeds=None, cache_position=None,
        position_ids=None, use_cache=True, pixel_values=None,
        pixel_values_videos=None, image_grid_thw=None, video_grid_thw=None,
        input_features=None, feature_attention_mask=None,
        use_audio_in_video=False, video_second_per_grid=None,return_dict=False, **kwargs):
        ...
        if use_cache == True:
            ...
            if past_key_values is not None and past_key_values[0][0] is not None:
                cache_position = paddle.arange(len(input_ids),len(input_ids)+1)
                input_ids = input_ids[:, -1:]
        
        model_inputs = dict(input_ids=input_ids,
            past_key_values=past_key_values, attention_mask=attention_mask,
            inputs_embeds=inputs_embeds, cache_position=cache_position,
            position_ids=position_ids, use_cache=use_cache, pixel_values=
            pixel_values, pixel_values_videos=pixel_values_videos,
            image_grid_thw=image_grid_thw, video_grid_thw=video_grid_thw,
            input_features=input_features, feature_attention_mask=
            feature_attention_mask, use_audio_in_video=use_audio_in_video,
            video_second_per_grid=video_second_per_grid,return_dict=return_dict)
    ...
    def update_model_kwargs_for_generation(self, outputs: ModelOutput,
        ...
        model_kwargs = super().update_model_kwargs_for_generation(outputs,
            model_kwargs, is_encoder_decoder)
...
class Qwen2_5OmniTalkerModel(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniTalkerConfig):
        ...
        self.vocab_size = config.vocab_size
        self.embed_tokens = paddle.nn.Embedding(num_embeddings=config.
            vocab_size, embedding_dim=config.embedding_size, padding_idx=
            self.padding_idx)
        self.layers = paddle.nn.LayerList(sublayers=[
            Qwen2_5OmniDecoderLayer(config, layer_idx) for layer_idx in
            range(config.num_hidden_layers)])
    ...
    def _update_causal_mask(self, attention_mask: paddle.Tensor,
        ...
        causal_mask = (self.
            ...
            attention_mask, sequence_length=sequence_length, target_length=
            target_length, dtype=dtype, cache_position=
            cache_position, batch_size=tuple(input_tensor.shape)[0], config
            =self.config, past_key_values=past_key_values))
        if (self._attn_implementation == 'sdpa' and attention_mask
             is not None and not output_attentions):
            causal_mask = self._unmask_unattended(causal_mask, min_dtype)
...
class Qwen2_5OmniTalkerForConditionalGeneration(
    ...
    def __init__(self, config: Qwen2_5OmniTalkerConfig):
        super().__init__(config)
        self.thinker_to_talker_proj = paddle.nn.Linear(in_features=config.
            embedding_size, out_features=config.hidden_size)
        self.model = Qwen2_5OmniTalkerModel(config)
        self.codebook_size = config.vocab_size
        self.codec_head = paddle.nn.Linear(in_features=config.hidden_size,
            out_features=self.codebook_size, bias_attr=False)
    ...
    def forward(self, input_ids: paddle.Tensor=None, attention_mask:
        Optional[paddle.Tensor]=None, position_ids: Optional[paddle.Tensor]
        =None, past_key_values: Optional[List[paddle.Tensor]]=None,
        thinker_reply_part: Optional[paddle.Tensor]=None, inputs_embeds:
        Optional[paddle.Tensor]=None, rope_deltas: Optional[paddle.Tensor]=
        None, use_cache: Optional[bool]=None, cache_position: Optional[
        paddle.Tensor]=None, input_text_ids: Optional[paddle.Tensor]=None,
        image_grid_thw: Optional[paddle.Tensor]=None, video_grid_thw:
        Optional[paddle.Tensor]=None, use_audio_in_video: Optional[bool]=
        None, audio_feature_lengths: Optional[paddle.Tensor]=None,
        ...
        if attention_mask is not None and position_ids is None:
            if (cache_position is None or cache_position is not None and 
                cache_position[0] == 0 or self.rope_deltas is None):
                position_ids, rope_deltas = self.get_rope_index(input_text_ids,
                    image_grid_thw, video_grid_thw, attention_mask,
                    use_audio_in_video, audio_feature_lengths,
                    video_second_per_grid)
                inputs_embeds[:, -1, :] += self.get_input_embeddings()(paddle
                    .to_tensor(data=[self.codec_bos_token], dtype='int64',
                    place=inputs_embeds.place))
                inputs_embeds[:, -2, :] += self.get_input_embeddings()(paddle
                    .to_tensor(data=[self.codec_pad_token], dtype='int64',
                    place=inputs_embeds.place))
                self.rope_deltas = rope_deltas
            else:
                batch_size, seq_length = tuple(input_ids.shape)
        ...
        outputs = self.model(attention_mask=attention_mask, position_ids=
            position_ids, past_key_values=past_key_values, inputs_embeds=
            talker_lm_input, use_cache=use_cache, output_attentions=
            output_attentions, output_hidden_states=output_hidden_states,
        ...
        if not return_dict:
            ...
            return (loss,) + output if loss is not None else output
        return Qwen2_5OmniTalkerCausalLMOutputWithPast(loss=loss, logits=
            logits, past_key_values=outputs.past_key_values, hidden_states=
            hidden_states, attentions=outputs.attentions, attention_mask=
            attention_mask, rope_deltas=self.rope_deltas,
            thinker_reply_part=thinker_reply_part)

    def _get_initial_cache_position(self, input_ids, model_kwargs):
        inputs_embeds = model_kwargs.pop('inputs_embeds')
        model_kwargs = super()._get_initial_cache_position(input_ids,
            model_kwargs)
        model_kwargs['inputs_embeds'] = inputs_embeds
        return model_kwargs

    def prepare_inputs_for_generation(self, input_ids, input_text_ids,
        past_key_values=None, attention_mask=None, inputs_embeds=None,
        thinker_reply_part=None, cache_position=None, position_ids=None,
        use_cache=True, pixel_values=None, pixel_values_videos=None,
        image_grid_thw=None, video_grid_thw=None, input_audio_features=None,
        audio_feature_attention_mask=None, audio_feature_lengths=None,
        use_audio_in_video=False, video_second_per_grid=None, **kwargs):
        ...
        model_inputs = dict(input_ids=input_ids,
            past_key_values=past_key_values, attention_mask=attention_mask, inputs_embeds=inputs_embeds, cache_position=cache_position,
            use_cache=use_cache, thinker_reply_part=thinker_reply_part,
            input_text_ids=input_text_ids, image_grid_thw=image_grid_thw,
            video_grid_thw=video_grid_thw, use_audio_in_video=
            use_audio_in_video, audio_feature_lengths=audio_feature_lengths,
            video_second_per_grid=video_second_per_grid,position_ids=position_ids,
        )
        model_inputs['position_ids'] = None
        
        # prepare cache_position
        past_length = past_key_values[0][0].shape[2] if past_key_values is not None else 0
        cache_position = paddle.arange(past_length, input_ids.shape[1] + past_length, dtype='int64')
...
class RotaryEmbedding(paddle.nn.Layer):
    ...
    def forward(self, x):
        ...
        freqs = paddle.einsum('i , j -> i j', t.astype(dtype=self.inv_freq.
            dtype), self.inv_freq)
...
class TDNNBlock(paddle.nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, dilation):
        ...
        self.conv = paddle.nn.Conv1D(in_channels=in_channels, out_channels=
            ...
            padding='same', padding_mode='zeros') # reflect -> zeros
...
class Res2NetBlock(paddle.nn.Layer):
    ...
    def __init__(self, in_channels, out_channels, scale=8, kernel_size=3,
        dilation=1):
        super().__init__()
        in_channel = in_channels // scale
        hidden_channel = out_channels // scale
        self.blocks = paddle.nn.LayerList(sublayers=[TDNNBlock(in_channel,
            hidden_channel, kernel_size=kernel_size, dilation=dilation) for
            i in range(scale - 1)])
        self.scale = scale

    def forward(self, hidden):
        outputs = []
        for i, hidden_part in enumerate(paddle.chunk(x=hidden, chunks=self.
            scale, axis=1)):
            if i == 0:
...
class SEBlock(paddle.nn.Layer):
    ...
    def __init__(self, in_channels, se_channels, out_channels):
        super().__init__()
        self.conv1 = paddle.nn.Conv1D(in_channels=in_channels, out_channels
            =se_channels, kernel_size=1, padding='same', padding_mode='zeros'
            )
        self.relu = paddle.nn.ReLU()
        self.conv2 = paddle.nn.Conv1D(in_channels=se_channels, out_channels
            =out_channels, kernel_size=1, padding='same', padding_mode=
            'zeros')
...
class AttentiveStatisticsPooling(paddle.nn.Layer):
    ...
    def __init__(self, channels, attention_channels=128):
        ...
        self.conv = paddle.nn.Conv1D(in_channels=attention_channels,
            out_channels=channels, kernel_size=1, padding='same',
            padding_mode='zeros')
    ...
    def _length_to_mask(self, length, max_len=None, dtype=None, device=None):
        ...
        mask = paddle.arange(dtype=length.dtype, end=max_len).expand(shape=
            [len(length), max_len]) < length.unsqueeze(axis=1)
        mask = paddle.to_tensor(data=mask, dtype=dtype, place=device)
        return mask

    def _compute_statistics(self, x, m, dim=2):
        mean = (m * x).sum(axis=dim)
        std = paddle.sqrt(x=(m * (x - mean.unsqueeze(axis=dim)).pow(y=2)).
            sum(axis=dim).clip(min=self.eps))
        return mean, std
    ...
    def forward(self, x):
        ...
        mask = self._length_to_mask(lengths * L, max_len=L, dtype=x.dtype,
            device=x.place)
...
class ECAPA_TDNN(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniDiTConfig):
        ...
        self.blocks.append(TDNNBlock(config.mel_dim, config.enc_channels[0],
            config.enc_kernel_sizes[0], config.enc_dilations[0]))
        for i in range(1, len(config.enc_channels) - 1):
            self.blocks.append(SERes2NetBlock(config.enc_channels[i - 1],
                config.enc_channels[i], res2net_scale=config.
                enc_res2net_scale, se_channels=config.enc_se_channels,
                kernel_size=config.enc_kernel_sizes[i], dilation=config.
                enc_dilations[i]))
        self.mfa = TDNNBlock(config.enc_channels[-1], config.enc_channels[-
            1], config.enc_kernel_sizes[-1], config.enc_dilations[-1])
        self.asp = AttentiveStatisticsPooling(config.enc_channels[-1],
            attention_channels=config.enc_attention_channels)
        self.fc = paddle.nn.Conv1D(in_channels=config.enc_channels[-1] * 2,
            out_channels=config.enc_dim, kernel_size=1, padding='same',
            padding_mode='zeros')
...
class InputEmbedding(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniDiTConfig):
        super().__init__()
        self.proj = paddle.nn.Linear(in_features=config.mel_dim + config.
            enc_dim + config.enc_emb_dim + config.emb_dim, out_features=
            config.hidden_size)
        self.spk_encoder = ECAPA_TDNN(config)

    def forward(self, x, spk, cond, code_embed, drop_audio_cond=False,
        code_embed_uncond=None, cfg=True):
        if cfg:
            ...
            code_embed = paddle.concat(x=[code_embed, code_embed_uncond],
                axis=0)
        elif drop_audio_cond:
            cond = paddle.zeros_like(x=cond)
            spk = paddle.zeros_like(x=spk)
        cond = self.spk_encoder(cond).unsqueeze(axis=1).tile(repeat_times=[
            1, x.shape[1], 1])
...
class CodecEmbedding(paddle.nn.Layer):
    ...
    def __init__(self, codec_num_embeds, codec_dim, repeats):
        ...
        self.codec_embed = paddle.nn.Embedding(num_embeddings=
            codec_num_embeds + 1, embedding_dim=codec_dim)

    def forward(self, code, drop_code=False):
        if drop_code:
            code = paddle.zeros_like(x=code)
        code_embed = self.codec_embed(code)
        code_embed = paddle.repeat_interleave(x=code_embed, repeats=self.
            repeats, axis=1)
        return code_embed
...
class AdaLayerNormZero(paddle.nn.Layer):
    ...
    def __init__(self, dim):
        ...
        self.norm = paddle.nn.LayerNorm(normalized_shape=dim, weight_attr=
            False, bias_attr=False, epsilon=1e-06)

    def forward(self, x, emb=None):
        emb = self.linear(self.silu(emb))
        shift_msa, scale_msa, gate_msa, shift_mlp, scale_mlp, gate_mlp = (
            paddle.chunk(x=emb, chunks=6, axis=1))
...
class FeedForward(paddle.nn.Layer):
    ...
    def __init__(self, dim, mult=4, dropout=0.0):
        ...
        self.ff = paddle.nn.LayerList(sublayers=[paddle.nn.Linear(
            in_features=dim, out_features=inner_dim), paddle.nn.GELU(
            approximate=True), paddle.nn.Dropout(p=dropout), paddle.nn.
            Linear(in_features=inner_dim, out_features=dim)])
...
class DiTAttention(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniDiTConfig):
        ...
        self._attn_implementation = config.get('_attn_implementation','eager')
        self.is_causal = False
        self.to_q = paddle.nn.Linear(in_features=config.hidden_size,
            out_features=self.inner_dim)
        self.to_k = paddle.nn.Linear(in_features=config.hidden_size,
            out_features=self.inner_dim)
        self.to_v = paddle.nn.Linear(in_features=config.hidden_size,
            out_features=self.inner_dim)
        self.to_out = paddle.nn.LayerList(sublayers=[paddle.nn.Linear(
            in_features=self.inner_dim, out_features=config.hidden_size),
    ...
    def forward(self, x, rope=None, mask=None) ->paddle.Tensor:
        ...
        cos, sin = rope
        query[:, :1], key[:, :1] = apply_rotary_pos_emb(query[:, :1], key[:,
            :1], cos, sin)
...
class SinusPositionEmbedding(paddle.nn.Layer):
    ...
    def forward(self, x, scale=1000):
        ...
        emb = paddle.exp(x=paddle.arange(end=half_dim).astype(dtype=
            'float32') * -emb)
...
class TimestepEmbedding(paddle.nn.Layer):
    ...
    def __init__(self, dim, freq_embed_dim=256):
        ...
        self.time_mlp = paddle.nn.LayerList(sublayers=[paddle.nn.Linear(
            in_features=freq_embed_dim, out_features=dim), paddle.nn.Silu(),
...
class DiTBlock(paddle.nn.Layer):
    ...
    def __init__(self, config: Qwen2_5OmniDiTConfig, look_ahead_block=0,
        ...
        self.ff_norm = paddle.nn.LayerNorm(normalized_shape=config.
            hidden_size, weight_attr=False, bias_attr=False, epsilon=1e-06)
        self.ff = FeedForward(dim=config.hidden_size, mult=config.ff_mult,
            dropout=config.dropout)

    def forward(self, x, t, rope=None, block_diff=None):
        norm, gate_msa, shift_mlp, scale_mlp, gate_mlp = self.attn_norm(x,
            emb=t)
        attn_output = self.attn(x=norm, rope=rope, mask=(block_diff >= -
            float(self.look_backward_block)) & (block_diff <= float(self.
            look_ahead_block)))
...
class SnakeBeta(paddle.nn.Layer):
    ...
    def __init__(self, in_features, alpha=1.0):
        ...
        self.alpha = paddle.base.framework.EagerParamBase.from_tensor(tensor
            =paddle.zeros(shape=in_features) * alpha)
    ...
    def forward(self, x):
        ...
        x = x + 1.0 / (beta + self.no_div_by_zero) * paddle.pow(x=paddle.
            sin(x=x * alpha), y=2)
        return x
...
def kaiser_sinc_filter1d(cutoff, half_width, kernel_size):
    ...
    if A > 50.0:
        ...
        beta = 0.0
        # TODO 算子缺失
    window = kaiser_window(kernel_size, beta=beta, periodic=False,
        dtype='float32')
    if even:
...
class UpSample1d(paddle.nn.Layer):
    ...
    def __init__(self, ratio=2, kernel_size=None):
        ...
        self.pad_left = self.pad * self.stride + (self.kernel_size - self.
            stride) // 2
        self.pad_right = self.pad * self.stride + (self.kernel_size - self.
            stride + 1) // 2
        filter = kaiser_sinc_filter1d(cutoff=0.5 / ratio, half_width=0.6 /
            ratio, kernel_size=self.kernel_size)
        self.register_buffer(name='filter', tensor=filter, persistable=False)

    def forward(self, x):
        _, C, _ = tuple(x.shape)
        x = paddle.nn.functional.pad(x=x, pad=(self.pad, self.pad), mode=
            'replicate', pad_from_left_axis=False)
        x = self.ratio * paddle.nn.functional.conv1d_transpose(x=x, weight=
            self.filter.expand(shape=[C, -1, -1]), stride=self.stride, groups=C
            )
...
class DownSample1d(paddle.nn.Layer):
    ...
    def forward(self, x):
        _, C, _ = tuple(x.shape)
        x = paddle.nn.functional.pad(x=x, pad=(self.pad_left, self.
            pad_right), mode='replicate', pad_from_left_axis=False)
        out = paddle.nn.functional.conv1d(x=x, weight=self.filter.expand(
            shape=[C, -1, -1]), stride=self.stride, groups=C)
        return out
...
class AMPBlock(paddle.nn.Layer):
    ...
    def __init__(self, channels, kernel_size=3, dilation=(1, 3, 5)):
        super().__init__()
        self.convs1 = paddle.nn.LayerList(sublayers=[paddle.nn.Conv1D(
            in_channels=channels, out_channels=channels, kernel_size=
            kernel_size, stride=1, dilation=dilation[0], padding=self.
            _get_padding(kernel_size, dilation[0])), paddle.nn.Conv1D(
            in_channels=channels, out_channels=channels, kernel_size=
            kernel_size, stride=1, dilation=dilation[1], padding=self.
            _get_padding(kernel_size, dilation[1])), paddle.nn.Conv1D(
            in_channels=channels, out_channels=channels, kernel_size=
            kernel_size, stride=1, dilation=dilation[2], padding=self.
            _get_padding(kernel_size, dilation[2]))])
        self.convs2 = paddle.nn.LayerList(sublayers=[paddle.nn.Conv1D(
            in_channels=channels, out_channels=channels, kernel_size=
            kernel_size, stride=1, dilation=1, padding=self._get_padding(
            kernel_size, 1)), paddle.nn.Conv1D(in_channels=channels,
            out_channels=channels, kernel_size=kernel_size, stride=1,
            dilation=1, padding=self._get_padding(kernel_size, 1)), paddle.
            nn.Conv1D(in_channels=channels, out_channels=channels,
            kernel_size=kernel_size, stride=1, dilation=1, padding=self.
            _get_padding(kernel_size, 1))])
        self.num_layers = len(self.convs1) + len(self.convs2)
        self.activations = paddle.nn.LayerList(sublayers=[TorchActivation1d
            (activation=SnakeBeta(channels)) for _ in range(self.num_layers)])
...
class Qwen2_5OmniToken2WavBigVGANModel(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniBigVGANConfig,**kwargs):
        ...
        self.conv_pre = paddle.nn.Conv1D(in_channels=config.mel_dim,
            out_channels=config.upsample_initial_channel, kernel_size=7,
            stride=1, padding=3)
        self.ups = paddle.nn.LayerList()
        for i, (u, k) in enumerate(zip(config.upsample_rates, config.
            upsample_kernel_sizes)):
            self.ups.append(paddle.nn.LayerList(sublayers=[paddle.nn.
                Conv1DTranspose(in_channels=config.upsample_initial_channel //
                2 ** i, out_channels=config.upsample_initial_channel // 2 **
    ...
    def _normalize(self, S, max_abs_value, min_db):
        return paddle.clip(x=2 * max_abs_value * ((S - min_db) / -min_db) -
            max_abs_value, min=-max_abs_value, max=max_abs_value)
...
class ODESolverRK4:
    ...
    def _rk4_alt_step_func(self, func, t0, dt, t1, y0, f0=None):
        ...
        k3 = func(t0 + dt * self._two_thirds, y0 + dt * (k2 - k1 * self.
            _one_third))
    ...
    def integrate(self, t):
        solution = paddle.empty(shape=[len(t), *tuple(self.y0.shape)],
            dtype=self.y0.dtype)
        solution[0] = self.y0
        j = 1
        y0 = self.y0
...
class Qwen2_5OmniToken2WavDiTModel(Qwen2_5OmniPreTrainedModel):
    ...
    def __init__(self, config: Qwen2_5OmniDiTConfig,**kwargs):
        ...
        self.text_embed = CodecEmbedding(config.num_embeds, config.emb_dim,
            config.repeats)
        ...
        for i in range(config.num_hidden_layers):
            self.transformer_blocks.append(DiTBlock(config,
                look_ahead_block=1 if i in config.look_ahead_layers else 0,
                look_backward_block=1 if i in config.look_backward_layers else
                0))
        self.norm_out = AdaLayerNormZero_Final(config.hidden_size)
        self.proj_out = paddle.nn.Linear(in_features=config.hidden_size,
            out_features=config.mel_dim)
    ...
    def _create_block_diff(self, x):
        ...
        return block_diff.expand(shape=[batch, self.num_attention_heads,
            seq_len, seq_len])

    def forward(self, x, cond, spk, code, time, drop_audio_cond=False,
        drop_code=False, cfg=True):
        batch = tuple(x.shape)[0]
        if time.ndim == 0:
            time = time.tile(repeat_times=batch)
        t = self.time_embed(time)
        code_embed = self.text_embed(code, drop_code=False if cfg else
            drop_code)
        code_embed_uncond = self.text_embed(code, drop_code=True
            ) if cfg else None
        hidden = self.input_embed(x, spk, cond, code_embed, drop_audio_cond
            =drop_audio_cond, code_embed_uncond=code_embed_uncond, cfg=cfg)
    ...
    @paddle.no_grad()
    def sample(self, cond, ref_mel, code, steps=10, cfg_strength=0.5,
        sway_sampling_coef=-1.0):
        y_all = paddle.randn(shape=[1, 30000, self.mel_dim], dtype=ref_mel.
            dtype)
        ...
        def fn(t, x):
            if cfg_strength < 1e-05:
                pred = self(x=x, spk=cond, cond=ref_mel, code=code, time=t,
                    drop_audio_cond=False, drop_code=False)
                return pred
            out_put = self(x=x, code=code, spk=cond, cond=ref_mel, time=t,
                cfg=True)
        ...
        if sway_sampling_coef is not None:
            t = t + sway_sampling_coef * (paddle.cos(x=np.pi / 2 * t) - 
                1 + t)
...
class Qwen2_5OmniToken2WavModel(Qwen2_5OmniPreTrainedModel):
    ...
    base_model_prefix = 'model'
    _no_split_modules = ['Qwen2_5OmniToken2WavDiTModel',
    ...
    def __init__(self, config: Qwen2_5OmniToken2WavConfig):
        ...
        self.code2wav_bigvgan_model = (Qwen2_5OmniToken2WavBigVGANModel.
            _from_config(config.bigvgan_config, attn_implementation=attn_impl))

    def forward(self, code, cond, ref_mel, steps=10, cfg_strength=0.5,
        sway_sampling_coef=-1.0, **kwargs):
        generated_mel = self.code2wav_dit_model.sample(cond, ref_mel, code,
            steps=steps, cfg_strength=cfg_strength, sway_sampling_coef=
            sway_sampling_coef)
...
class Qwen2_5OmniModel(Qwen2_5OmniPreTrainedModel):
    ...
    def enable_talker(self):
        self.talker = Qwen2_5OmniTalkerForConditionalGeneration(self.config
            .talker_config)
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args,
        config=None, cache_dir=None, ignore_mismatched_sizes=False,
        force_download=False, local_files_only=False, token=None, revision=
        'main', use_safetensors=None, weights_only=True, **kwargs):
        model = super().from_pretrained(pretrained_model_name_or_path, *
            model_args, config=config, cache_dir=cache_dir,
            ignore_mismatched_sizes=ignore_mismatched_sizes, force_download
            =force_download, local_files_only=local_files_only, token=token,
            revision=revision, use_safetensors=use_safetensors,
            weights_only=weights_only, **kwargs)
        spk_path = cached_file(pretrained_model_name_or_path, 'spk_dict.pdparams',
            subfolder=kwargs.pop('subfolder', None), cache_dir=kwargs.pop(
            'cache_dir', None))
        if spk_path is None:
            raise ValueError(
    ...
    @paddle.no_grad()
    def generate(self, input_ids: Optional[paddle.to_tensor]=None, spk: str
        ='Chelsie', use_audio_in_video: bool=False, return_audio: Optional[
        bool]=None, thinker_max_new_tokens: int=1024, talker_max_new_tokens:
        int=4096, talker_do_sample: bool=True, talker_top_k: int=40,
        talker_top_p: float=0.8, talker_temperature: float=0.9,
        talker_eos_token_id: list[int]=[8292, 8294],
        talker_repetition_penalty: float=1.05, **kwargs):
        """
        ...
        for key, value in kwargs.items():
            if key.startswith('thinker_'):
                ...
                talker_kwargs['audio_feature_lengths'] = paddle.sum(x=value,
                    axis=1)
        ...
        thinker_token_embeds = [x[0]for x in
            thinker_result.hidden_states]
        thinker_hidden_states = [x[1][-1] for x in
            thinker_result.hidden_states]
        talker_text_bos_token = speaker_params['bos_token']
        talker_input_text_ids = paddle.concat(x=[input_ids, paddle.to_tensor(data=[[talker_text_bos_token]], dtype=
            'int64'), thinker_generate_ids[:, :1]],
            axis=-1)
        talker_input_ids = paddle.concat(x=[paddle.full_like(x=input_ids,
            fill_value=self.talker.codec_mask_token), paddle.to_tensor(data
            =[[self.talker.codec_pad_token]], dtype='int64'), paddle.to_tensor(data=[[self.talker.
            codec_bos_token]], dtype='int64')], axis=1
            )
        thinker_reply_part = paddle.concat(x=thinker_hidden_states[1:], axis=1
            ) + paddle.concat(x=thinker_token_embeds[1:], axis=1)
        talker_inputs_embeds = thinker_hidden_states[0] + thinker_token_embeds[
            0]
        talker_inputs_embeds = paddle.concat(x=[talker_inputs_embeds, self.
            thinker.get_input_embeddings()(paddle.to_tensor(data=[[
            talker_text_bos_token]], dtype='int64')), thinker_reply_part[:, :1, :]],
            axis=1)
        thinker_reply_part = paddle.concat(x=[thinker_reply_part[:, 1:, :],
            self.thinker.get_input_embeddings()(paddle.to_tensor(data=[[
            self.talker.text_eos_token]], dtype='int64')), self.thinker.
            get_input_embeddings()(paddle.to_tensor(data=[[self.talker.
            text_pad_token]], dtype='int64'))], axis=1)
        talker_attention_mask = paddle.concat(x=[kwargs['attention_mask'],
            paddle.ones(shape=(1, 2), dtype=kwargs['attention_mask'].dtype)
            ], axis=1)
        # import pdb;pdb.set_trace()
        
        return thinker_result.sequences, 0 # TODO Fix Bug
        ...
        wav = self.token2wav(talker_generate_codes,
            cond=speaker_params['cond'].astype(
            dtype='float32'), ref_mel=speaker_params['ref_mel'].astype(dtype='float32'), **token2wav_kwargs)
        return thinker_result.sequences, wav.astype(dtype='float32')


__all__ = ['Qwen2_5OmniModel', 'Qwen2_5OmniThinkerTextModel',
    'Qwen2_5OmniThinkerForConditionalGeneration', 'Qwen2_5OmniTalkerModel',
    'Qwen2_5OmniTalkerForConditionalGeneration',
    'Qwen2_5OmniToken2WavDiTModel', 'Qwen2_5OmniToken2WavBigVGANModel',
    'Qwen2_5OmniToken2WavModel', 'Qwen2_5OmniPreTrainedModel',
    'Qwen2_5OmniPreTrainedModelForConditionalGeneration']
