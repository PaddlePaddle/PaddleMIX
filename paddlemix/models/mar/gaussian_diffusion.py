...
class GaussianDiffusion:
    ...
    def q_mean_variance(self, x_start, t):
        ...
        """
        mean = _extract_into_tensor(self.sqrt_alphas_cumprod, t, tuple(x_start.shape)) * x_start
        variance = _extract_into_tensor(1.0 - self.alphas_cumprod, t, tuple(x_start.shape))
        log_variance = _extract_into_tensor(self.log_one_minus_alphas_cumprod, t, tuple(x_start.shape))
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        assert tuple(noise.shape) == tuple(x_start.shape)
        return (
            _extract_into_tensor(self.sqrt_alphas_cumprod, t, tuple(x_start.shape)) * x_start
            + _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, tuple(x_start.shape)) * noise
        )
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=None, model_kwargs=None):
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_output, model_var_values = split(x=model_output, num_or_sections=C, axis=1)
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        return (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, tuple(x_t.shape)) * x_t - pred_xstart
        ) / _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, tuple(x_t.shape))
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        """
        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, tuple(x.shape))
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = paddle.randn(shape=shape).cuda(blocking=True)
        ...
        for i in indices:
            t = paddle.to_tensor(data=[i] * shape[0]).cuda(blocking=True)
            with paddle.no_grad():
                out = self.p_sample(
                    model,
                    img,
    ...
    def ddim_sample(
        ...
        alpha_bar = _extract_into_tensor(self.alphas_cumprod, t, tuple(x.shape))
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod_prev, t, tuple(x.shape))
        sigma = (
    ...
    def ddim_reverse_sample(
        ...
        if cond_fn is not None:
            out = self.condition_score(cond_fn, out, x, t, model_kwargs=model_kwargs)
        eps = (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, tuple(x.shape)) * x - out["pred_xstart"]
        ) / _extract_into_tensor(self.sqrt_recipm1_alphas_cumprod, t, tuple(x.shape))
        alpha_bar_next = _extract_into_tensor(self.alphas_cumprod_next, t, tuple(x.shape))
    ...
    def _vb_terms_bpd(self, model, x_start, x_t, t, clip_denoised=True, model_kwargs=None):
        ...
        assert tuple(decoder_nll.shape) == tuple(x_start.shape)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.model_var_type in [
                ...
                B, C = tuple(x_t.shape)[:2]
                assert tuple(model_output.shape) == (B, C * 2, *tuple(x_t.shape)[2:])
                model_output, model_var_values = split(x=model_output, num_or_sections=C, axis=1)
            ...
            assert tuple(model_output.shape) == tuple(target.shape) == tuple(x_start.shape)
    ...
    def _prior_bpd(self, x_start):
        ...
        """
        batch_size = tuple(x_start.shape)[0]
        t = paddle.to_tensor(data=[self.num_timesteps - 1] * batch_size, place=x_start.place)
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    res = paddle.to_tensor(data=arr).to(device=timesteps.place)[timesteps].astype(dtype="float32")
