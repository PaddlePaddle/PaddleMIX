...
class Blip2OptInstruct(Blip2Base):
    def __init__(
        self,
        config=Blip2Config(),
        ...
        self.prompt = prompt
        prompt_tokens = self.llm_tokenizer(self.prompt, return_tensors="pd", return_attention_mask=True)
    ...
    def generate(
        ...
        if self.qformer_text_input:
            ...
            print(self.tokenizer)
            print("-------------------------第一次输出query_tokens------------------------")
            print(query_tokens)
            print(query_tokens.shape[:])
            print("----------------------query_tokens.shape[:-1]----------------")
            print(query_tokens.shape[:-1])
            query_atts = paddle.ones(query_tokens.shape[:-1], dtype=paddle.int64)
            print("------------------------query_atts---------------------------")
            print(query_atts)
            ...
            # sh = text_Qformer.input_ids.shape
            # text_Qformer.attention_mask = paddle.ones(sh,dtype="int64")
        ...
        if image.dim() == 5:
            ...
            print("------------------------------第二次输出query_tokens------------------------------")
            print(query_tokens)
            print(query_tokens.shape[:])
            print("------------------------------第一次输出query_output------------------------------")
            print(query_output)
            print("-----------------------------输出query_output.last_hidden_state------------------")
        ...
        llm_tokens = self.llm_tokenizer(prompt, padding="longest", return_attention_mask=True, return_tensors="pd")
        print("----------------------------------------llm_tokens----------------------------------")
        print(llm_tokens)
        
        # llm_sh = llm_tokens.input_ids.shape
        # llm_tokens.attention_mask = paddle.ones(llm_sh,dtype="int64")

        with self.maybe_autocast():
            inputs_embeds = self.llm_model.get_input_embeddings()(llm_tokens.input_ids)
            print("-------------------------------第一次输出inputs_embeds----------------------------")
            print(inputs_embeds)
            print(inputs_embeds.shape)
            inputs_embeds = paddle.concat([inputs_llm, inputs_embeds], axis=1)
            print("-------------------------------第二次输出inputs_embeds----------------------------")
            print(inputs_embeds)
            print(inputs_embeds.shape)
        ...
        print(outputs)
        print(type(outputs))
        print("--------------------------------第一次输出outputs[0]---------------------------------")
        print(outputs[0])
        
        # outputs[outputs == 0] = 2  # convert output id 0 to 2 (eos_token_id)
        output_text = self.llm_tokenizer.batch_decode(outputs[0], skip_special_tokens=True)
        print("--------------------------------第一次输出output_text---------------------------------")
        print(output_text)
        output_text = [text.strip() for text in output_text]
        print("--------------------------------第二次输出output_text---------------------------------")
        print(output_text)
        return output_text
