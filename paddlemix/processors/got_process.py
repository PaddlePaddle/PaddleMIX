...
class BlipImageBaseProcessor(BaseProcessor):
    def __init__(self, mean=None, std=None):
        ...
        self.normalize = paddle.vision.transforms.Normalize(mean, std)
...
def autocontrast_func(img, cutoff=0):
    ...
    def tune_channel(ch):
        ...
        if cut == 0:
            ...
            low = 0 if tuple(low.shape)[0] == 0 else low[0]
            high = np.argwhere(np.cumsum(hist[::-1]) > cut)
            high = n_bins - 1 if tuple(high.shape)[0] == 0 else n_bins - 1 - high[0]
        if high <= low:
...
class VideoRandomAugment(object):
    ...
    def __call__(self, frames):
        assert tuple(frames.shape)[-1] == 3, "Expecting last dimension for 3-channels RGB (b, h, w, c)."
        if self.tensor_in_tensor_out:
            frames = frames.numpy().astype(np.uint8)
        num_frames = tuple(frames.shape)[0]
        ops = num_frames * [self.get_random_ops()]
        apply_or_not = num_frames * [np.random.random(size=self.N) > self.p]
        frames = paddle.stack(x=list(map(self._aug, frames, ops, apply_or_not)), axis=0).astype(dtype="float32")
        return frames
        ...
        self.transform = paddle.vision.transforms.Compose(
            [
                paddle.vision.transforms.RandomResizedCrop(
                    image_size, scale=(min_scale, max_scale), interpolation="bicubic"
                ),
