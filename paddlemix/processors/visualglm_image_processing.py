...
class VisualGLMProcessor(ProcessorMixin):
    ...
    def __init__(self, image_processor, tokenizer):
        ...
        self.special_tokens = {
            self.image_start_tag: self.tokenizer.vocab_size + 1,
            self.image_end_tag: self.tokenizer.vocab_size + 2,
        }
    ...
    def build_inputs_with_image(
        ...
        if image is not None:
            ...
            first_text_input = [self.special_tokens[prompt[:image_start_position]]]
            ...
            all_input_ids = first_text_input + image_input + second_text_input["input_ids"]
            ...
            inputs = {
                ...
                "pre_image_length": len(first_text_input),
    ...
    def build_inputs_with_image_and_text(
        self,
        record: dict = None,
        **kwargs,
    ):
        image = record["image"]
        query = record["text_input"]

        image = self.process_images(Image.open(image))
        input0 = [self.special_tokens[self.image_start_tag]]
        input1 = [self.tokenizer.pad_token_id] * self.num_query_tokens
        input2 = self.tokenizer.encode("</img>问：" + query + "\n答：", add_special_tokens=False)["input_ids"]
        a_ids = sum([input0, input1, input2], [])
        b_ids = self.tokenizer.encode(text=record["answers"], add_special_tokens=False)["input_ids"]
    ...
    def __call__(
        self,
        image: Union[Image.Image, np.ndarray, paddle.Tensor] = None,
        query: str = None,
        ...
        **kwargs,
        ...
        if mode == "train":
            inputs = self.build_inputs_with_image_and_text(record)
            return inputs
