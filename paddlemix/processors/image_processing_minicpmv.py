...
class MiniCPMVImageProcessor(BaseImageProcessor):
    ...
    def reshape_by_patch(self, image):
        ...
        patches = paddle.nn.functional.unfold(
            image.unsqueeze(0), [patch_size, patch_size], strides=[patch_size, patch_size]  # Add batch dimension
        ).squeeze(
            0
        ...
        **kwargs
        ...
        for _images in images_list:
            ...
            for image in _images:
                ...
                image_patches = [normalize(image=image, mean=self.mean, std=self.std) for image in image_patches]
                image_patches = [
                ...
                for slice_image in image_patches:
                    ...
                    tgt_sizes.append(
                        np.array(
                            (
                                tuple(slice_image.shape)[1] // self.patch_size,
                                tuple(slice_image.shape)[2] // self.patch_size,
                            )
                        )
                    )
            if tgt_sizes:
