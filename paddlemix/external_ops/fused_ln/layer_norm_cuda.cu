...
static void GetRowsCols(const std::vector<int64_t> &shape, int *p_rows,
                        int *p_cols) {
  int rows = 1;
...
std::vector<paddle::Tensor> LnFwd(const paddle::Tensor &x,
                                  ...
                                  const paddle::Tensor &bias, float epsilon) {
...
LnBwd(const paddle::Tensor &x, const paddle::Tensor &scale,
      ...
      const paddle::Tensor &invvar, const paddle::Tensor &dy, float epsilon) {
  CHECK_CUDA(dy);
  CHECK_CUDA(mean);
  CHECK_CUDA(invvar);
  CHECK_CUDA(x);
  ...
  cuda_layer_norm_gradient(x, scale, bias, mean, invvar, dy, rows, cols,
                           epsilon, &grad_x, &grad_scale, &grad_bias);
  return {grad_x, grad_scale, grad_bias};
}

std::vector<std::vector<int64_t>> LnBwdInferShape(
    std::vector<int64_t> input_shape, std::vector<int64_t> gamma_shape,
    std::vector<int64_t> beta_shape, std::vector<int64_t> mean_shape,
    std::vector<int64_t> invvar_shape, std::vector<int64_t> dout_shape,
    float epsilon) {
