...
void launch_(LaunchParams<BwdParams> &launch_params, // NOLINT
  ...
  using KernelTraits =
      KernelTraits<weight_t, input_t, output_t, compute_t, index_t, HIDDEN_SIZE,
  ...
  if (configure_params) {
    ...
    cudaError status_ = cudaOccupancyMaxActiveBlocksPerMultiprocessor(
        &ctas_per_sm, kernel, KernelTraits::THREADS_PER_CTA,
  ...
  if (KernelTraits::CTAS_PER_ROW == 1) {
    kernel<<<ctas_per_col, KernelTraits::THREADS_PER_CTA,
             KernelTraits::SMEM_BYTES, stream>>>(launch_params.params);
  } else {
    ...
    cudaLaunchCooperativeKernel((void *)kernel,    // NOLINT
                                grid, block,
                                (void **)&params_, // NOLINT
                                KernelTraits::SMEM_BYTES, stream);
  }

  using KernelTraitsF =
      layer_norm::KernelTraitsFinalize<HIDDEN_SIZE, weight_t, input_t, output_t,
                                       compute_t, index_t,
