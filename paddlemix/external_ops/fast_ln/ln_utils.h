...
template <> struct TypeToVec2<float> { using Type = float2; };

template <> struct TypeToVec2<half> { using Type = half2; };

template <> struct TypeToVec2<nv_bfloat16> { using Type = nv_bfloat162; };
...
struct Reducer : public Reducer<T, 1, WARPS_M, WARPS_N> {
  ...
  inline __device__ Reducer(Params &params, // NOLINT
                            uint32_t bidm, uint32_t bidn, uint32_t warp_m,
...
#pragma unroll
  ...
  template <typename Op> inline __device__ T reduce(T data, Op &op) { // NOLINT
