...
uint64_t get_key(paddle::DataType weight_type, paddle::DataType input_type,
                 paddle::DataType output_type, paddle::DataType compute_type,
...
layer_norm::FwdFunction &get_fwd_launcher(paddle::DataType weight_type,
  ...
  if (iter != layer_norm::FWD_FUNCS.end()) {
    ...
    PD_CHECK(false, "FWD: Unsupported hidden_size or types: ", hidden_size,
             weight_type, input_type, output_type, compute_type);
  }
}
...
layer_norm::BwdFunction &get_bwd_launcher(paddle::DataType weight_type,
  ...
  if (iter != layer_norm::BWD_FUNCS.end()) {
    ...
    PD_CHECK(false, "BWD: Unsupported hidden_size or types: ", hidden_size,
             weight_type, input_type, output_type, compute_type);
  }
}
...
std::vector<paddle::Tensor> LnFwd(const paddle::Tensor &x,
  ...
  auto launcher = get_fwd_launcher(weight_type, input_type, output_type,
                                   compute_type, hidden_size);
  ...
  if (launch_params.barrier_size > 0) {
    barrier = paddle::zeros({launch_params.barrier_size},
                            paddle::DataType::INT32, place);
    workspace = paddle::empty({launch_params.workspace_bytes},
...
LnBwd(const paddle::Tensor &x, const paddle::Tensor &scale,
      const paddle::Tensor &mean, const paddle::Tensor &invvar,
      const paddle::Tensor &dy, const float epsilon) {
  auto input_type = x.type();
  auto weight_type = scale.type();
  auto output_type = weight_type;
  auto compute_type = paddle::DataType::FLOAT32;
  ...
  auto launcher = get_bwd_launcher(weight_type, input_type, output_type,
                                   compute_type, hidden_size);
...
LnFwdInferShape(std::vector<int64_t> x_shape, std::vector<int64_t> scale_shape,
                std::vector<int64_t> bias_shape, float epsilon) {
  int64_t rows = 1;
...
std::vector<std::vector<int64_t>>
LnBwdInferShape(std::vector<int64_t> x_shape, std::vector<int64_t> scale_shape,
                std::vector<int64_t> mean_shape,
                std::vector<int64_t> invvar_shape,
                std::vector<int64_t> dy_shape, float epsilon) {
