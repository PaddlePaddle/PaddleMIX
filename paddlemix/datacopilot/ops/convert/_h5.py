...
from ...core import SCHEMA, MMDataset
from ...misc import ParallelMode, enumerate_chunk, freeze_rng_state, parallel_map
...
def open_h5(path, max_size):
    ...
    of.flush()
    ds.resize((offset[0],))
    ds_offset.resize((offset[1],))
    of.close()


def write_h5(dataset, max_size, output_dir, part_name, part, subpart=0):
    """write dataset to h5"""
    path = os.path.join(output_dir, "{}-part-{:05d}-{:04d}.h5".format(part_name, part, subpart))
...
def from_h5(
    ...
    *,
    ...
    for f in files:
        assert os.path.exists(f) and os.path.splitext(f)[-1] == ".h5", f"{f} is not valid h5 file"

    if load_all_at_once:
        data = _H5Data(files)
        items = parallel_map(
            lambda i: data[i], range(len(data)), max_workers=max_workers, mode=mode, progress=progress
        )
    else:
        items = _H5Data(files)
