...
def convert_ppdiffusers_pipeline_to_fastdeploy_pipeline(
    ...
    unet_model = UNet2DConditionModel.from_pretrained(unet_path, resnet_pre_temb_non_linearity=True)
    if lcm_lora_path is not None:
        # merge lcm_lora weights
        merge_weights(unet_model, lcm_lora_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to the `unet` checkpoint to convert (either a local directory or on the bos).",
    )
    parser.add_argument(
        "--lcm_lora_path",
        type=str,
        default=None,
        help="Path to the lcm lora path, which is in kohya format.",
    )
