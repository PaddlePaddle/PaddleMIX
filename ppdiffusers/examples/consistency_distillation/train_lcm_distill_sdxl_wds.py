...
def update_ema(target_params, source_params, rate=0.99):
    ...
    for targ, src in zip(target_params, source_params):
        targ.detach().copy_(rate * targ + (1 - rate) * src, False)


# From LatentConsistencyModel.get_guidance_scale_embedding
def guidance_scale_embedding(w, embedding_dim=512, dtype=paddle.float32):
    """
...
def main(args):
    ...
    unet.load_dict(teacher_unet.state_dict())
    unet.train()
    ...
    target_unet.load_dict(unet.state_dict())
    target_unet.train()
    ...
    def compute_embeddings(
        ...
        original_sizes = paddle.to_tensor(original_sizes, dtype=paddle.int64)
        crops_coords_top_left = paddle.to_tensor(crops_coords_top_left, dtype=paddle.int64)
        ...
        add_time_ids = paddle.concat([original_sizes, crops_coords_top_left, add_time_ids], axis=-1)
        add_time_ids = add_time_ids._to(dtype=prompt_embeds.dtype)
    ...
    uncond_prompt_embeds = paddle.zeros([args.train_batch_size, 77, 2048])
    uncond_pooled_prompt_embeds = paddle.zeros([args.train_batch_size, 1280])
                ...
                with paddle.no_grad():
                    with paddle.amp.auto_cast():
                        cond_teacher_output = teacher_unet(
                            ...
                            added_cond_kwargs={k: v._to(dtype=weight_dtype) for k, v in encoded_text.items()},
                        ).sample
                        ...
                        uncond_teacher_output = teacher_unet(
                            ...
                            added_cond_kwargs={
                                k: v._to(dtype=weight_dtype) for k, v in uncond_added_conditions.items()
                            },
                        ).sample
