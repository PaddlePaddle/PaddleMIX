...
def images_to_gif_bytes(images: List, duration: int = 1000) -> bytes:
    with BytesIO() as output_buffer:
        images[0].save(output_buffer, format="GIF", save_all=True, append_images=images[1:], duration=duration, loop=0)
...
def extract_gif_frames_from_midpoint(
    image: Union[str, PIL.Image.Image], fps: int = 8, target_duration: int = 1000
) -> list:
...
def get_crop_coordinates(old_size: tuple, new_size: tuple) -> tuple:
    ...
    """
    if not (isinstance(old_size, tuple) and isinstance(new_size, tuple) and len(old_size) == 2 and len(new_size) == 2):
