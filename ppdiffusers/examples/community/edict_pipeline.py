...
class EDICTPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        prompt_embeds = self.text_encoder(text_inputs.input_ids).last_hidden_state
        ...
        if do_classifier_free_guidance:
            ...
            negative_prompt_embeds = self.text_encoder(uncond_input.input_ids).last_hidden_state

            prompt_embeds = paddle.concat([negative_prompt_embeds, prompt_embeds])

        return prompt_embeds

    def denoise_mixing_layer(self, x: paddle.Tensor, y: paddle.Tensor):
        x = self.mixing_coeff * x + (1 - self.mixing_coeff) * y
        y = self.mixing_coeff * y + (1 - self.mixing_coeff) * x

        return [x, y]

    def noise_mixing_layer(self, x: paddle.Tensor, y: paddle.Tensor):
        y = (y - (1 - self.mixing_coeff) * x) / self.mixing_coeff
        x = (x - (1 - self.mixing_coeff) * y) / self.mixing_coeff

        return [x, y]

    def _get_alpha_and_beta(self, t: paddle.Tensor):
    ...
    @paddle.no_grad()
    def decode_latents(self, latents: paddle.Tensor):
    ...
    def prepare_latents(
        ...
        image = image.cast(dtype=text_embeds.dtype)
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps)):
            ...
            for j in range(2):
                ...
                latent_model_input = paddle.concat([model_input] * 2) if do_classifier_free_guidance else model_input
