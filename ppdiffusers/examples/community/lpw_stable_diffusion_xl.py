...
def get_weighted_text_embeddings_sdxl(
    ...
    Returns:
        ...
        neg_prompt_embeds (paddle.Tensor)
    """
    ...
    for i in range(len(prompt_token_groups)):
        ...
        token_tensor = paddle.to_tensor([prompt_token_groups[i]], dtype=paddle.int64, place=device)
        weight_tensor = paddle.to_tensor(prompt_weight_groups[i], dtype=paddle.float16, place=device)

        token_tensor_2 = paddle.to_tensor([prompt_token_groups_2[i]], dtype=paddle.int64, place=device)
        ...
        token_embedding = paddle.concat(prompt_embeds_list, axis=-1).squeeze(0)
        ...
        neg_token_tensor = paddle.to_tensor([neg_prompt_token_groups[i]], dtype=paddle.int64, place=device)
        neg_token_tensor_2 = paddle.to_tensor([neg_prompt_token_groups_2[i]], dtype=paddle.int64, place=device)
        neg_weight_tensor = paddle.to_tensor(neg_prompt_weight_groups[i], dtype=paddle.float16, place=device)
        ...
        neg_token_embedding = paddle.concat(neg_prompt_embeds_list, axis=-1).squeeze(0)
    ...
    prompt_embeds = paddle.concat(embeds, axis=1)
    negative_prompt_embeds = paddle.concat(neg_embeds, axis=1)
    ...
    negative_prompt_embeds = negative_prompt_embeds.reshape([bs_embed * num_images_per_prompt, seq_len, -1])

    pooled_prompt_embeds = pooled_prompt_embeds.tile([1, num_images_per_prompt, 1]).reshape(
        [bs_embed * num_images_per_prompt, -1]
    )
    negative_pooled_prompt_embeds = negative_pooled_prompt_embeds.tile([1, num_images_per_prompt, 1]).reshape(
        [bs_embed * num_images_per_prompt, -1]
    )
...
class SDXLLongPromptWeightingPipeline(
    ...
    def encode_image(self, image, device, num_images_per_prompt, output_hidden_states=None):
        ...
        if output_hidden_states:
            ...
            uncond_image_enc_hidden_states = self.image_encoder(
                paddle.zeros_like(image), output_hidden_states=True
            ).hidden_states[-2]
    ...
    def prepare_latents(
        ...
        if image is None:
            ...
            if not isinstance(image, (paddle.Tensor, Image.Image, list)):
                raise ValueError(
            ...
            if batch_size > init_latents.shape[0] and batch_size % init_latents.shape[0] == 0:
                ...
                init_latents = paddle.cat([init_latents], axis=0)
    ...
    def upcast_vae(self):
        ...
        # if xformers or paddle_2_0 is used attention block does not need
    ...
    def get_guidance_scale_embedding(self, w, embedding_dim=512, dtype=paddle.float32):
        ...
        if embedding_dim % 2 == 1:  # zero pad
            # TODO: check pad length
            emb = paddle.nn.functional.pad(emb, (0, 1))
