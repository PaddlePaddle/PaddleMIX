...
class CanvasRegion:
    ...
    def get_region_generator(self):
        ...
        return paddle.Generator().manual_seed(self.region_seed)
...
class Text2ImageRegion(DiffusionRegion):
    ...
    def encode_prompt(self, text_encoder):
        ...
        self.encoded_prompt = text_encoder(self.tokenized_prompt.input_ids)[0]
...
class Image2ImageRegion(DiffusionRegion):
    ...
    reference_image: paddle.Tensor = None
...
class MaskWeightsBuilder:
    ...
    def compute_mask_weights(self, region: DiffusionRegion) -> paddle.to_tensor:
    ...
    def _constant_weights(self, region: DiffusionRegion) -> paddle.to_tensor:
...
class StableDiffusionCanvasPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        init_noise = paddle.randn(shape=latents_shape, generator=generator)
        ...
        for region in reroll_regions:
            if region.reroll_mode == RerollModes.RESET.value:
                ...
                ] = paddle.randn(shape=region_shape, generator=region.get_region_generator())
        ...
        for region in all_eps_rerolls:
            if region.noise_eps > 0:
                ...
                eps_noise = (
                    paddle.randn(shape=region_noise.shape, generator=region.get_region_generator()) * region.noise_eps
                )
                init_noise[
                    :,
                    :,
        ...
        mask_weights = [mask_builder.compute_mask_weights(region) for region in text2image_regions]
        ...
        for i, t in tqdm(enumerate(self.scheduler.timesteps)):
            ...
            for region in text2image_regions:
                ...
                latent_model_input = paddle.concat(x=[region_latents] * 2)
