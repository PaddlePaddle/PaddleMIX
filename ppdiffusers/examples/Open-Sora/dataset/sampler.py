...
class VariableVideoBatchSampler(DistributedSampler):
    def __init__(
        ...
        self.seed = seed
        self.num_replicas = paddle.distributed.get_world_size()
    ...
    def __iter__(self) -> Iterator[List[int]]:
        ...
        for bucket_id, data_list in bucket_sample_dict.items():
            ...
            # handle shuffle
            if self.shuffle:
                data_indices = paddle.randperm(n=len(data_list)).tolist()
        ...
        if self.shuffle:
            bucket_id_access_order_indices = paddle.randperm(n=len(bucket_id_access_order)).tolist()
    ...
    def _print_bucket_info(self, bucket_sample_dict: dict, verbose=True) -> None:
        ...
        if paddle.distributed.get_rank() == 0 and verbose:
