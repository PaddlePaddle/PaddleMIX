...
class EMAModuleWrapper:
    def __init__(
        self,
        parameters: Iterable[paddle.nn.Parameter],
        decay: float = 0.9999,
    ...
    @paddle.no_grad()
    def step(self, parameters: Iterable[paddle.nn.Parameter], optimization_step):
        parameters = list(parameters)
        ...
        if (optimization_step + 1) % self.update_step_interval == 0:
            for ema_parameter, parameter in zip(self.ema_parameters, parameters, strict=True):
                if parameter.requires_grad:
                    if ema_parameter.place == parameter.place:
    ...
    def to(self, device: str = None, dtype: paddle.dtype = None) -> None:
        self.device = device
        self.ema_parameters = [
            p.to(place=device, dtype=dtype) if p.is_floating_point() else p.to(place=device)
            for p in self.ema_parameters
        ]

    def copy_ema_to(self, parameters: Iterable[paddle.nn.Parameter], store_temp: bool = True) -> None:
        if store_temp:
            self.temp_stored_parameters = [parameter.detach().cpu() for parameter in parameters]

        parameters = list(parameters)
        for ema_parameter, parameter in zip(self.ema_parameters, parameters, strict=True):
            # parameter.data.copy_(ema_parameter.to(device=parameter.place).data)
            ema_parameter_on_same_device = paddle.to_tensor(ema_parameter.data, place=parameter.place)
            parameter.set_value(ema_parameter_on_same_device)

    def copy_temp_to(self, parameters: Iterable[paddle.nn.Parameter]) -> None:
        for temp_parameter, parameter in zip(self.temp_stored_parameters, parameters, strict=True):
            temp_parameter_on_same_device = paddle.to_tensor(temp_parameter.data, place=parameter.place)
            parameter.set_value(temp_parameter_on_same_device)
