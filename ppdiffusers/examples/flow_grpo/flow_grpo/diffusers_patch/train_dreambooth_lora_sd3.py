...
def _encode_prompt_with_clip(
    ...
    if tokenizer is not None:
        # print('tokenizer', tokenizer.device)
        text_inputs = tokenizer(
            prompt,
            padding="max_length",
            max_length=77,
        ...
        # print('ddebug_text_inputs', type(text_inputs.input_ids))
        # text_input_ids = paddle.to_tensor(text_inputs.input_ids)
        text_input_ids = text_inputs.input_ids
    else:
        if text_input_ids is None:
            raise ValueError("text_input_ids must be provided when the tokenizer is not specified")
    # print('input_ids.place', text_input_ids.place)
    # a = paddle.rand([10])
    # print('rankb', paddle.distributed.get_rank(), text_input_ids.place, paddle.device.get_device())
    # print(paddle.distributed.get_rank(), 'device', device)
    prompt_embeds = text_encoder(text_input_ids.to(device), output_hidden_states=True)
    # prompt_embeds = text_encoder(text_input_ids, output_hidden_states=True)
    # print('text_input_ids.device', text_input_ids.place)
