...
def jpeg_incompressibility():
    def _fn(images, prompts, metadata):
        if isinstance(images, paddle.Tensor):
            images = (images * 255).round().clip(0, 255).to(paddle.uint8).cpu().numpy()
...
def aesthetic_score():
    ...
    scorer = AestheticScorer(dtype=paddle.float32).cuda()

    def _fn(images, prompts, metadata):
        if isinstance(images, paddle.Tensor):
            images = (images * 255).round().clip(0, 255).to(paddle.uint8)
        else:
            images = images.transpose(0, 3, 1, 2)  # NHWC -> NCHW
            images = paddle.tensor(images, dtype=paddle.uint8)
...
def clip_score():
    ...
    scorer = ClipScorer(dtype=paddle.float32).cuda()

    def _fn(images, prompts, metadata):
        if not isinstance(images, paddle.Tensor):
            images = images.transpose(0, 3, 1, 2)  # NHWC -> NCHW
            images = paddle.tensor(images, dtype=paddle.uint8) / 255.0
...
def image_similarity_score(device):
    ...
    def _fn(images, ref_images):
        if not isinstance(images, paddle.Tensor):
            images = images.transpose(0, 3, 1, 2)  # NHWC -> NCHW
            images = paddle.tensor(images, dtype=paddle.uint8) / 255.0
        if not isinstance(ref_images, paddle.Tensor):
            ...
            ref_images = paddle.tensor(ref_images, dtype=paddle.uint8) / 255.0
...
def pickscore_score(device):
    ...
    scorer = PickScoreScorer(dtype=paddle.float32, device=device)
...
def imagereward_score(device):
    ...
    scorer = ImageRewardScorer(dtype=paddle.float32, device=device)
...
def qwenvl_score(device):
    ...
    scorer = QwenVLScorer(dtype=paddle.bfloat16, device=device)
...
def geneval_score(device):
    ...
    def _fn(images, prompts, metadatas, only_strict):
        ...
        # metadatas_batched = []
        # n_batches = int(np.ceil(len(metadatas['tag']) / batch_size))
        # for i in range(n_batches):
        #     batch_metadata = {k: v[i * batch_size:(i + 1) * batch_size] for k, v in metadatas.items()}
        #     metadatas_batched.append(batch_metadata)
        all_scores = []
        all_rewards = []
        all_strict_rewards = []
...
def multi_score(device, score_dict):
    ...
    def _fn(images, prompts, metadata, ref_images=None, only_strict=True):
        total_scores = []
        score_details = {}
        print("ddebug_images", images.place, paddle.distributed.get_rank())
        for score_name, weight in score_dict.items():
...
def main():
    ...
    images = paddle.stack([transform(Image.open(image_path).convert("RGB")) for image_path in image_paths])
    prompts = [
