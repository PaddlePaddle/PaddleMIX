    ...
    while True:
        ...
        for checkpoint in new_checkpoints:
            ...
            generator = create_generator(
                os.path.join(checkpoint, "model.safetensors"), base_model=generator, args=args
            )
            ...
            for i, data in tqdm(
                ...
                total=len(dataset) // args.eval_batch_size,
            ):
            ...
            paddle.debice.cuda.empty_cache()

            if accelerator.is_main_process:
                fid = evaluate_model(args, accelerator.device, data_dict["all_images"], patch_fid=False)
                patch_fid = evaluate_model(args, accelerator.device, data_dict["all_images"], patch_fid=True)
                print(f"checkpoint {checkpoint} fid {fid} patch fid {patch_fid}")

                wandb.log({"fid": fid, "patch_fid": patch_fid}, step=model_index)
                ...
                if args.clip_score:
                    ...
                    # wandb.log({"clip_score": clip_score}, step=model_index)

                if args.image_reward:
                    pass
                    # image_reward = compute_image_reward(
                    #     images=data_dict["all_images"], captions=data_dict["all_captions"], device=accelerator.device
                    # )
                    # print(f"checkpoint {checkpoint} image reward {image_reward}")
                    # wandb.log({"image_reward": image_reward}, step=model_index)
