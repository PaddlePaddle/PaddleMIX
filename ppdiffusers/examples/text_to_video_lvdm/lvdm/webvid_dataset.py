...
class WebVidDataset(paddle.io.Dataset):
    ...
    def __init__(
        ...
        if self.spatial_transform == 'center_crop_resize':
            ...
            self.video_transform = paddle.vision.transforms.Resize((resolution,
                                                                    resolution))
    ...
    def _make_dataset(self):
        ...
        if self.annotation_path is not None:
            ...
            self.annotations = sum([
                glob.glob(
                    os.path.join(data_folder, '**', f'*.{ext}'), recursive=True)
    ...
    def get_data_decord(self, index):
        ...
        assert frames.shape[
            0] == self.video_length, f'{len(frames)}, self.video_length={self.video_length}'
        frames = paddle.to_tensor(data=frames.asnumpy()).astype(
            dtype='float32').transpose(perm=[0, 3, 1, 2])
        ...
        assert frames.shape[2] == self.resolution[0] and frames.shape[
            3] == self.resolution[
                1], f'frames={frames.shape}, self.resolution={self.resolution}'
...
def main():
    ...
    for id, data in enumerate(dataloader):
        endtime = time.time()
        print(id, 'time:', endtime - starttime, ' shape:', data['video'].shape,
              data['caption'])
        starttime = endtime
    return
