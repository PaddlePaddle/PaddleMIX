...
from ppdiffusers.utils import USE_PEFT_BACKEND, is_paddle_version, logging, scale_lora_layers, unscale_lora_layers,is_torch_version
import paddle
import numpy as np
...
def SortTaylor_forward(
        ...
        for index_block, block in enumerate(self.transformer_blocks):
            ...
            should_compute_block = self.count % self.step_Num == 0 or (self.result_list !=[] and self.result_list[index_block] ==1)
            if should_compute_block:
                ...
                derivative_approximation(cache_dic=cache_dic, current=current, feature=encoder_hidden_states.clone() - ori_encoder_hidden_states)
                if self.count % self.step_Num == 0:
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            joint_attention_kwargs['current']['layer'] = index_block
            should_compute_block = self.count % self.step_Num == 0 or (self.result_single_list !=[] and self.result_single_list[index_block] ==1)
            if should_compute_block:
        ...
        if self.count % self.step_Num == 1:
            ...
            for i in range(len(self.transformer_blocks)):
                cosine_similarity = paddle.nn.functional.cosine_similarity(self.previous_block_residual[i][:,:,:self.previous_block_residual[i].shape[-1]//8].to(paddle.float32), self.current_block_residual[i][:,:,:self.current_block_residual[i].shape[-1]//8].to(paddle.float32), axis=-1)
            ...
            for i in range(len(self.single_transformer_blocks)):
                cosine_similarity = paddle.nn.functional.cosine_similarity(self.previous_single_block_residual[i][:,:,:self.previous_single_block_residual[i].shape[-1]//8].to(paddle.float32), self.current_single_block_residual[i][:,:,:self.current_single_block_residual[i].shape[-1]//8].to(paddle.float32), axis=-1)
