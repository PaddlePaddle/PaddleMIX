...
def BlockDanceForward(
    ...
    for index_block, block in enumerate(self.transformer_blocks):
        should_compute_block = (
            not is_within_block_range
            or (is_within_block_range and (index_block > self.block_step or self.count % self.block_step_N == 0))
            or index_block == 0
            or self.previous_block is None
            or index_block == len(self.transformer_blocks) - 1
        )
    ...
    for index_block, block in enumerate(self.single_transformer_blocks):
        should_compute_block = (
            not is_within_block_range
            or (
                is_within_block_range and (index_block > self.block_step_single or self.count % self.block_step_N == 0)
            )
            or index_block == 0
            or self.previous_single_block is None
            or index_block == len(self.single_transformer_blocks) - 1
        )
        if should_compute_block:
            if self.training and self.gradient_checkpointing:
