...
class BasePredictor(object):
    ...
    def build_inference_model(self):
        if paddle.in_dynamic_mode():
            ...
            self.program, self.feed_names, self.fetch_targets = paddle.static.load_inference_model(
                self.weight_path, executor=self.exe, model_filename=model_file, params_filename=param_file
            )
    ...
    def base_forward(self, inputs):
        if paddle.in_dynamic_mode():
            ...
            out = self.exe.run(self.program, fetch_list=self.fetch_targets, feed=feed_dict)
