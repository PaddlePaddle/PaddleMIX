...
# from ..utils.init import kaiming_normal_, constant_


def conv(in_channels, out_channels, kernel_size, bias_attr=False, stride=1):
    return nn.Conv2D(
        in_channels, out_channels, kernel_size, padding=(kernel_size // 2), bias_attr=bias_attr, stride=stride
    )
...
class Encoder(nn.Layer):
    def __init__(self, n_feat, kernel_size, reduction, act, bias_attr, scale_unetfeats, csff):
        ...
        self.encoder_level2 = [
            CAB(n_feat + scale_unetfeats, kernel_size, reduction, bias_attr=bias_attr, act=act) for _ in range(2)
        ]
        self.encoder_level3 = [
            CAB(n_feat + (scale_unetfeats * 2), kernel_size, reduction, bias_attr=bias_attr, act=act) for _ in range(2)
        ]
        ...
        if csff:
            ...
            self.csff_enc2 = nn.Conv2D(
                n_feat + scale_unetfeats, n_feat + scale_unetfeats, kernel_size=1, bias_attr=bias_attr
            )
            self.csff_enc3 = nn.Conv2D(
                n_feat + (scale_unetfeats * 2), n_feat + (scale_unetfeats * 2), kernel_size=1, bias_attr=bias_attr
            )
...
class MPRNet(nn.Layer):
    def __init__(
        ...
        self.stage3_orsnet = ORSNet(
            n_feat, scale_orsnetfeats, kernel_size, reduction, act, bias_attr, scale_unetfeats, num_cab
        )
