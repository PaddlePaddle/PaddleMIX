...
def default_conv(in_channels, out_channels, kernel_size, bias=True):
    return nn.Conv2D(in_channels, out_channels, kernel_size, padding=(kernel_size // 2), bias_attr=bias)
...
class DownBlock(nn.Layer):
    def __init__(self, negval, n_feats, n_colors, scale, nFeat=None, in_channels=None, out_channels=None):
        ...
        dual_block = [
            nn.Sequential(
                nn.Conv2D(in_channels, nFeat, kernel_size=3, stride=2, padding=1, bias_attr=False),
        ...
        for _ in range(1, int(math.log2(scale))):
            dual_block.append(
                nn.Sequential(
                    nn.Conv2D(nFeat, nFeat, kernel_size=3, stride=2, padding=1, bias_attr=False),
                    nn.LeakyReLU(negative_slope=negval),
                )
            )

        dual_block.append(nn.Conv2D(nFeat, out_channels, kernel_size=3, stride=1, padding=1, bias_attr=False))
...
class RCAB(nn.Layer):
    def __init__(self, conv, n_feat, kernel_size, reduction=16, bias=True, bn=False, act=nn.ReLU(), res_scale=1):
...
class DRNGenerator(nn.Layer):
    ...
    def __init__(
        ...
        self.upsample = nn.Upsample(scale_factor=max(scale), mode="bicubic", align_corners=False)
        ...
        self.down = [
            DownBlock(negval, n_feats, n_colors, 2, n_feats * pow(2, p), n_feats * pow(2, p), n_feats * pow(2, p + 1))
        ...
        up_body_blocks.insert(
            0, [RCAB(conv, n_feats * pow(2, self.phase), kernel_size, act=act) for _ in range(n_blocks)]
        )
