...
class LoadTypesModel:
    ...
    def mix2img(
        ...
        **kwargs
        ...
        flag = False
        imgs = []
        with paddle.amp.auto_cast(flag):  # flag
            # 获取重新调整的生图数参数
            num_iters, num_images_per_iter_per_prompt = self.num_differ_imgs(
                height, width, num_images_per_prompt, max_limit=2048
            )
            # 逐张生成图片
            for i in range(0, num_images_per_prompt, num_images_per_iter_per_prompt):
                # 最后一次不完整
                current_num_images_per_iter_per_prompt = (
                    num_images_per_iter_per_prompt
                    if ((num_images_per_prompt - i - num_images_per_iter_per_prompt) >= 0)
                    else num_images_per_prompt % num_images_per_iter_per_prompt
                )
                ...
                img = self.pipeline(
                    ...
                    width=width,
                    num_inference_steps=num_inference_steps,
                    num_images_per_prompt=current_num_images_per_iter_per_prompt,
                    generator=generator,
                    strength=strength,
                    image=image,
