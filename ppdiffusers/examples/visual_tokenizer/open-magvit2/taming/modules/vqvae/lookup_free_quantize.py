...
def entropy_loss(
    logits, mask=None, temperature=0.01, sample_minimization_weight=1.0, batch_maximization_weight=1.0, eps=1e-05
):
    """
    ...
    log_probs = paddle.nn.functional.log_softmax(x=logits / temperature + eps, axis=-1)
    if mask is not None:
...
class LFQ(paddle.nn.Layer):
        ...
        *,
        ...
        if not self.token_factorization:
            ...
            self.register_buffer(
                name="post_mask", tensor=2 ** paddle.arange(end=factorized_bits[1]), persistable=False
            )
    ...
    def get_codebook_entry(self, x, bhwc, order):
        if self.token_factorization:
            if order == "pre":
                mask = 2 ** paddle.arange(dtype="int64", end=self.factorized_bits[0])
            else:
                mask = 2 ** paddle.arange(dtype="int64", end=self.factorized_bits[1])
        else:
            mask = 2 ** paddle.arange(dtype="int64", end=self.codebook_dim)
        x = x.unsqueeze(axis=-1) & mask != 0
        x = x * 2.0 - 1.0
    ...
    def bits_to_indices(self, bits):
        ...
        indices = 2 ** paddle.arange(start=0, end=self.codebook_dim, step=1, dtype="int64")
    ...
    def forward(self, x, inv_temperature=100.0, return_loss_breakdown=False, mask=None, return_loss=True):
        """
        einstein notation
        b - batch
        ...
        if self.token_factorization:
            indices_pre = reduce(
                (quantized[..., : self.factorized_bits[0]] > 0).astype(dtype="int32")
                * self.pre_mask.astype(dtype="int32"),
                "b n c d -> b n c",
                "sum",
            )
            indices_post = reduce(
                (quantized[..., self.factorized_bits[0] :] > 0).astype(dtype="int32")
                * self.post_mask.astype(dtype="int32"),
                "b n c d -> b n c",
                "sum",
            )
        else:
            indices = reduce(
                (quantized > 0).astype(dtype="int32") * self.mask.astype(dtype="int32"), "b n c d -> b n c", "sum"
            )
...
if __name__ == "__main__":
    quantizer = LFQ(codebook_size=2**18, dim=18, sample_minimization_weight=1.0, batch_maximization_weight=1.0)
    image_feats = paddle.randn(shape=[2, 18, 16, 16])
    quantized, indices, entropy_aux_loss = quantizer(image_feats, inv_temperature=100.0)
    assert tuple(image_feats.shape) == tuple(quantized.shape)
    assert (quantized == quantizer.indices_to_codes(indices)).astype("bool").all()
