...
def get_timestep_embedding(timesteps, embedding_dim):
    ...
    if embedding_dim % 2 == 1:
        emb = paddle.nn.functional.pad(x=emb, pad=(0, 1, 0, 0), pad_from_left_axis=False)
    return emb
...
class VUNet(paddle.nn.Layer):
    def __init__(
        ...
        **ignore_kwargs
        ...
        self.conv_in = paddle.nn.Conv2D(
            in_channels=c_channels, out_channels=self.ch, kernel_size=3, stride=1, padding=1
        )
        curr_res = resolution
        in_ch_mult = (1,) + tuple(ch_mult)
        self.down = paddle.nn.LayerList()
        for i_level in range(self.num_resolutions):
            ...
            if i_level != self.num_resolutions - 1:
                ...
                curr_res = curr_res // 2
            self.down.append(down)
        self.z_in = paddle.nn.Conv2D(in_channels=z_channels, out_channels=block_in, kernel_size=1, stride=1, padding=0)
        self.mid = paddle.nn.Layer()
        self.mid.block_1 = ResnetBlock(
