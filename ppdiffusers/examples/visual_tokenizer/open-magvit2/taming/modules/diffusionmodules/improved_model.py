...
class ResBlock(paddle.nn.Layer):
    def __init__(self, in_filters, out_filters, use_conv_shortcut=False, use_agn=False) -> None:
        super().__init__()
        ...
        self.conv1 = paddle.nn.Conv2D(
            in_channels=in_filters, out_channels=out_filters, kernel_size=(3, 3), padding=1, bias_attr=False
        )
        self.conv2 = paddle.nn.Conv2D(
            in_channels=out_filters, out_channels=out_filters, kernel_size=(3, 3), padding=1, bias_attr=False
        )
        if in_filters != out_filters:
            if self.use_conv_shortcut:
                ...
                self.nin_shortcut = paddle.nn.Conv2D(
                    in_channels=in_filters, out_channels=out_filters, kernel_size=(1, 1), padding=0, bias_attr=False
                )
...
class Encoder(paddle.nn.Layer):
    def __init__(
        self, *, ch, out_ch, in_channels, num_res_blocks, z_channels, ch_mult=(1, 2, 2, 4), resolution, double_z=False
    ):
        super().__init__()
        ...
        self.conv_in = paddle.nn.Conv2D(
            in_channels=in_channels, out_channels=ch, kernel_size=(3, 3), padding=1, bias_attr=False
        )
        ...
        for i_level in range(self.num_blocks):
            ...
            if i_level < self.num_blocks - 1:
                down.downsample = paddle.nn.Conv2D(
                    in_channels=block_out, out_channels=block_out, kernel_size=(3, 3), stride=(2, 2), padding=1
                )
        ...
        self.norm_out = paddle.nn.GroupNorm(num_groups=32, num_channels=block_out, epsilon=1e-06)
        self.conv_out = paddle.nn.Conv2D(in_channels=block_out, out_channels=z_channels, kernel_size=(1, 1))
...
class Decoder(paddle.nn.Layer):
    def __init__(
        ...
        self.conv_in = paddle.nn.Conv2D(
            in_channels=z_channels, out_channels=block_in, kernel_size=(3, 3), padding=1, bias_attr=True
        )
        self.mid_block = paddle.nn.LayerList()
        for res_idx in range(self.num_res_blocks):
            self.mid_block.append(ResBlock(block_in, block_in))
        ...
        for i_level in reversed(range(self.num_blocks)):
            ...
            if len(self.up) == 0:
                ...
                self.up.insert(0, up)
        self.norm_out = paddle.nn.GroupNorm(num_groups=32, num_channels=block_in, epsilon=1e-06)
        self.conv_out = paddle.nn.Conv2D(in_channels=block_in, out_channels=out_ch, kernel_size=(3, 3), padding=1)
...
def depth_to_space(x: paddle.Tensor, block_size: int) -> paddle.Tensor:
    ...
    s = block_size**2
    if c % s != 0:
        raise ValueError("Expecting a channels-first (*CHW) tensor with C divisible by {s}, but got C={c} channels")
    outer_dims = tuple(x.shape)[:-3]
    x = x.reshape([-1, block_size, block_size, c // s, h, w])
    x = x.transpose(perm=[0, 3, 4, 1, 5, 2])
    x = x.contiguous().reshpw([*outer_dims, c // s, h * block_size, w * block_size])
    return x
...
class Upsampler(paddle.nn.Layer):
    def __init__(self, dim, dim_out=None):
        super().__init__()
        dim_out = dim * 4
        self.conv1 = paddle.nn.Conv2D(in_channels=dim, out_channels=dim_out, kernel_size=(3, 3), padding=1)
...
class AdaptiveGroupNorm(paddle.nn.Layer):
    def __init__(self, z_channel, in_filters, num_groups=32, eps=1e-06):
        ...
        self.gn = paddle.nn.GroupNorm(
            num_groups=32, num_channels=in_filters, epsilon=eps, weight_attr=False, bias_attr=False
        )
        self.gamma = paddle.nn.Linear(in_features=z_channel, out_features=in_filters)
        self.beta = paddle.nn.Linear(in_features=z_channel, out_features=in_filters)
        self.eps = eps
    ...
    def forward(self, x, quantizer):
        ...
        scale = scale.sqrt()
        scale = self.gamma(scale).reshape([B, C, 1, 1])
        bias = rearrange(quantizer, "b c h w -> b c (h w)")
        bias = bias.mean(axis=-1)
        bias = self.beta(bias).reshape([B, C, 1, 1])
        x = self.gn(x)
        x = scale * x + bias
        return x
