...
def vanilla_d_loss(logits_real, logits_fake):
    d_loss = 0.5 * (paddle.mean(x=paddle.nn.functional.softplus(x=-
        logits_real)) + paddle.mean(x=paddle.nn.functional.softplus(x=
        logits_fake)))
    return d_loss
...
def _sigmoid_cross_entropy_with_logits(labels, logits):
    ...
    return relu_logits - logits * labels + paddle.log1p(x=paddle.exp(x=
        neg_abs_logits))
...
def non_saturate_gen_loss(logits_fake):
    ...
    gen_loss = paddle.mean(x=_sigmoid_cross_entropy_with_logits(labels=
        paddle.ones_like(x=logits_fake), logits=logits_fake))
    return gen_loss
...
def non_saturate_discriminator_loss(logits_real, logits_fake):
    ...
    real_loss = _sigmoid_cross_entropy_with_logits(labels=paddle.ones_like(
        x=logits_real), logits=logits_real)
    fake_loss = _sigmoid_cross_entropy_with_logits(labels=paddle.zeros_like
        (x=logits_fake), logits=logits_fake)
...
def lecam_reg(real_pred, fake_pred, lecam_ema):
    reg = paddle.mean(x=paddle.nn.functional.relu(x=real_pred - lecam_ema.
        logits_fake_ema).pow(y=2)) + paddle.mean(x=paddle.nn.functional.
        relu(x=lecam_ema.logits_real_ema - fake_pred).pow(y=2))
    return reg


class VQLPIPSWithDiscriminator(paddle.nn.Layer):

    def __init__(self, disc_start, codebook_weight=1.0, pixelloss_weight=
        1.0, disc_num_layers=3, disc_in_channels=3, disc_factor=1.0,
        disc_weight=1.0, commit_weight=0.25, codebook_enlarge_ratio=3,
        codebook_enlarge_steps=2000, perceptual_weight=1.0, use_actnorm=
        False, disc_conditional=False, disc_ndf=64, disc_loss='hinge',
        gen_loss_weight=None, lecam_loss_weight=None):
        super().__init__()
    ...
    def forward(self, codebook_loss, loss_break, inputs, reconstructions,
        ...
        if optimizer_idx == 0:
            if cond is None:
                ...
                logits_fake = self.discriminator(paddle.concat(x=(
                    reconstructions.contiguous(), cond), axis=1))
            ...
            if disc_factor == 0:
                log = {'{}/total_loss'.format(split): loss.clone().detach(),
                    '{}/per_sample_entropy'.format(split): loss_break.
                    per_sample_entropy.detach(), '{}/codebook_entropy'.
                    format(split): loss_break.codebook_entropy.detach(),
                    '{}/commit_loss'.format(split): loss_break.commitment.
                    detach(), '{}/nll_loss'.format(split): nll_loss.detach(
                    ), '{}/reconstruct_loss'.format(split): rec_loss.detach
                    ().mean(), '{}/perceptual_loss'.format(split): p_loss.
                    detach().mean(), '{}/d_weight'.format(split): paddle.
                    to_tensor(data=0.0), '{}/disc_factor'.format(split):
                    paddle.to_tensor(data=0.0), '{}/g_loss'.format(split):
                ...
                log = {'{}/total_loss'.format(split): loss.clone().detach(),
                    ...
                    format(split): loss_break.codebook_entropy.detach(),
                    '{}/commit_loss'.format(split): loss_break.commitment.
                    detach(), '{}/entropy_loss'.format(split):
                    codebook_loss.detach(), '{}/nll_loss'.format(split):
                    nll_loss.detach(), '{}/reconstruct_loss'.format(split):
                    rec_loss.detach().mean(), '{}/perceptual_loss'.format(
                    split): p_loss.detach().mean(), '{}/d_weight'.format(
                    split): d_weight, '{}/disc_factor'.format(split):
                    paddle.to_tensor(data=disc_factor), '{}/g_loss'.format(
                    split): g_loss.detach()}
            else:
                log = {'{}/reconstruct_loss'.format(split): rec_loss.detach
                    ().mean(), '{}/perceptual_loss'.format(split): p_loss.
                    detach().mean(), '{}/g_loss'.format(split): real_g_loss
                    .detach()}
            return loss, log
        if optimizer_idx == 1:
            if cond is None:
                ...
                logits_fake = self.discriminator(paddle.concat(x=(reconstructions.contiguous().detach(), cond), axis=1))
            ...
            if disc_factor == 0:
                log = {'{}/disc_loss'.format(split): paddle.to_tensor(data=
                    0.0), '{}/logits_real'.format(split): paddle.to_tensor(
                    data=0.0), '{}/logits_fake'.format(split): paddle.
                    to_tensor(data=0.0), '{}/disc_factor'.format(split):
                    paddle.to_tensor(data=disc_factor), '{}/lecam_loss'.
                    format(split): lecam_loss.detach(),
                    '{}/non_saturated_d_loss'.format(split):
                    non_saturate_d_loss.detach()}
            else:
                log = {'{}/disc_loss'.format(split): d_loss.clone().detach(
                    ).mean(), '{}/logits_real'.format(split): logits_real.
                    detach().mean(), '{}/logits_fake'.format(split):
                    logits_fake.detach().mean(), '{}/disc_factor'.format(
                    split): paddle.to_tensor(data=disc_factor),
