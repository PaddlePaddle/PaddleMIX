...
class ScalingLayer(paddle.nn.Layer):
    def __init__(self):
        ...
        self.register_buffer(
            name="shift", tensor=paddle.to_tensor(data=[-0.03, -0.088, -0.188], dtype="float32")[None, :, None, None]
        )
        self.register_buffer(
            name="scale", tensor=paddle.to_tensor(data=[0.458, 0.448, 0.45], dtype="float32")[None, :, None, None]
        )
...
class NetLinLayer(paddle.nn.Layer):
    ...
    def __init__(self, chn_in, chn_out=1, use_dropout=False):
        ...
        layers += [
            paddle.nn.Conv2D(
                in_channels=chn_in, out_channels=chn_out, kernel_size=1, stride=1, padding=0, bias_attr=False
            )
        ]
...
class vgg16(paddle.nn.Layer):
    def __init__(self, requires_grad=False, pretrained=True):
        ...
        self.N_slices = 5
        for x in range(4):
            self.slice1.add_sublayer(name=str(x), sublayer=vgg_pretrained_features[x])
        for x in range(4, 9):
            self.slice2.add_sublayer(name=str(x), sublayer=vgg_pretrained_features[x])
        for x in range(9, 16):
            self.slice3.add_sublayer(name=str(x), sublayer=vgg_pretrained_features[x])
        for x in range(16, 23):
            self.slice4.add_sublayer(name=str(x), sublayer=vgg_pretrained_features[x])
        for x in range(23, 30):
            self.slice5.add_sublayer(name=str(x), sublayer=vgg_pretrained_features[x])
