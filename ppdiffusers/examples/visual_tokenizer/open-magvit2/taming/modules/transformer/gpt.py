...
def top_k_top_p_filtering(
    logits, top_k: int = 0, top_p: float = 1.0, filter_value: float = -float("Inf"), min_tokens_to_keep: int = 1
):
    ...
    if top_p < 1.0:
        ...
        indices_to_remove = (
            sorted_indices_to_remove.astype("int32")
            .put_along_axis(
                axis=1, indices=sorted_indices, values=sorted_indices_to_remove.astype("int32"), broadcast=False
            )
            .astype(dtype)
        )
        logits[indices_to_remove] = filter_value
...
class LabelEmbedder(paddle.nn.Layer):
    ...
    def __init__(self, num_classes, hidden_size, dropout_prob=0.1):
        ...
        self.embedding_table = paddle.nn.Embedding(
            num_embeddings=num_classes + use_cfg_embedding, embedding_dim=hidden_size
        )
    ...
    def token_drop(self, labels, force_drop_ids=None):
        ...
        if force_drop_ids is None:
            drop_ids = paddle.rand(shape=tuple(labels.shape)[0]) < self.dropout_prob
        else:
...
class MLP(paddle.nn.Layer):
    def __init__(self, in_features, hidden_features, out_features):
        ...
        self.fc1 = paddle.nn.Linear(in_features=in_features, out_features=hidden_features, bias_attr=False)
        self.act = paddle.nn.GELU(approximate=True)
        self.fc2 = paddle.nn.Linear(in_features=hidden_features, out_features=out_features, bias_attr=False)
...
class RMSNorm(paddle.nn.Layer):
    def __init__(self, dim: int, eps: float = 1e-05):
        super().__init__()
        self.eps = eps
        self.weight = paddle.base.framework.EagerParamBase.from_tensor(tensor=paddle.ones(shape=dim))

    def _norm(self, x):
        return x * paddle.rsqrt(x=paddle.mean(x=x * x, axis=-1, keepdim=True) + self.eps)
...
class FeedForward(paddle.nn.Layer):
    def __init__(self, config):
        ...
        self.w1 = paddle.nn.Linear(in_features=config.dim, out_features=hidden_dim, bias_attr=False)
        self.w3 = paddle.nn.Linear(in_features=config.dim, out_features=hidden_dim, bias_attr=False)
        self.w2 = paddle.nn.Linear(in_features=hidden_dim, out_features=config.dim, bias_attr=False)

        self.ffn_dropout = paddle.nn.Dropout(p=config.ffn_dropout_p)

    def forward(self, x):
        return self.ffn_dropout(self.w2(paddle.nn.functional.silu(x=self.w1(x)) * self.w3(x)))
...
class Attention(paddle.nn.Layer):
    def __init__(self, config):
        ...
        self.wqkv = paddle.nn.Linear(in_features=config.dim, out_features=total_kv_dim, bias_attr=False)
        self.wo = paddle.nn.Linear(in_features=config.dim, out_features=config.dim, bias_attr=False)
        self.kv_cache = None
    ...
    def scaled_dot_product_attention(
        ...
        if is_causal:
            ...
            attn_bias.masked_fill_(mask=temp_mask.logical_not(), value=float("-inf"))
            attn_bias.to(query.dtype)
        if attn_mask is not None:
            if attn_mask.dtype == paddle.bool:
                attn_bias.masked_fill_(mask=attn_mask.logical_not(), value=float("-inf"))
            else:
                attn_bias += attn_mask

        attn_weight = query @ key.transpose(perm=paddle_aux.transpose_aux_func(key.ndim, -2, -1)) * scale_factor
        attn_weight += attn_bias
        attn_weight = paddle.nn.functional.softmax(x=attn_weight, axis=-1)

        attn_weight = paddle.nn.functional.dropout(x=attn_weight, p=dropout_p, training=True)
        return attn_weight @ value

    def forward(
        self,
        x: paddle.Tensor,
        freqs_cis: paddle.Tensor = None,
        input_pos: Optional[paddle.Tensor] = None,
        ...
        xq, xk, xv = map(lambda x: x.transpose(perm=paddle_aux.transpose_aux_func(x.ndim, 1, 2)), (xq, xk, xv))
        if self.kv_cache is not None:
            keys, values = self.kv_cache.update(input_pos, xk, xv)
        else:
            keys, values = xk, xv
        keys = keys.repeat_interleave(repeats=self.n_head // self.n_kv_head, axis=1)
        values = values.repeat_interleave(repeats=self.n_head // self.n_kv_head, axis=1)
        ...
        output = (
            output.transpose(perm=paddle_aux.transpose_aux_func(output.ndim, 1, 2))
            .contiguous()
...
def precompute_freqs_cis_2d(grid_size: int, n_elem: int, base: int = 10000, cls_token_num=120):
    half_dim = n_elem // 2
    freqs = 1.0 / base ** (
        paddle.arange(start=0, end=half_dim, step=2)[: half_dim // 2].astype(dtype="float32") / half_dim
    )
    t = paddle.arange(end=grid_size).astype(dtype="float32")
    freqs = paddle.outer(x=t, y=freqs)
    freqs_grid = paddle.concat(
        x=[freqs[:, None, :].expand(shape=[-1, grid_size, -1]), freqs[None, :, :].expand(shape=[grid_size, -1, -1])],
        axis=-1,
    )
    cache_grid = paddle.stack(x=[paddle.cos(x=freqs_grid), paddle.sin(x=freqs_grid)], axis=-1)
    cache = cache_grid.flatten(start_axis=0, stop_axis=1)
    cond_cache = paddle.concat(x=[paddle.zeros(shape=[cls_token_num, n_elem // 2, 2]), cache])
    return cond_cache
...
class Block(paddle.nn.Layer):
    ...
    def forward(
        self, x: paddle.Tensor, freqs_cis: paddle.Tensor, start_pos: int, mask: Optional[paddle.Tensor] = None
    ):
...
class KVCache(paddle.nn.Layer):
    def __init__(self, max_batch_size, max_seq_length, n_head, head_dim, dtype):
        ...
        self.register_buffer(name="k_cache", tensor=paddle.zeros(shape=cache_shape, dtype=dtype))
        self.register_buffer(name="v_cache", tensor=paddle.zeros(shape=cache_shape, dtype=dtype))
...
class GPT(paddle.nn.Layer):
    def __init__(
        ...
        if token_factorization:
            self.pre_emb = paddle.nn.Embedding(num_embeddings=2 ** factorized_bits[0], embedding_dim=self.config.dim)
            self.post_emb = paddle.nn.Embedding(num_embeddings=2 ** factorized_bits[1], embedding_dim=self.config.dim)
            self.class_emb = LabelEmbedder(self.config.class_num, self.config.dim)
        else:
            self.tok_emb = paddle.nn.Embedding(num_embeddings=self.config.vocab_size, embedding_dim=self.config.dim)
            self.class_emb = LabelEmbedder(self.config.class_num, self.config.dim)
        self.token_drop = paddle.nn.Dropout(p=self.config.token_drop)
        spatial_dpr = [
            x.item()
            for x in paddle.linspace(start=0, stop=self.config.drop_path_rate, num=self.config.spatial_n_layer)
        ]
        factorized_dpr = [
            x.item()
            for x in paddle.linspace(start=0, stop=self.config.drop_path_rate, num=self.config.factorized_n_layer)
        ]
        ...
        self.head = paddle.nn.LayerList(
            sublayers=[
                paddle.nn.Linear(
                    in_features=self.config.dim, out_features=2 ** self.config.factorized_bits[_], bias_attr=False
                )
                for _ in range(factorized_k)
            ]
        )
        grid_size = int(self.config.block_size**0.5)
        assert grid_size * grid_size == self.config.block_size
        self.freqs_cis = precompute_freqs_cis_2d(
            grid_size, self.config.dim // self.config.n_head, self.config.rope_base, self.config.cls_token_num
        )
    ...
    def setup_caches(self, max_batch_size, max_seq_length, dtype):
        ...
        causal_mask = paddle.tril(x=paddle.ones(shape=[self.max_seq_length, self.max_seq_length], dtype="bool"))
        self.causal_mask = causal_mask.unsqueeze(axis=0).tile(repeat_times=[self.max_batch_size, 1, 1])
        grid_size = int(self.config.block_size**0.5)
        assert grid_size * grid_size == self.config.block_size
        self.freqs_cis = precompute_freqs_cis_2d(
            grid_size, self.config.dim // self.config.n_head, self.config.rope_base, self.config.cls_token_num
    ...
    def forward(self, idx, input_pos=None, mask=None, targets=None):
        if self.token_factorization:
            ...
            token_embeddings = paddle.concat(x=[cls_token_embeddings, token_embeddings[:, :-1, :]], axis=1)
            h = self.token_drop(token_embeddings)
        else:
            idx, idx_cls = idx[0], idx[1]
            token_embeddings = self.tok_emb(idx)
            cls_token_embeddings = self.class_emb(idx_cls, train=self.training)[:, : self.config.cls_token_num]
            token_embeddings = paddle.concat(x=[cls_token_embeddings, token_embeddings], axis=1)
            h = self.token_drop(token_embeddings)
        B, N, D = tuple(h.shape)
        ...
        if targets is not None:
            loss = paddle.nn.functional.cross_entropy(
                input=logits.reshape([-1, logits.shape[-1]]), label=targets.reshape([-1])
            )
        return logits, loss
...
def sample(
    ...
    if cfg_scale[0] > 1.0:
        cond_token, uncond_token = paddle_aux.split(x=x, num_or_sections=bs // 2, axis=0)
    ...
    for n in range(steps):
        ...
        for i in range(k):
            ...
            if cfg_scale[i] > 1.0:
                cond_logits, uncond_logits = paddle_aux.split(x=logits, num_or_sections=bs // 2, axis=0)
