...
class Net2NetTransformer(paddle.nn.Layer):
    ...
    def forward(self, x, c):
        ...
        if self.training and self.pkeep < 1.0:
            mask = paddle.bernoulli(x=self.pkeep * paddle.ones(shape=tuple(z_indices.shape)))
            mask = mask.round().to(dtype="int64")
            r_indices = paddle.randint_like(z_indices, self.transformer.config.vocab_size)
    ...
    def encode_to_c(self, c):
        if self.downsample_cond_size > -1:
            c = paddle.nn.functional.interpolate(x=c, size=(self.downsample_cond_size, self.downsample_cond_size))
    ...
    def shared_step(self, batch, batch_idx):
        ...
        if self.token_factorization:
            ...
            loss_pre = paddle.nn.functional.cross_entropy(
                input=logits_pre.reshape(-1, logits_pre.shape[-1]), label=target_pre.reshape(-1)
            )
            loss_post = paddle.nn.functional.cross_entropy(
                input=logits_post.reshape(-1, logits_post.shape[-1]), label=target_post.reshape(-1)
            )
            loss = loss_pre + loss_post
        else:
            loss = paddle.nn.functional.cross_entropy(
                input=logits.reshape(-1, logits.shape[-1]), label=target.reshape(-1)
            )
    ...
    def configure_optimizers(self):
        ...
        param_dict = {pn: p for pn, p in param_dict.items() if not p.stop_gradient}
        ...
        optimizer = paddle.optimizer.AdamW(optim_groups, lr=self.learning_rate, betas=(0.9, 0.95), **extra_args)
        return optimizer
