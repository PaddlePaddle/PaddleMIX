...
from taming.modules.diffusionmodules.improved_model import Decoder, Encoder
from taming.modules.ema import LitEma
from taming.modules.scheduler.lr_scheduler import (
...
class VQModel(paddle.nn.Layer):
            ...
            if self.use_ema:
                ...
                for k, v in sd.items():
                    if "encoder" in k:
                        ...
                        new_params[k] = v
        missing_keys, unexpected_keys = self.set_state_dict(state_dict=new_params)
    ...
    def on_train_start(self):
        ...
        if self.resume_lr is not None:
            ...
            for opt_gen_param_group, opt_disc_param_group in zip(
                self.opt_gen.param_groups, self.opt_disc.param_groups
            ):
    ...
    def training_step(self, batch, global_step):
        ...
        aeloss, log_dict_ae = self.loss(
            eloss, loss_break, x, xrec, 0, global_step, last_layer=self.get_last_layer(), split="train"
        )
        ...
        discloss, log_dict_disc = self.loss(
            eloss, loss_break, x, xrec, 1, global_step, last_layer=self.get_last_layer(), split="train"
        )
    ...
    def validation_step(self, batch, global_step):
        ...
        if self.use_ema:
            with self.ema_scope():
                log_dict_ae_ema, log_dict_disc_ema = self._validation_step(batch, global_step, suffix="_ema")
    ...
    def _validation_step(self, batch, global_step, suffix=""):
        ...
        aeloss, log_dict_ae = self.loss(
            eloss, loss_break, x, x_rec, 0, global_step, last_layer=self.get_last_layer(), split="val" + suffix
        )
        aeloss.detach()

        discloss, log_dict_disc = self.loss(
            eloss, loss_break, x, x_rec, 1, global_step, last_layer=self.get_last_layer(), split="val" + suffix
        )
        discloss.detach()
    ...
    def configure_optimizers(self, train_dataloader_len, world_size, max_epochs):
        ...
        opt_disc = paddle.optimizer.Adam(
            parameters=self.loss.discriminator.parameters(), learning_rate=lr, beta1=0.5, beta2=0.9, weight_decay=0.0
        )
        ...
        if self.scheduler_type == "linear-warmup":
            tmp_lr = paddle.optimizer.lr.LambdaDecay(
                lr_lambda=Scheduler_LinearWarmup(warmup_steps), learning_rate=opt_gen.get_lr()
            )
            opt_gen.set_lr_scheduler(tmp_lr)
            scheduler_ae = tmp_lr

            tmp_lr = paddle.optimizer.lr.LambdaDecay(
                lr_lambda=Scheduler_LinearWarmup(warmup_steps), learning_rate=opt_disc.get_lr()
            )
            ...
            tmp_lr = paddle.optimizer.lr.LambdaDecay(
                lr_lambda=Scheduler_LinearWarmup_CosineDecay(
                    warmup_steps=warmup_steps, max_steps=training_steps, multiplier_min=multiplier_min
                ),
                learning_rate=opt_gen.get_lr(),
            )
            opt_gen.set_lr_scheduler(tmp_lr)
            scheduler_ae = tmp_lr
