...
def give_synsets_from_indices(indices, path_to_yaml=
    'data/imagenet_idx_to_synset.yaml'):
    synsets = []
    with open(path_to_yaml) as f:
        di2s = yaml.load(f)
    for idx in indices:
        synsets.append(str(di2s[idx]))
    print('Using {} different synsets for construction of Restricted Imagenet.'
        .format(len(synsets)))
    return synsets
...
class ImageNetBase(paddle.io.Dataset):
    ...
    def _filter_relpaths(self, relpaths):
        ...
        relpaths = [rpath for rpath in relpaths if not rpath.split('/')[-1] in
            ignore]
        if 'sub_indices' in self.config:
            indices = str_to_indices(self.config['sub_indices'])
            synsets = give_synsets_from_indices(indices, path_to_yaml=self.
                idx2syn)
            files = []
    ...
    def _prepare_synset_to_human(self):
        ...
        if not os.path.exists(self.human_dict) or not os.path.getsize(self.
            human_dict) == SIZE:
    ...
    def _load(self):
        ...
        class_dict = dict((synset, i) for i, synset in enumerate(
            unique_synsets))
        ...
        labels = {'relpath': np.array(self.relpaths), 'synsets': np.array(
            self.synsets), 'class_label': np.array(self.class_labels),
            'human_label': np.array(self.human_labels)}
        self.data = ImagePaths(self.abspaths, labels=labels, size=retrieve(
            self.config, 'size', default=0), random_crop=self.random_crop)
...
class ImageNetTrain(ImageNetBase):
    ...
    def _prepare(self):
        self.random_crop = retrieve(self.config,
            'ImageNetTrain/random_crop', default=True)
        ...
        if self.config['subset'] is not None:
            self.txt_filelist = os.path.join('../../data', '{}_{}.txt'.
                format(self.NAME, self.config['subset']))
        else:
...
class ImageNetValidation(ImageNetBase):
    ...
    def _prepare(self):
        self.random_crop = retrieve(self.config,
            'ImageNetValidation/random_crop', default=False)
        cachedir = os.environ.get('XDG_CACHE_HOME', os.path.expanduser(
            '../data'))
        self.root = os.path.join(cachedir, self.NAME)
        self.datadir = self.root
        ...
        if not bdu.is_prepared(self.root):
            ...
            if not os.path.exists(datadir):
                ...
                if not os.path.exists(vspath) or not os.path.getsize(vspath
                    ) == self.SIZES[1]:
...
def get_preprocessor(size=None, random_crop=False, additional_targets=None,
    ...
    if size is not None and size > 0:
        ...
        if not random_crop:
            cropper = albumentations.CenterCrop(height=crop_size, width=
                crop_size)
        else:
            cropper = albumentations.RandomCrop(height=crop_size, width=
                crop_size)
        transforms = [cropper]
        preprocessor = albumentations.Compose(transforms,
            additional_targets=additional_targets)
...
def rgba_to_depth(x):
    ...
    assert len(tuple(x.shape)) == 3 and tuple(x.shape)[2] == 4
    y = x.copy()
    y.dtype = np.float32
    y = y.reshape(tuple(x.shape)[:2])
    return np.ascontiguousarray(y)


class BaseWithDepth(paddle.io.Dataset):
    DEFAULT_DEPTH_ROOT = 'data/imagenet_depth'

    def __init__(self, config=None, size=None, random_crop=False, crop_size
        =None, root=None):
        self.config = config
        self.base_dset = self.get_base_dset()
        self.preprocessor = get_preprocessor(size=size, crop_size=crop_size,
            random_crop=random_crop, additional_targets={'depth': 'image'})
        self.crop_size = crop_size
        if self.crop_size is not None:
            self.rescaler = albumentations.Compose([albumentations.
                SmallestMaxSize(max_size=self.crop_size)],
    ...
    def preprocess_depth(self, path):
        ...
        depth = (depth - depth.min()) / max(1e-08, depth.max() - depth.min())
        depth = 2.0 * depth - 1.0
        return depth
...
class DRINExamples(paddle.io.Dataset):

    def __init__(self):
        self.preprocessor = get_preprocessor(size=256, additional_targets={
            'depth': 'image'})
        with open('data/drin_examples.txt', 'r') as f:
            relpaths = f.read().splitlines()
        self.image_paths = [os.path.join('data/drin_images', relpath) for
            relpath in relpaths]
        self.depth_paths = [os.path.join('data/drin_depth', relpath.replace
            ('.JPEG', '.png')) for relpath in relpaths]
...
class ImageNetScale(paddle.io.Dataset):
    ...
    def __init__(self, size=None, crop_size=None, random_crop=False,
        ...
        if self.crop_size is not None and self.crop_size > 0:
            if len(transforms) == 0:
                self.rescaler = albumentations.SmallestMaxSize(max_size=
                    self.crop_size)
    ...
    def __getitem__(self, i):
        ...
        if self.up_factor is None:
            ...
            lr = imscale(image, self.up_factor, keepshapes=True, keepmode=
                self.keep_mode)
