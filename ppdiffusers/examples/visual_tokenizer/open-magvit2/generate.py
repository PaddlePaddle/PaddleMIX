...
def sample_classconditional(
    ...
    c_indices = repeat(paddle.to_tensor(data=[class_label]), "1 -> b 1", b=batch_size)
    if cfg_scale[0] > 1.0:
...
def load_model(config, ckpt, gpu, eval_mode):
    if ckpt:
        ...
        global_step = None
    model = load_model_from_config(config.model, pl_sd, gpu=gpu, eval_mode=eval_mode)["model"]
