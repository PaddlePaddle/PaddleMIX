...
try:
    ...
    pass
if hasattr(paddle, 'npu'):
    DEVICE = str('npu:0' if torch_npu.npu.is_available() else 'cpu').replace(
        'cuda', 'gpu')
else:
...
def chw_to_pillow(x):
    return Image.fromarray((255 * rescale(x.detach().cpu().numpy().
        transpose(1, 2, 0))).clip(0, 255).astype(np.uint8))


@paddle.no_grad()
def sample_classconditional(model, batch_size, class_label, steps=256,
    temperature=None, top_k=None, callback=None, dim_z=18, h=16, w=16,
    verbose_time=False, top_p=None, token_factorization=False, cfg_scale=1.0):
    log = dict()
    assert type(class_label
        ) == int, f'expecting type int but type is {type(class_label)}'
    assert not model.be_unconditional, 'Expecting a class-conditional Net2NetTransformer.'
    c_indices = repeat(paddle.to_tensor(data=[class_label]), '1 -> b 1', b=
        batch_size).to(model.place)
    if cfg_scale[0] > 1.0:
        cond_null = paddle.ones_like(x=c_indices
            ) * model.transformer.config.class_num
    ...
    index_sample = sample(cond_combined, model.transformer, steps=steps,
        sample_logits=True, top_k=top_k, callback=callback, temperature=
        temperature, top_p=top_p, token_factorization=token_factorization,
        cfg_scale=cfg_scale)
    if verbose_time:
...
@paddle.no_grad()
def run_for_evaluation(logdir, model, batch_size, temperature, top_k,
    unconditional=True, num_samples=50000, given_classes=None, top_p=None,
    token_factorization=False, cfg_scale=1.0, chunk_id=0):
    ...
    for class_label in tqdm(given_classes, desc='Classes'):
        for n, bs in tqdm(enumerate(batches), desc='Sampling Class'):
            if bs == 0:
                break
            logs = sample_classconditional(model, batch_size=bs,
                class_label=class_label, temperature=temperature, top_k=
                top_k, top_p=top_p, token_factorization=token_factorization,
                cfg_scale=cfg_scale)
            batch_images = save_npz_from_logs(logs, logdir, base_count=n *
                batch_size)
            images_npz.append(batch_images)
    images_npz = np.vstack(images_npz)
    np.savez(os.path.join(logdir, 'samples_{}.npz'.format(chunk_id)),
        images_npz)
...
def get_parser():
    ...
    parser.add_argument('--ckpt', type=str, nargs='?', help=
        'load from logdir or checkpoint in logdir')
    parser.add_argument('-o', '--outdir', type=str, nargs='?', help=
        'path where the samples will be logged to.', default='')
    parser.add_argument('--config', nargs='*', metavar='config.yaml', help=
        ...
        , default=list())
    parser.add_argument('-n', '--num_samples', type=int, nargs='?', help=
        'num_samples to draw', default=50000)
    parser.add_argument('--batch_size', type=int, nargs='?', help=
        'the batch size', default=25)
    parser.add_argument('-k', '--top_k', type=str, nargs='?', help=
        'top-k value to sample with', default=250)
    parser.add_argument('-t', '--temperature', type=str, nargs='?', help=
        'temperature value to sample with', default=1.0)
    parser.add_argument('-p', '--top_p', type=str, nargs='?', help=
        'top-p value to sample with', default=1.0)
    parser.add_argument('--classes', type=str, nargs='?', help=
        ...
        , default='imagenet')
    parser.add_argument('--token_factorization', action='store_true', help=
        'whether to use token factorization')
...
def load_model(config, ckpt, gpu, eval_mode):
    if ckpt:
        ...
        global_step = None
    model = load_model_from_config(config.model, pl_sd['state_dict'], gpu=
        gpu, eval_mode=eval_mode)['model']
...
if __name__ == '__main__':
    ...
    if opt.classes == 'imagenet':
        ...
        class_chunks = given_classes[chunk_id * chunk_classes:(chunk_id + 1
            ) * chunk_classes]
        given_classes = class_chunks
    else:
        cls_str = opt.classes
        assert not cls_str.endswith(','
    ...
    run_for_evaluation(logdir, model, opt.batch_size, opt.temperature, opt.
        top_k, unconditional=model.be_unconditional, given_classes=
        given_classes, num_samples=opt.num_samples, top_p=opt.top_p,
        token_factorization=opt.token_factorization, cfg_scale=opt.
        cfg_scale, chunk_id=chunk_id)
    end_time = time.time()
