#include <vector>
#include <map>
#include "triton_adaptive_layer_norm_fp16_2048_0_0_kernel.h"
#include "paddle/extension.h"

std::map<std::vector<int>, int> map_problem_triton_adaptive_layer_norm_fp16_2048_0_0;
...
std::vector<paddle::Tensor> triton_adaptive_layer_norm_fp16_2048_0_0_func(
  ...
  auto run_triton_kernel = [&](int algo_id) -> CUresult{
      return triton_adaptive_layer_norm_fp16_2048_0_0_kernel(run_stream,
                                               x_ptr,y_ptr,weight_ptr,bias_ptr,scale_ptr,shift_ptr,M,N,seq_size,epsilon,

                                               algo_id);
  };

  if (!map_problem_triton_adaptive_layer_norm_fp16_2048_0_0.count(problem_size)) {
    std::cout << "we are tuning for triton_adaptive_layer_norm_fp16_2048_0_0 which key is: {";
    ...
    for (int algo_id = 0; algo_id < triton_adaptive_layer_norm_fp16_2048_0_0_kernel_get_num_algos(); ++algo_id) {
    ...
    map_problem_triton_adaptive_layer_norm_fp16_2048_0_0[problem_size] = select_id;
    std::cout << "select algo id: " << select_id << std::endl;
    ;
  }

  if (map_problem_triton_adaptive_layer_norm_fp16_2048_0_0.count(problem_size)) {
    int algo_id = map_problem_triton_adaptive_layer_norm_fp16_2048_0_0[problem_size];
...
std::vector<std::vector<int64_t>> triton_adaptive_layer_norm_fp16_2048_0_0_InferShape(
        const std::vector<int64_t>& A_shape) {
  return {A_shape};
}

std::vector<paddle::DataType> triton_adaptive_layer_norm_fp16_2048_0_0_InferDtype(const paddle::DataType& A_dtype) {
    return {A_dtype};
}

PD_BUILD_OP(triton_adaptive_layer_norm_fp16_2048_0_0)
    .Inputs({"x", "scale", "shift", paddle::Optional("weight"), paddle::Optional("bias")})
    .Outputs({"out"})
    .SetKernelFn(PD_KERNEL(triton_adaptive_layer_norm_fp16_2048_0_0_func))
    .Attrs({"epsilon: float"})
    .SetInferDtypeFn(PD_INFER_DTYPE(triton_adaptive_layer_norm_fp16_2048_0_0_InferDtype))
    .SetInferShapeFn(PD_INFER_SHAPE(triton_adaptive_layer_norm_fp16_2048_0_0_InferShape));
