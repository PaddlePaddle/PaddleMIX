...
- bf16 仅支持 A100, 3090, 3080 等硬件，无法使用 V100 进行训练，如果你的环境是以上设备，修改 `--bf16` 为 `True` 后即可体验更快速的训练。
...
请将下面的代码保存到 eval.py 中并运行。你可以选择直接加载训练好的模型权重完成推理，具体做法参考 5.1。如果你使用真实数据完成了模型训练并保存了 checkpoint，你可以选择加载自行训练的模型参数进行推理，具体做法参考 5.2。
