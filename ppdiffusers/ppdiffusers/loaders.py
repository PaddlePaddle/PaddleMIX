...
if is_paddlenlp_available():
    from paddlenlp.transformers import CLIPTextModel, CLIPTextModelWithProjection, PretrainedModel, PretrainedTokenizer
...
class PatchedLoraProjection(nn.Layer):
    ...
    def forward(self, input):
        return self.regular_linear_layer(
            input) + self.lora_scale * self.lora_linear_layer(input)
...
class UNet2DConditionLoadersMixin:
    ...
    def load_attn_procs(self,
        ...
        is_lora = all(("lora" in k or k.endswith(".alpha"))
                    ...
                    for k in network_alphas:
                        if k.replace(".alpha", "") in key:
                            mapped_network_alphas.update({
                                attn_processor_key: network_alphas[k]
                            })
            ...
            for key, value_dict in lora_grouped_dict.items():
                ...
                if "lora.down.weight" in value_dict:
                    ...
                    for projection_id in ["to_k", "to_q", "to_v", "to_out"]:
                        rank = value_dict[
                            f"{projection_id}_lora.down.weight"].shape[1]
                        hidden_size = value_dict[
                            f"{projection_id}_lora.up.weight"].shape[1]

                        rank_mapping.update({
                            f"{projection_id}_lora.down.weight": rank
                        })
                        hidden_size_mapping.update({
                            f"{projection_id}_lora.up.weight": hidden_size
                        })
                    ...
                    if isinstance(attn_processor, (AttnAddedKVProcessor,
                        ...
                        cross_attention_dim = value_dict[
                            "add_k_proj_lora.down.weight"].shape[1]
                        attn_processor_class = LoRAAttnAddedKVProcessor
                    else:
                        cross_attention_dim = value_dict[
                            "to_k_lora.down.weight"].shape[1]
                    ...
                    if attn_processor_class is not LoRAAttnAddedKVProcessor:
                        attn_processors[key] = attn_processor_class(
                            ...
                            hidden_size=hidden_size_mapping.get(
                                "to_k_lora.up.weight"),
                            cross_attention_dim=cross_attention_dim,
                            network_alpha=mapped_network_alphas.get(key),
                            q_rank=rank_mapping.get("to_q_lora.down.weight"),
                            q_hidden_size=hidden_size_mapping.get(
                                "to_q_lora.up.weight"),
                            v_rank=rank_mapping.get("to_v_lora.down.weight"),
                            v_hidden_size=hidden_size_mapping.get(
                                "to_v_lora.up.weight"),
                            out_rank=rank_mapping.get(
                                "to_out_lora.down.weight"),
                            out_hidden_size=hidden_size_mapping.get(
        ...
        non_attn_lora_layers = [(t, l.to(dtype=self.dtype))
                                for t, l in non_attn_lora_layers]
        # set layers
...
class TextualInversionLoaderMixin:
    ...
    def load_textual_inversion(
            self,
            pretrained_model_name_or_path: Union[str, List[str], Dict[
                str, paddle.Tensor], List[Dict[str, paddle.Tensor]]],
        ...
        for pretrained_model_name_or_path, token in zip(
            ...
            is_multi_vector = len(embedding.shape) > 1 and embedding.shape[
                0] > 1
...
class LoraLoaderMixin:
    ...
    def lora_state_dict(
        ...
        network_alphas = None
        if all((k.startswith("lora_te_") or k.startswith("lora_unet_") or
                k.startswith("lora_te1_") or k.startswith("lora_te2_"))
               for k in state_dict.keys()):
            from_diffusers = True
            # Map SDXL blocks correctly.
            if unet_config is not None:
                # use unet config to remap block numbers
                state_dict = cls._map_sgm_blocks_to_diffusers(state_dict,
                                                              unet_config)
            state_dict, network_alphas = cls._convert_kohya_lora_to_diffusers(
                state_dict)
    ...
    @classmethod
    def _map_sgm_blocks_to_diffusers(cls,
                                     state_dict,
                                     unet_config,
                                     delimiter="_",
                                     block_slice_pos=5):
        ...
        for i in input_block_ids:
            ...
            for key in input_blocks[i]:
                ...
                inner_block_key = inner_block_map[
                    inner_block_id] if "op" not in key else "downsamplers"
                inner_layers_in_block = str(
                    layer_in_block_id) if "op" not in key else "0"
        ...
        for i in middle_block_ids:
            ...
            for key in middle_blocks[i]:
                new_key = delimiter.join(
                    key.split(delimiter)[:block_slice_pos - 1] + key_part +
        ...
        for i in output_block_ids:
            ...
            for key in output_blocks[i]:
                ...
                inner_layers_in_block = str(
                    layer_in_block_id) if inner_block_id < 2 else "0"
                new_key = delimiter.join(
                    key.split(delimiter)[:block_slice_pos - 1] + [
                        str(block_id), inner_block_key, inner_layers_in_block
                    ] + key.split(delimiter)[block_slice_pos + 1:])
    ...
    def load_lora_into_unet(cls, state_dict, network_alphas, unet):
        ...
        if all(
                key.startswith(cls.unet_name) or
            ...
            if network_alphas is not None:
                alpha_keys = [
                    k for k in network_alphas.keys()
                    if k.startswith(cls.unet_name)
                ]
                network_alphas = {
                    k.replace(f"{cls.unet_name}.", ""): v
    ...
    @classmethod
    def load_lora_into_text_encoder(cls,
                                    state_dict,
                                    network_alphas,
                                    text_encoder,
                                    prefix=None,
        ...
        if any(cls.text_encoder_name in key for key in keys):
            ...
            if len(text_encoder_lora_state_dict) > 0:
                ...
                if any("to_out_lora" in k
                       for k in text_encoder_lora_state_dict.keys()):
                    ...
                    for name, _ in text_encoder_attn_modules(text_encoder):
                        text_encoder_lora_state_dict[
                            f"{name}.q_proj.lora_linear_layer.up.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_q_lora.up.weight")
                        text_encoder_lora_state_dict[
                            f"{name}.k_proj.lora_linear_layer.up.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_k_lora.up.weight")
                        text_encoder_lora_state_dict[
                            f"{name}.v_proj.lora_linear_layer.up.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_v_lora.up.weight")
                        text_encoder_lora_state_dict[
                            f"{name}.out_proj.lora_linear_layer.up.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_out_lora.up.weight")

                        text_encoder_lora_state_dict[
                            f"{name}.q_proj.lora_linear_layer.down.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_q_lora.down.weight")
                        text_encoder_lora_state_dict[
                            f"{name}.k_proj.lora_linear_layer.down.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_k_lora.down.weight")
                        text_encoder_lora_state_dict[
                            f"{name}.v_proj.lora_linear_layer.down.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_v_lora.down.weight")
                        text_encoder_lora_state_dict[
                            f"{name}.out_proj.lora_linear_layer.down.weight"] = text_encoder_lora_state_dict.pop(
                                f"{name}.to_out_lora.down.weight")

                rank = text_encoder_lora_state_dict[
                    "text_model.transformer.layers.0.self_attn.out_proj.lora_linear_layer.up.weight"].shape[
                        0]
                patch_mlp = any(".linear1." in key
                                for key in text_encoder_lora_state_dict.keys())
                ...
                # set correct dtype & device
                text_encoder_lora_state_dict = {
                    k: v._to(dtype=text_encoder.dtype)
    ...
    def _modify_text_encoder(
        ...
        for name, attn_module in text_encoder_attn_modules(text_encoder):
            ...
            attn_module.q_proj = PatchedLoraProjection(
                attn_module.q_proj,
                lora_scale,
                network_alpha=query_alpha,
                rank=rank,
                dtype=dtype)
            lora_parameters.extend(
                attn_module.q_proj.lora_linear_layer.parameters())

            attn_module.k_proj = PatchedLoraProjection(
                attn_module.k_proj,
                lora_scale,
                network_alpha=key_alpha,
                rank=rank,
                dtype=dtype)
            lora_parameters.extend(
                attn_module.k_proj.lora_linear_layer.parameters())

            attn_module.v_proj = PatchedLoraProjection(
                attn_module.v_proj,
                lora_scale,
                network_alpha=value_alpha,
                rank=rank,
                dtype=dtype)
            lora_parameters.extend(
                attn_module.v_proj.lora_linear_layer.parameters())

            attn_module.out_proj = PatchedLoraProjection(
                attn_module.out_proj,
                lora_scale,
                network_alpha=proj_alpha,
                rank=rank,
                dtype=dtype)
            lora_parameters.extend(
                attn_module.out_proj.lora_linear_layer.parameters())
        ...
        if patch_mlp:
            for name, mlp_module in text_encoder_mlp_modules(text_encoder):
                ...
                mlp_module.linear1 = PatchedLoraProjection(
                    mlp_module.linear1,
                    lora_scale,
                    network_alpha=fc1_alpha,
                    rank=rank,
                    dtype=dtype)
                lora_parameters.extend(
                    mlp_module.linear1.lora_linear_layer.parameters())

                mlp_module.linear2 = PatchedLoraProjection(
                    mlp_module.linear2,
                    lora_scale,
                    network_alpha=fc2_alpha,
                    rank=rank,
                    dtype=dtype)
                lora_parameters.extend(
                    mlp_module.linear2.lora_linear_layer.parameters())
    ...
    def save_lora_weights(
        ...
        if unet_lora_layers is not None:
            weights = (unet_lora_layers.state_dict()
                       if isinstance(unet_lora_layers, nn.Layer) else
                       unet_lora_layers)
        ...
        if text_encoder_lora_layers is not None:
            weights = (text_encoder_lora_layers.state_dict()
                       if isinstance(text_encoder_lora_layers, nn.Layer) else
    ...
    def _convert_kohya_lora_to_diffusers(cls, state_dict):
        ...
        lora_keys = [
            k for k in state_dict.keys() if k.endswith("lora_down.weight")
        ]
        for key in lora_keys:
            ...
            if lora_name.startswith("lora_unet_"):
                ...
                if "emb" in diffusers_name:
                    pattern = r"\.\d+(?=\D*$)"
                    diffusers_name = re.sub(pattern,
                                            "",
                                            diffusers_name,
                                            count=1)
                if ".in." in diffusers_name:
                    diffusers_name = diffusers_name.replace("in.layers.2",
                                                            "conv1")
                if ".out." in diffusers_name:
                    diffusers_name = diffusers_name.replace("out.layers.3",
                                                            "conv2")
                ...
                if "transformer_blocks" in diffusers_name:
                    if "attn1" in diffusers_name or "attn2" in diffusers_name:
                        ...
                        unet_state_dict[diffusers_name.replace(
                            ".down.", ".up.")] = state_dict.pop(lora_name_up)
                    elif "ff" in diffusers_name:
                        unet_state_dict[diffusers_name] = state_dict.pop(key)
                        unet_state_dict[diffusers_name.replace(
                            ".down.", ".up.")] = state_dict.pop(lora_name_up)
                elif any(
                        key in diffusers_name
                        for key in ("proj_in", "proj_out")):
                    unet_state_dict[diffusers_name] = state_dict.pop(key)
                    unet_state_dict[diffusers_name.replace(
                        ".down.", ".up.")] = state_dict.pop(lora_name_up)
                else:
        ...
        unet_state_dict = {
            f"{cls.unet_name}.{module_name}": params
        ...
        } if len(te2_state_dict) > 0 else None)
    ...
    def unload_lora_weights(self):
        ...
        if unet_attention_classes.issubset(LORA_ATTENTION_PROCESSORS):
            ...
            # attention.
            if len(unet_attention_classes
                   ) > 1 or LoRAAttnAddedKVProcessor in unet_attention_classes:
                ...
                self.unet.set_attn_processor(regular_attention_classes[
                    attention_proc_class]())
...
class FromSingleFileMixin:
    ...
    def from_ckpt(cls, *args, **kwargs):
        ...
        deprecate(
            "from_ckpt", "0.21.0", deprecation_message, standard_warn=False)
...
class FromOriginalVAEMixin:
    ...
    def from_single_file(cls, pretrained_model_link_or_path, **kwargs):
        ...
        if scaling_factor is None:
            if ("model" in original_config and
                    "params" in original_config.model and
                    "scale_factor" in original_config.model.params):
