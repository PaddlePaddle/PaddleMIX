...
class BatchedBrownianTree:
    ...
    def __init__(self, x, t0, t1, seed=None, **kwargs):
        ...
        try:
            ...
            self.batched = False
        self.trees = [
            paddlesde.BrownianTree(t0, w0, t1, entropy=s, **kwargs)
            for s in seed
        ]
    ...
    def __call__(self, t0, t1):
        ...
        w = paddle.stack([tree(t0, t1) for tree in self.trees]) * (
                self.sign * sign)
...
class GaussianDiffusion_SDEdit(object):
        ...
        if guide_scale is None:
            ...
            if guide_scale == 1.:
                ...
                if guide_rescale is not None:
                    ...
                    ratio = (
                            paddle.std(y_out.flatten(1), axis=1) /  # noqa
                            (paddle.std(out.flatten(1), axis=1) + 1e-12)
    ...
    def sample(self,
        ...
        steps = paddle.to_tensor(
            steps, dtype=paddle.float32, place=noise.place)
        ...
        if schedule == 'karras':
            if sigmas[0] == float('inf'):
                ...
                sigmas = paddle.concat([
                    sigmas.to_tensor([float('inf')]), sigmas,
                    paddle.zeros([1]).astype(sigmas.dtype)
                ])
            else:
                sigmas = karras_schedule(
                    n=len(steps),
    ...
    def _sigma_to_t(self, sigma):
        if sigma == float('inf'):
            ...
            log_sigmas = paddle.sqrt(self.sigmas ** 2 /  # noqa
                                     (1 - self.sigmas ** 2)).log().astype(sigma.dtype)
            log_sigma = sigma.log()
    ...
    def _t_to_sigma(self, t):
        ...
        log_sigmas = paddle.sqrt(self.sigmas ** 2 /  # noqa
                                 (1 - self.sigmas ** 2)).log().astype(t.dtype)
        log_sigma = (1 - w) * log_sigmas[low_idx] + w * log_sigmas[high_idx]
        log_sigma[paddle.isnan(log_sigma)
                  | paddle.isinf(log_sigma)] = float('inf')
