...
class T2IAdapter(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if adapter_type == "full_adapter":
            self.adapter = FullAdapter(in_channels, channels, num_res_blocks,
                                       downscale_factor)
        elif adapter_type == "light_adapter":
            self.adapter = LightAdapter(in_channels, channels, num_res_blocks,
                                        downscale_factor)
        else:
            raise ValueError(
...
class FullAdapter(nn.Layer):
    def __init__(
        ...
        self.body = nn.LayerList([
            ...
            * [
                AdapterBlock(
                    channels[i - 1], channels[i], num_res_blocks, down=True)
...
class LightAdapter(nn.Layer):
    def __init__(
        ...
        self.body = nn.LayerList([
            ...
            * [
                LightAdapterBlock(
                    channels[i], channels[i + 1], num_res_blocks, down=True)
                for i in range(len(channels) - 1)
            ],
            LightAdapterBlock(
                channels[-1], channels[-1], num_res_blocks, down=True),
        ])
