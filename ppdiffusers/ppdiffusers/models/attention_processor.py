...
class Attention(nn.Layer):
    ...
    def __init__(
        ...
        if norm_num_groups is not None:
            self.group_norm = nn.GroupNorm(
                num_channels=query_dim, num_groups=norm_num_groups, epsilon=eps)
        else:
        ...
        if processor is None:
            processor = (AttnProcessor2_5()
                         if is_ppxformers_available() else AttnProcessor())
    ...
    def set_use_memory_efficient_attention_xformers(
        ...
        is_custom_diffusion = hasattr(self, "processor") and isinstance(
            self.processor, (CustomDiffusionAttnProcessor,
        ...
        if use_memory_efficient_attention_xformers:
            ...
            if is_lora:
                attn_processor_class = (LoRAAttnProcessor2_5
                                        if is_ppxformers_available() else
                                        LoRAAttnProcessor)
    ...
    def set_processor(self, processor: "AttnProcessor"):
        ...
        if hasattr(self, "processor") and isinstance(
                self.processor,
                nn.Layer) and not isinstance(processor, nn.Layer):
            logger.info(
    ...
    def __call__(self,
        ...
        if input_ndim == 4:
            ...
            hidden_states = hidden_states.reshape(
                [batch_size, channel, height * width]).transpose([0, 2, 1])
        ...
        if attn.group_norm is not None:
            hidden_states = attn.group_norm(
                hidden_states.transpose([0, 2, 1])).transpose([0, 2, 1])
...
class LoRAAttnProcessor(nn.Layer):
    ...
    def __init__(self,
        ...
        self.to_q_lora = LoRALinearLayer(q_hidden_size, q_hidden_size, q_rank,
                                         network_alpha)
        self.to_k_lora = LoRALinearLayer(cross_attention_dim or hidden_size,
                                         hidden_size, rank, network_alpha)
        self.to_v_lora = LoRALinearLayer(cross_attention_dim or v_hidden_size,
                                         v_hidden_size, v_rank, network_alpha)
        self.to_out_lora = LoRALinearLayer(out_hidden_size, out_hidden_size,
                                           out_rank, network_alpha)
...
class CustomDiffusionAttnProcessor(nn.Layer):
    ...
    def __call__(self,
        ...
        attention_mask = attn.prepare_attention_mask(
            attention_mask, sequence_length, batch_size)
        if self.train_q_out:
            query = self.to_q_custom_diffusion(hidden_states).cast(
...
class LoRAAttnAddedKVProcessor(nn.Layer):
    ...
    def __init__(self,
        ...
        self.to_q_lora = LoRALinearLayer(hidden_size, hidden_size, rank,
                                         network_alpha)
        self.add_k_proj_lora = LoRALinearLayer(cross_attention_dim or
                                               hidden_size, hidden_size, rank,
                                               network_alpha)
        self.add_v_proj_lora = LoRALinearLayer(cross_attention_dim or
                                               hidden_size, hidden_size, rank,
                                               network_alpha)
    ...
    def __call__(self,
        ...
        encoder_hidden_states_key_proj = attn.add_k_proj(
            encoder_hidden_states) + scale * self.add_k_proj_lora(
                encoder_hidden_states)
        encoder_hidden_states_value_proj = attn.add_v_proj(
            encoder_hidden_states) + scale * self.add_v_proj_lora(
                encoder_hidden_states)
        encoder_hidden_states_key_proj = attn.head_to_batch_dim(
            encoder_hidden_states_key_proj)
        encoder_hidden_states_value_proj = attn.head_to_batch_dim(
        ...
        if not attn.only_cross_attention:
            key = attn.to_k(hidden_states) + scale * self.to_k_lora(
                hidden_states)
            value = attn.to_v(hidden_states) + scale * self.to_v_lora(
                hidden_states)
            key = attn.head_to_batch_dim(key)
            value = attn.head_to_batch_dim(value)
...
class XFormersAttnProcessor:
    ...
    def __call__(self,
        ...
        attention_mask = attn.prepare_attention_mask(
            attention_mask, key_tokens, batch_size, transpose=False)
        if attention_mask is not None:
...
AttentionProcessor = Union[
    ...
    LoRAAttnProcessor2_5, LoRAAttnAddedKVProcessor,
