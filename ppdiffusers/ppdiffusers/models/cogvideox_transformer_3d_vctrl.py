...
class CogVideoXTransformer3DVCtrlModel(ModelMixin, ConfigMixin, CacheMixin):
    ...
    def __init__(
        ...
        self.proj_out = paddle.nn.Linear(in_features=inner_dim, out_features=patch_size * patch_size * out_channels)
    ...
    def forward(
            ...
            if block_vctrl_residuals is not None:
                if vctrl_layout_type == "even":
                    ...
                    hidden_states = hidden_states + paddle.nn.functional.adaptive_avg_pool1d(
                        block_vctrl_residuals[i // interval_control], hidden_states.shape[-1]
                    )
        ...
        output = (
            output.transpose(perm=[0, 1, 4, 2, 5, 3, 6])
            .flatten(start_axis=5, stop_axis=6)
            .flatten(start_axis=3, stop_axis=4)
        )
