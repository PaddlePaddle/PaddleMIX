...
class UViTModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        pos_embed = paddle.concat(
            [
                self.pos_embed[:, :1 + 1, :], self.pos_embed_token,
                self.pos_embed[:, 1 + 1:, :]
            ],
            axis=1)
...
class UViTModel_T2I(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            ...
            embed_dim=embed_dim,
            add_pos_embed=False, )
        num_patches = (self.img_size[0] // patch_size) * (self.img_size[1] //
                                                          patch_size)
        ...
        self.final_layer = nn.Conv2D(
            ...
            padding=1) if conv else nn.Identity()
    ...
    def forward(
        ...
        time_token = get_timestep_embedding(timesteps, self.embed_dim, True,
                                            0).unsqueeze(axis=1)
