...
class FluxTransformer2DModel(
    ...
    def jit_single_transformer_blocks(self, hidden_states, temb, image_rotary_emb, controlnet_single_block_samples):
        ...
        for index_block, block in enumerate(self.single_transformer_blocks):
            ...
            if controlnet_single_block_samples is not None:
                ...
                interval_control = int(np.ceil(interval_control))
                # TODO: fix hard code 512
                hidden_states[:, 512:, ...] = (
                    hidden_states[:, 512:, ...] + controlnet_single_block_samples[index_block // interval_control]
                )
        return hidden_states
    ...
    def jit_forward(
        ...
        list_image_rotary_emb = list(image_rotary_emb)
        encoder_hidden_states, hidden_states = self.jit_transformer_blocks(
            hidden_states,
            encoder_hidden_states,
            temb,
            list_image_rotary_emb,
            controlnet_block_samples,
            controlnet_blocks_repeat,
        )
        hidden_states = paddle.concat([encoder_hidden_states, hidden_states], axis=1)

        hidden_states = self.jit_single_transformer_blocks(
            hidden_states, temb, list_image_rotary_emb, controlnet_single_block_samples
        )
