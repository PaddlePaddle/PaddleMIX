...
class WanRMS_norm(nn.Layer):
    ...
    def forward(self, x):
        return F.normalize(x, axis=(1 if self.channel_first else -1)) * self.scale * self.gamma + self.bias
...
class WanUpsample(nn.Upsample):
    ...
    def forward(self, x):
        return super().forward(x.cast(paddle.float32)).cast(x.dtype)
            ...
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    if (
                        ...
                        and (isinstance(feat_cache[idx], paddle.Tensor) or feat_cache[idx] != "Rep")
                    ):
        ...
        if self.mode == "downsample3d":
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    cache_x = x[:, :, -1:, :, :].clone()
                    x = self.time_conv(paddle.concat([feat_cache[idx][:, :, -1:, :, :], x], 2))
                    feat_cache[idx] = cache_x
                    feat_idx[0] += 1
        return x


class WanResidualBlock(nn.Layer):
    r"""
...
class WanUpBlock(nn.Layer):
    ...
    def __init__(
        ...
        if upsample_mode is not None:
            self.upsamplers = nn.LayerList([WanResample(out_dim, mode=upsample_mode)])
...
class AutoencoderKLWan(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.mean = paddle.to_tensor(latents_mean)
        self.std = paddle.to_tensor(latents_std)
        self.scale = paddle.stack([self.mean, 1.0 / self.std])  # Shape: [2, C]
    ...
    def _encode(self, x: paddle.Tensor) -> paddle.Tensor:
        ...
        for i in range(iter_):
            ...
            if i == 0:
                ...
                out = paddle.concat([out, out_], 2)

        enc = self.quant_conv(out)
        mu, logvar = enc[:, : self.z_dim, :, :, :], enc[:, self.z_dim :, :, :, :]
        enc = paddle.concat([mu, logvar], axis=1)
        self.clear_cache()
        return enc
    ...
    def _decode(self, z: paddle.Tensor, return_dict: bool = True) -> Union[DecoderOutput, paddle.Tensor]:
        ...
        out = paddle.clip(out, min=-1.0, max=1.0)
        self.clear_cache()
        if not return_dict:
            return (out,)
