...
class TransformerSpatioTemporalModel(nn.Layer):
    """
    A Transformer model for video-like data.
    ...
    def __init__(
        ...
        self.temporal_transformer_blocks = nn.LayerList(
            [
    ...
    def forward(
        ...
        Args:
            ...
            encoder_hidden_states ( `paddle.Tensor` of shape `(batch size, encoder_hidden_states dim)`, *optional*):
                Conditional embeddings for cross attention layer. If not given, cross-attention defaults to
                self-attention.
            image_only_indicator (`paddle.Tensor` of shape `(batch size, num_frames)`, *optional*):
        ...
        hidden_states = hidden_states.transpose([0, 2, 3, 1]).reshape([batch_frames, height * width, inner_dim])
        hidden_states = self.proj_in(hidden_states)

        num_frames_emb = paddle.arange(num_frames)
        num_frames_emb = num_frames_emb.tile([batch_size, 1])
        num_frames_emb = num_frames_emb.reshape(
            [
                -1,
            ]
        )
        ...
        t_emb = t_emb.cast(dtype=hidden_states.dtype)
        ...
        # 3. Output
        hidden_states = self.proj_out(hidden_states)
        hidden_states = hidden_states.reshape([batch_frames, height, width, inner_dim]).transpose([0, 3, 1, 2])
