...
class UNetSpatioTemporalConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        # out
        self.conv_norm_out = nn.GroupNorm(num_channels=block_out_channels[0], num_groups=32, epsilon=1e-5)
    ...
    def forward(
        ...
        time_embeds = time_embeds.cast(emb.dtype)
        ...
        emb = emb.repeat_interleave(num_frames, axis=0)
        # encoder_hidden_states: [batch, 1, channels] -> [batch * frames, 1, channels]
        encoder_hidden_states = encoder_hidden_states.repeat_interleave(num_frames, axis=0)

        # 2. pre-process
        sample = self.conv_in(sample)

        image_only_indicator = paddle.zeros([batch_size, num_frames], dtype=sample.dtype)
