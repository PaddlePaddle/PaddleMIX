...
class QKVAttention(paddle.nn.Layer):
    ...
    def forward(self, qkv, rp=None, mask=None):
        ...
        weight = paddle.einsum('bct,bcs->bts', (q * scale).reshape(
            [bs * self.n_heads, ch,
             length]), (k * scale).reshape([bs * self.n_heads, ch, length]))
        if rp is not None:
            k_rp, v_rp = rp
            weight2 = paddle.einsum('bct,tsc->bst', (q * scale).reshape(
                [bs * self.n_heads, ch, length]), k_rp)
            weight += weight2
        if mask is not None:
            INF = -100000000.0
            weight = paddle.where(
