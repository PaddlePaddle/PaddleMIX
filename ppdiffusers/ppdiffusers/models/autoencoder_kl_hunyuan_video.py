...
def prepare_causal_attention_mask(num_frames: int, height_width: int, dtype: paddle.dtype, device: (paddle.CPUPlace, paddle.CUDAPlace, str), batch_size: int = None
    ...
    if batch_size is not None:
        mask = mask.unsqueeze(axis=0).expand(shape=[batch_size, -1, -1])
    return mask


class HunyuanVideoCausalConv3d(paddle.nn.Layer):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class HunyuanVideoUpsampleCausal3D(paddle.nn.Layer):
    def __init__(
        self,
        in_channels: int,
    ...
    def forward(self, hidden_states: paddle.Tensor) -> paddle.Tensor:
        ...
        first_frame = paddle.nn.functional.interpolate(
            x=first_frame.squeeze(axis=2),
            scale_factor=self.upsample_factor[1:],
            mode="nearest",
        ).unsqueeze(axis=2)
        if num_frames > 1:
            other_frames = other_frames.contiguous()
            other_frames = paddle.nn.functional.interpolate(
                x=other_frames, scale_factor=self.upsample_factor, mode="nearest"
            )
            hidden_states = paddle.concat(x=(first_frame, other_frames), axis=2)
        else:
...
class HunyuanVideoDownsampleCausal3D(paddle.nn.Layer):
    def __init__(
        self,
        channels: int,
        out_channels: Optional[int] = None,
...
class HunyuanVideoResnetBlockCausal3D(paddle.nn.Layer):
    def __init__(
        self,
        in_channels: int,
        out_channels: Optional[int] = None,
...
class HunyuanVideoMidBlock3D(paddle.nn.Layer):
    def __init__(
        self,
        in_channels: int,
        dropout: float = 0.0,
...
class HunyuanVideoDownBlock3D(paddle.nn.Layer):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class HunyuanVideoUpBlock3D(paddle.nn.Layer):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
...
class HunyuanVideoEncoder3D(paddle.nn.Layer):
    r"""
...
class HunyuanVideoDecoder3D(paddle.nn.Layer):
    r"""
...
class AutoencoderKLHunyuanVideo(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.quant_conv = paddle.nn.Conv3D(
            in_channels=2 * latent_channels,
            out_channels=2 * latent_channels,
            kernel_size=1,
        )
        self.post_quant_conv = paddle.nn.Conv3D(
    ...
    def tiled_encode(self, x: paddle.Tensor) -> AutoencoderKLOutput:
        ...
        enc = paddle.concat(x=result_rows, axis=3)[:, :, :, :latent_height, :latent_width]
        return enc
    ...
    def tiled_decode(self, z: paddle.Tensor, return_dict: bool = True) -> Union[DecoderOutput, paddle.Tensor]:
        ...
        for i, row in enumerate(rows):
            ...
            result_rows.append(paddle.concat(x=result_row, axis=-1))

        dec = paddle.concat(x=result_rows, axis=3)[:, :, :, :sample_height, :sample_width]

        if not return_dict:
            return (dec,)
        return DecoderOutput(sample=dec)

    def _temporal_tiled_encode(self, x: paddle.Tensor) -> AutoencoderKLOutput:
        batch_size, num_channels, num_frames, height, width = tuple(x.shape)
    ...
    def _temporal_tiled_decode(self, z: paddle.Tensor, return_dict: bool = True) -> Union[DecoderOutput, paddle.Tensor]:
        ...
        dec = paddle.concat(x=result_row, axis=2)[:, :, :num_sample_frames]
