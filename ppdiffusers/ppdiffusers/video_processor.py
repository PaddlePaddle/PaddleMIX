...
class VideoProcessor(VaeImageProcessor):
    ...
    def preprocess_video(self, video, height: Optional[int] = None, width: Optional[int] = None) -> paddle.Tensor:
        """
        ...
        if isinstance(video, list) and isinstance(video[0], paddle.Tensor) and video[0].ndim == 5:
            warnings.warn(
        ...
        video = paddle.stack(x=[self.preprocess(img, height=height, width=width) for img in video], axis=0)
        video = video.transpose(perm=[0, 2, 1, 3, 4])
        return video

    def postprocess_video(
        self, video: paddle.Tensor, output_type: str = "np"
        ...
        if output_type == "np":
            ...
            raise ValueError(f"{output_type} does not exist. Please choose one of ['np', 'pd', 'pil']")
        return outputs
