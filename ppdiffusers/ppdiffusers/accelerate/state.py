    ...
    **Available attributes:**
    ...
    def __init__(self, cpu: bool = False, **kwargs):
        ...
        if not self.initialized:
            self._cpu = cpu
            self.backend = None
            env_device = os.environ.get("ACCELERATE_PADDLE_DEVICE", None)
            self.device = str(env_device) if env_device is not None else None
            ...
            if local_rank != -1 and not cpu and paddle.is_compiled_with_cuda():
                ...
                if self.device is None:
                    self.device = "cpu" if cpu else self.default_device
            ...
            if isinstance(inputs, (list, tuple, paddle.Tensor)):
                ...
                if apply_padding:
                    if isinstance(result, paddle.Tensor):
                        from .utils import pad_across_processes, send_to_device
    ...
    **Available attributes:**
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._check_initialized(mixed_precision, cpu, fp16_opt_level)
        if not self.initialized:
    ...
    def _check_initialized(self, mixed_precision=None, cpu=None, fp16_opt_level=None):
        ...
        if self.initialized:
            ...
            if cpu and self.device != "cpu":
                raise ValueError(err.format(flag="cpu=True"))
            if mixed_precision is not None and mixed_precision != self._mixed_precision:
                raise ValueError(err.format(flag=f"mixed_precision='{mixed_precision}'"))
            if fp16_opt_level is not None and fp16_opt_level != self._fp16_opt_level:
                raise ValueError(err.format(flag=f"fp16_opt_level='{fp16_opt_level}'"))
    ...
    def fp16_opt_level(self):
        fp16_opt_level = self._fp16_opt_level
        return fp16_opt_level
