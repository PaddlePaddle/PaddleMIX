...
from .constants import PADDLE_DISTRIBUTED_OPERATION_TYPES
...
def recursively_apply(func, data, *args, test_type=is_paddle_tensor, error_on_other_type=False, **kwargs):
    """
...
def initialize_tensors(data_structure):
    ...
    def _initialize_tensor(tensor_info):
        return paddle.empty(tensor_info.shape, dtype=tensor_info.dtype)
...
def gather(tensor):
    ...
    Returns:
        The same data structure as `tensor` with all tensors sent to the proper device.
    """
    if PartialState().distributed_type in PADDLE_DISTRIBUTED_OPERATION_TYPES:
...
def reduce(tensor, reduction="mean", scale=1.0):
    ...
    def _reduce_across_processes(tensor, reduction="mean", scale=1.0):
        ...
        elif state.distributed_type.value in PADDLE_DISTRIBUTED_OPERATION_TYPES:
