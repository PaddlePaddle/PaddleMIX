...
from .configuration import UMT5Config
...
class UMT5DenseGatedActDense(nn.Layer):
    def __init__(self, config: UMT5Config):
        super().__init__()
...
class UMT5Attention(nn.Layer):
    ...
    def _shape(self, projection: paddle.Tensor) -> paddle.Tensor:
    ...
    def _relative_position_bucket(self, relative_position):
        ...
        if not self.is_decoder:
            num_buckets //= 2
            relative_buckets += (relative_position > 0).to(dtype=paddle.int64) * num_buckets
        ...
        log_ratio = paddle.log(relative_position.cast(paddle.float32) / max_exact) / math.log(max_distance / max_exact)
        log_ratio = log_ratio * (num_buckets - max_exact)
        relative_position_if_large = max_exact + log_ratio.to(dtype=paddle.int64)
    ...
    def forward(
        ...
        query_states = query_states.view([batch_size, -1, self.n_heads, self.key_value_proj_dim]).transpose([0, 2, 1])
        ...
        if is_cross_attention and past_key_value is not None and is_updated:
            ...
            key_states = key_states.view([batch_size, -1, self.n_heads, self.key_value_proj_dim]).transpose([0, 2, 1])
            value_states = value_states.view([batch_size, -1, self.n_heads, self.key_value_proj_dim]).transpose(
                [0, 2, 1]
            )
        ...
        # compute scores, equivalent of paddle.einsum("bnqd,bnkd->bnqk", query_states, key_states), compatible with onnx op>9
        scores = paddle.matmul(query_states, key_states.transpose([0, 1, 3, 2]))
        ...
        if not self.has_relative_attention_bias:
            position_bias = paddle.zeros((1, self.n_heads, seq_length, key_length), dtype=scores.dtype)
        else:
        ...
        if self.pruned_heads:
            mask = paddle.ones(position_bias.shape[1])
        ...
        attn_weights = nn.functional.softmax(scores.cast(paddle.float32), axis=-1).cast(scores.dtype)
...
class UMT5LayerSelfAttention(nn.Layer):
...
class UMT5LayerCrossAttention(nn.Layer):
    def __init__(self, config, layer_idx: Optional[int] = None):
        super().__init__()
...
class UMT5ClassificationHead(nn.Layer):
...
class UMT5Stack(UMT5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.is_decoder = config.is_decoder
        self.block = nn.LayerList([UMT5Block(config, layer_idx=i) for i in range(config.num_layers)])
    ...
    def forward(
        ...
        if cache_position is None:
            cache_position = paddle.arange(
                ...
                past_key_values_length + seq_length,  # device=inputs_embeds.device
            )
        ...
        if attention_mask is None:
            ...
            # attention_mask = paddle.ones(batch_size, mask_seq_length, device=inputs_embeds.device)
        ...
        if self.is_decoder:
            ...
            causal_mask = (1.0 - causal_mask) * paddle.finfo(inputs_embeds.dtype).min
        else:
            causal_mask = None
...
class UMT5Model(UMT5PreTrainedModel):
    ...
    def forward(
        ...
        >>> # This is not needed for paddle's UMT5ForConditionalGeneration as it does this internally using labels arg.
