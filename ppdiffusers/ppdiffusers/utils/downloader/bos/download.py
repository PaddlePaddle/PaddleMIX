...
def repo_folder_name(*, repo_id: str, repo_type: str) -> str:
    """Return a serialized version of a bos repo name and type, safe for disk storage
...
REPO_TYPES = ["model"]
BOS_OFFLINE = _is_true(os.environ.get("BOS_OFFLINE"))

BOS_DISABLE_SYMLINKS_WARNING: bool = _is_true(os.environ.get("BOS_DISABLE_SYMLINKS_WARNING"))
BOS_LOCAL_DIR_AUTO_SYMLINK_THRESHOLD: int = (
    _as_int(os.environ.get("BOS_LOCAL_DIR_AUTO_SYMLINK_THRESHOLD")) or 5 * 1024 * 1024
)
DOWNLOAD_CHUNK_SIZE = 10 * 1024 * 1024


DEFAULT_ETAG_TIMEOUT = 10
BOS_ETAG_TIMEOUT: int = _as_int(os.environ.get("BOS_ETAG_TIMEOUT")) or DEFAULT_ETAG_TIMEOUT
...
BOS_DOWNLOAD_TIMEOUT: int = _as_int(os.environ.get("BOS_DOWNLOAD_TIMEOUT")) or DEFAULT_DOWNLOAD_TIMEOUT


class OfflineModeIsEnabled(ConnectionError):
    """Raised when a request is made but `BOS_OFFLINE=1` is set as environment variable."""
...
class OfflineAdapter(HTTPAdapter):
    def send(self, request: PreparedRequest, *args, **kwargs) -> Response:
        raise OfflineModeIsEnabled(
            f"Cannot reach {request.url}: offline mode is enabled. To disable it, please unset the `BOS_OFFLINE` environment variable."
        )
...
def are_symlinks_supported(cache_dir: Union[str, Path, None] = None) -> bool:
    ...
    on the precise cache folder. By default, the default BOS cache directory is checked.
    ...
    if cache_dir not in _are_symlinks_supported_in_dir:
        ...
        with SoftTemporaryDirectory(dir=cache_dir) as tmpdir:
            ...
            try:
                ...
                if not BOS_DISABLE_SYMLINKS_WARNING:
                    message = (
                        ...
                        " `BOS_DISABLE_SYMLINKS_WARNING` environment variable."
                    )
                    if os.name == "nt":
                        message += (
...
def _to_local_dir(
    ...
    if use_symlinks == "auto":
        use_symlinks = os.stat(real_blob_path).st_size > BOS_LOCAL_DIR_AUTO_SYMLINK_THRESHOLD
...
def bos_download(
    ...
    **kwargs,
    ...
    if BOS_ETAG_TIMEOUT != DEFAULT_ETAG_TIMEOUT:
        # Respect environment variable above user value
        etag_timeout = BOS_ETAG_TIMEOUT
    ...
    with FileLock(lock_path):
        ...
        if local_dir is None:
            ...
            is_big_file = os.stat(temp_file.name).st_size > BOS_LOCAL_DIR_AUTO_SYMLINK_THRESHOLD
