...
PPDIFFUSERS_OLD_TO_PPDIFFUSERS = {
...
def convert_state_dict_to_ppdiffusers(state_dict, original_type=None, **kwargs):
    ...
    if original_type not in PPDIFFUSERS_STATE_DICT_MAPPINGS.keys():
        raise ValueError(f"Original type {original_type} is not supported")

    mapping = PPDIFFUSERS_STATE_DICT_MAPPINGS[original_type]
    return convert_state_dict(state_dict, mapping)
