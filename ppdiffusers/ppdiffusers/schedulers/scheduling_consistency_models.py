...
class CMStochasticIterativeScheduler(SchedulerMixin, ConfigMixin):
    ...
    def get_scalings_for_boundary_condition(self, sigma):
        ...
        c_out = (sigma - sigma_min) * sigma_data / (
    ...
    def step(
        ...
        if (isinstance(timestep, int) or "int" in timestep.dtype):
            raise ValueError((
        ...
        if len(self.timesteps) > 1:
            ...
            noise = paddle.zeros_like(model_output)
        z = noise * self.config.s_noise

        sigma_hat = sigma_next.clip(min=sigma_min, max=sigma_max)
