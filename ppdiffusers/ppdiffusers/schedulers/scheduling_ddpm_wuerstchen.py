...
class DDPMWuerstchenScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.scaler = scaler
        self.s = paddle.to_tensor([s])
        self._init_alpha_cumprod = paddle.cos(self.s / (1 + self.s) * math.pi * 0.5) ** 2
    ...
    def _alpha_cumprod(self, t):
        if self.scaler > 1:
            ...
            t = t**self.scaler
        alpha_cumprod = paddle.cos((t + self.s) / (1 + self.s) * math.pi * 0.5) ** 2 / self._init_alpha_cumprod
        return alpha_cumprod.clip(0.0001, 0.9999)
    ...
    def set_timesteps(
        ...
        if timesteps is None:
            timesteps = paddle.linspace(1.0, 0.0, num_inference_steps + 1)
    ...
    def step(
        ...
        alpha_cumprod = self._alpha_cumprod(t).reshape([t.shape[0], *[1 for _ in sample.shape[1:]]])
        alpha_cumprod_prev = self._alpha_cumprod(prev_t).reshape([prev_t.shape[0], *[1 for _ in sample.shape[1:]]])
        ...
        pred = mu + std * (prev_t != 0).cast("float32").reshape([prev_t.shape[0], *[1 for _ in sample.shape[1:]]])
    ...
    def add_noise(
        ...
        alpha_cumprod = self._alpha_cumprod(timesteps).reshape(
        ...
        return noisy_samples.cast(dtype=dtype)
    ...
    def previous_timestep(self, timestep):
        ...
        prev_t = self.timesteps[index + 1][None].expand(
            [
                timestep.shape[0],
            ]
        )
