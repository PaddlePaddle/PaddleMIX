...
class DPMSolverMultistepScheduler(SchedulerMixin, ConfigMixin):
    ...
    def set_timesteps(self, num_inference_steps: int=None):
        ...
        if self.config.timestep_spacing == "linspace":
            timesteps = (
                np.linspace(0, last_timestep - 1, num_inference_steps +
            ...
            # creates integer timesteps by multiplying by ratio
            # casting to int to avoid issues when num_inference_step is power of 3
            timesteps = np.arange(last_timestep, 0,
    ...
    def dpm_solver_first_order_update(
        ...
        if self.config.algorithm_type == "dpmsolver++":
            ...
            x_t = ((sigma_t / sigma_s * paddle.exp(-h)) * sample +
                   (alpha_t * (1 - paddle.exp(-2.0 * h))) * model_output +
                   sigma_t * paddle.sqrt(1.0 - paddle.exp(-2 * h)) * noise)
        elif self.config.algorithm_type == "sde-dpmsolver":
            assert noise is not None
            x_t = ((alpha_t / alpha_s) * sample - 2.0 *
                   (sigma_t * (paddle.exp(h) - 1.0)) * model_output + sigma_t *
                   paddle.sqrt(paddle.exp(2 * h) - 1.0) * noise)
        return x_t
    ...
    def multistep_dpm_solver_second_order_update(
        ...
        if self.config.algorithm_type == "dpmsolver++":
            ...
            assert noise is not None
            if self.config.solver_type == "midpoint":
                x_t = ((sigma_t / sigma_s0 * paddle.exp(-h)) * sample +
                       (alpha_t * (1 - paddle.exp(-2.0 * h))) * D0 + 0.5 *
                       (alpha_t * (1 - paddle.exp(-2.0 * h))) * D1 + sigma_t *
                       paddle.sqrt(1.0 - paddle.exp(-2 * h)) * noise)
            elif self.config.solver_type == "heun":
                x_t = (
                    (sigma_t / sigma_s0 * paddle.exp(-h)) * sample +
                    (alpha_t * (1 - paddle.exp(-2.0 * h))) * D0 + (alpha_t * (
                        (1.0 - paddle.exp(-2.0 * h)) / (-2.0 * h) + 1.0)) * D1 +
                    sigma_t * paddle.sqrt(1.0 - paddle.exp(-2 * h)) * noise)
        elif self.config.algorithm_type == "sde-dpmsolver":
            assert noise is not None
            if self.config.solver_type == "midpoint":
                x_t = ((alpha_t / alpha_s0) * sample - 2.0 *
                       ...
                       (sigma_t * (paddle.exp(h) - 1.0)) * D1 + sigma_t *
                       paddle.sqrt(paddle.exp(2 * h) - 1.0) * noise)
            elif self.config.solver_type == "heun":
                x_t = ((alpha_t / alpha_s0) * sample - 2.0 *
                       (sigma_t *
                        (paddle.exp(h) - 1.0)) * D0 - 2.0 * (sigma_t * (
