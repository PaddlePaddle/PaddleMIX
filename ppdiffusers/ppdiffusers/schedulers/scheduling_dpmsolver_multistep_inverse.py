...
class DPMSolverMultistepInverseScheduler(SchedulerMixin, ConfigMixin):
    ...
    # Copied from ppdiffusers.schedulers.scheduling_dpmsolver_multistep.DPMSolverMultistepScheduler.dpm_solver_first_order_update
        ...
        **kwargs,
        ...
        if self.config.algorithm_type == "dpmsolver++":
            ...
            x_t = (
                ...
                - 2.0 * (sigma_t * (paddle.exp(h) - 1.0)) * model_output
                + sigma_t * paddle.sqrt(paddle.exp(2 * h) - 1.0) * noise
            )
        return x_t

    # Copied from ppdiffusers.schedulers.scheduling_dpmsolver_multistep.DPMSolverMultistepScheduler.multistep_dpm_solver_second_order_update
        ...
        **kwargs,
        ...
        if self.config.algorithm_type == "dpmsolver++":
            ...
            if self.config.solver_type == "midpoint":
                x_t = (
                    (alpha_t / alpha_s0) * sample
                    - 2.0 * (sigma_t * (paddle.exp(h) - 1.0)) * D0
                ...
                x_t = (
                    (alpha_t / alpha_s0) * sample
                    - 2.0 * (sigma_t * (paddle.exp(h) - 1.0)) * D0
                    - 2.0 * (sigma_t * ((paddle.exp(h) - 1.0) / h - 1.0)) * D1
                    + sigma_t * paddle.sqrt(paddle.exp(2 * h) - 1.0) * noise
                )
        return x_t
    ...
    # Copied from ppdiffusers.schedulers.scheduling_dpmsolver_multistep.DPMSolverMultistepScheduler.step
    def step(
        self,
        model_output: paddle.Tensor,
        timestep: int,
