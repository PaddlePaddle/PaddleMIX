...
class CogVideoXDDIMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if trained_betas is not None:
            ...
            self.betas = paddle.linspace(start=beta_start, stop=beta_end, num=num_train_timesteps, dtype="float32")
        elif beta_schedule == "scaled_linear":
            self.betas = (
                paddle.linspace(
                    start=beta_start**0.5, stop=beta_end**0.5, num=num_train_timesteps, dtype="float64"
                )
                ** 2
            )
        ...
        self.final_alpha_cumprod = paddle.to_tensor(data=1.0) if set_alpha_to_one else self.alphas_cumprod[0]
        self.init_noise_sigma = 1.0
        self.num_inference_steps = None
        self.timesteps = paddle.to_tensor(data=np.arange(0, num_train_timesteps)[::-1].copy().astype(np.int64))
    ...
    def add_noise(
        ...
        while len(tuple(sqrt_alpha_prod.shape)) < len(tuple(original_samples.shape)):
            sqrt_alpha_prod = sqrt_alpha_prod.unsqueeze(axis=-1)
        sqrt_one_minus_alpha_prod = (1 - alphas_cumprod[timesteps]) ** 0.5
        sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod.flatten()
        while len(tuple(sqrt_one_minus_alpha_prod.shape)) < len(tuple(original_samples.shape)):
    ...
    def get_velocity(self, sample: paddle.Tensor, noise: paddle.Tensor, timesteps: paddle.Tensor) -> paddle.Tensor:
        ...
        sqrt_one_minus_alpha_prod = (1 - alphas_cumprod[timesteps]) ** 0.5
        sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod.flatten()
        while len(tuple(sqrt_one_minus_alpha_prod.shape)) < len(tuple(sample.shape)):
            sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod.unsqueeze(axis=-1)
