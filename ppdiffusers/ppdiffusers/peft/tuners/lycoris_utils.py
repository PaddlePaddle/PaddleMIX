...
from ppdiffusers.peft.config import PeftConfig
from ppdiffusers.peft.utils import ModulesToSaveWrapper, _get_submodules
...
class LycorisLayer(BaseTunerLayer):
    ...
    def _init_empty_weights(self, cls, *args, **kwargs) -> None:
        ...
        kwargs = kwargs.copy()
        with paddle.LazyGuard(), no_init_weights(True):
    ...
    @abstractmethod
    def _get_delta_activations(self, adapter_name: str, x: paddle.Tensor, *args: Any, **kwargs: Any) -> paddle.Tensor:
...
class LycorisTuner(BaseTuner):
    ...
    prefix: str
    layers_mapping: Dict[Type[paddle.nn.Layer], Type[LycorisLayer]]
    ...
    def _create_and_replace(
        ...
        adapter_name: str,
        target: Union[LycorisLayer, nn.Layer],
        target_name,
        parent,
        current_key,
        **optional_kwargs,
    ...
    @classmethod
    def _create_new_module(cls, config: LycorisConfig, adapter_name: str, target: nn.Layer, **kwargs) -> LycorisLayer:
