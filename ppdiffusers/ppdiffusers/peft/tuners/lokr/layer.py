...
class LoKrLayer(nn.Layer, LycorisLayer):
    ...
    def __init__(self, base_layer: nn.Layer) -> None:
        super().__init__()
    ...
    def create_adapter_parameters(
        ...
        if use_w1:
            self.lokr_w1[adapter_name] = nn.Parameter(paddle.empty([shape[0][0], shape[1][0]]))
        else:
            self.lokr_w1_a[adapter_name] = nn.Parameter(paddle.empty([shape[0][0], r]))
            self.lokr_w1_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1][0]]))

        if len(shape) == 4:
            # Conv2d
            if use_w2:
                self.lokr_w2[adapter_name] = nn.Parameter(paddle.empty([shape[0][1], shape[1][1], *shape[2:]]))
            elif use_effective_conv2d:
                self.lokr_t2[adapter_name] = nn.Parameter(paddle.empty([r, r, shape[2], shape[3]]))
                self.lokr_w2_a[adapter_name] = nn.Parameter(paddle.empty([r, shape[0][1]]))  # b, 1-mode
                self.lokr_w2_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1][1]]))  # d, 2-mode
            else:
                self.lokr_w2_a[adapter_name] = nn.Parameter(paddle.empty([shape[0][1], r]))
                self.lokr_w2_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1][1] * shape[2] * shape[3]]))
        else:
            # Linear
            if use_w2:
                self.lokr_w2[adapter_name] = nn.Parameter(paddle.empty([shape[0][1], shape[1][1]]))
            else:
                self.lokr_w2_a[adapter_name] = nn.Parameter(paddle.empty([shape[0][1], r]))
                self.lokr_w2_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1][1]]))
    ...
    def update_layer(
        ...
        **kwargs,
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            in_dim, out_dim = base_layer._in_channels, base_layer._out_channels
            k_size = base_layer._kernel_size
            ...
            use_effective_conv2d = use_effective_conv2d and base_layer._kernel_size != [1, 1]
        else:
...
def make_weight_cp(t, wa, wb):
    rebuild2 = paddle.einsum("i j k l, i p, j r -> p r k l", t, wa, wb)  # [c, d, k1, k2]
    return rebuild2
