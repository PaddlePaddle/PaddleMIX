...
from ppdiffusers.peft.tuners.prompt_tuning import PromptEmbedding
from ppdiffusers.peft.utils import TaskType
...
class MultitaskPromptEmbedding(PromptEmbedding):
    def __init__(self, config: MultitaskPromptTuningConfig, word_embeddings):
        ...
        if config.prompt_tuning_init in [
            ...
            state_dict: dict = paddle.load(
                config.prompt_tuning_init_state_dict_path,
                return_numpy=False if word_embeddings.place.is_gpu_place() else True,
            )
        ...
        if config.prompt_tuning_init in [
            ...
            prefix_task_cols_: paddle.Tensor = state_dict["prefix_task_cols"]
            prefix_task_rows_: paddle.Tensor = state_dict["prefix_task_rows"]
    ...
    def forward(self, indices, task_ids):
        ...
        task_cols = paddle.index_select(self.prefix_task_cols, task_ids, 0)
        task_rows = paddle.index_select(self.prefix_task_rows, task_ids, 0)
        task_prompts = paddle.matmul(task_cols, task_rows)
