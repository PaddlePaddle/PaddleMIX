...
class IA3Model(BaseTuner):
    ...
    Returns:
        `paddle.nn.Layer`: The (IA)^3 model.

    Example:

        ```py
        >>> from paddlenlp.transformers import AutoModelForSeq2SeqLM, ia3Config
        >>> from ppdiffusers.peft import IA3Model, IA3Config
    ...
    **Attributes**:
    ...
    def _create_new_module(ia3_config, adapter_name, target, **kwargs):
        ...
        if isinstance(target, paddle.nn.Conv2D):
            ...
            raise ValueError(
                ...
                f"Currently, only `paddle.nn.Linear`, `paddle.nn.Conv2D` are supported."
            )
        return new_module
    ...
    def merge_and_unload(self, safe_merge: bool = False, adapter_names: Optional[List[str]] = None) -> paddle.nn.Layer:
        r"""
