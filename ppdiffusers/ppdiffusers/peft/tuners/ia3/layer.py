...
class IA3Layer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Layer, is_feedforward: bool, **kwargs) -> None:
    ...
    def update_layer(self, adapter_name, init_ia3_weights):
        ...
        if self.is_feedforward:
            weight = paddle.randn((1, self.in_features))
        else:
            weight = paddle.randn((self.out_features, 1))
...
class Linear(nn.Layer, IA3Layer):
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.ia3_l.keys():
                ...
                if safe_merge:
                    ...
                    orig_weights = paddle.multiply(orig_weights, ia3_l)
                    ...
                    base_layer.weight.data = paddle.multiply(base_layer.weight.data, ia3_l)
                ...
                if not self.is_feedforward and (base_layer.bias is not None):
                    ...
                    base_layer.bias.data = paddle.multiply(base_layer.bias.data, scaling.data)
    ...
    def unmerge(self) -> None:
        ...
        while len(self.merged_adapters) > 0:
            ...
            if active_adapter in self.ia3_l.keys():
                ...
                base_layer.weight.data = paddle.divide(base_layer.weight.data, ia3_l)

                if not self.is_feedforward and (base_layer.bias is not None):
                    scaling = self.ia3_l[active_adapter].reshape(base_layer.bias.shape)
                    base_layer.bias.data = paddle.divide(base_layer.bias.data, scaling.data + 1e-8)
...
class Conv2d(nn.Layer, IA3Layer):
    def __init__(
        self,
        base_layer: nn.Layer,
        adapter_name: str,
    ...
    def update_layer(self, adapter_name, init_ia3_weights):
        # Actual trainable parameters
        if self.is_feedforward:
            weight = paddle.randn((1, self.in_features, 1, 1))
        else:
            weight = paddle.randn((1, self.out_features, 1, 1))
        self.ia3_l[adapter_name] = nn.Parameter(weight)
        if init_ia3_weights:
            self.reset_ia3_parameters(adapter_name)
        self.set_adapter(self.active_adapters)
        ...
        for active_adapter in adapter_names:
            if active_adapter in self.ia3_l.keys():
                ...
                if not self.is_feedforward:
                    ia3_scaling = ia3_scaling.transpose([1, 0, 2, 3])

                if safe_merge:
                    output_weight = paddle.multiply(base_layer.weight.data, ia3_scaling).clone()

                    if not paddle.isfinite(output_weight).all():
                        raise ValueError(
                            f"NaNs detected in the merged weights. The adapter {active_adapter} seems to be broken"
                        )
                    ...
                    base_layer.weight.data = paddle.multiply(base_layer.weight.data, ia3_scaling)
    ...
    def unmerge(self) -> None:
        ...
        while len(self.merged_adapters) > 0:
            ...
            if active_adapter in self.ia3_l.keys():
                ...
                if not self.is_feedforward:
                    ia3_scaling = ia3_scaling.transpose([1, 0, 2, 3])
                base_layer.weight.data = paddle.divide(base_layer.weight.data, ia3_scaling + 1e-8)
