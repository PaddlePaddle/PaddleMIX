...
from ppdiffusers.peft.utils import COMMON_LAYERS_PATTERN
...
class BaseTuner(nn.Layer, ABC):
    ...
    def __init__(
        self, model: nn.Layer, peft_config: Union[PeftConfig, dict[str, PeftConfig]], adapter_name: str
    ) -> None:
        super().__init__()
    ...
    def inject_adapter(self, model: nn.Layer, adapter_name: str):
        r"""
...
class BaseTunerLayer(ABC):
    ...
    def get_base_layer(self) -> nn.Layer:
        """
...
def inspect_matched_modules(tuner: BaseTuner, adapter_name: str = "default") -> dict:
    ...
    key_list = [key for key, _ in tuner.model.named_sublayers(include_self=True)]
