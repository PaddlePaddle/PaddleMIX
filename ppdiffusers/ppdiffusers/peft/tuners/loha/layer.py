...
class LoHaLayer(nn.Layer, LycorisLayer):
    ...
    def create_adapter_parameters(self, adapter_name: str, r: int, shape: Tuple[int, ...]):
        ...
        if len(shape) == 4:
            self.hada_t1[adapter_name] = nn.Parameter(paddle.empty([r, r, shape[2], shape[3]]))
            self.hada_w1_a[adapter_name] = nn.Parameter(paddle.empty([r, shape[0]]))  # out_dim, 1-mode
            self.hada_w1_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1]]))  # in_dim , 2-mode

            self.hada_t2[adapter_name] = nn.Parameter(paddle.empty([r, r, shape[2], shape[3]]))
            self.hada_w2_a[adapter_name] = nn.Parameter(paddle.empty([r, shape[0]]))  # out_dim, 1-mode
            self.hada_w2_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1]]))  # in_dim , 2-mode
        else:
            self.hada_w1_a[adapter_name] = nn.Parameter(paddle.empty([shape[0], r]))
            self.hada_w1_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1]]))

            self.hada_w2_a[adapter_name] = nn.Parameter(paddle.empty([shape[0], r]))
            self.hada_w2_b[adapter_name] = nn.Parameter(paddle.empty([r, shape[1]]))
        ...
        **kwargs,
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            if use_effective_conv2d:
                shape = (base_layer._out_channels, base_layer._in_channels, *base_layer._kernel_size)
            else:
                shape = (
    ...
    def get_delta_weight(self, adapter_name: str) -> paddle.Tensor:
        ...
        if adapter_name in self.hada_t1.keys():
            weight = make_weight_cp(
                ...
                scale=paddle.to_tensor(self.scaling[adapter_name]),
            )
        else:
            weight = make_weight(
                self.hada_w1_a[adapter_name],
        ...
        if self.training and rank_dropout:
            drop = (paddle.rand([weight.shape[0]]) > rank_dropout).cast(weight.dtype)
            drop = drop.reshape([-1, *[1] * len(weight.shape[1:])])
...
class Conv2d(LoHaLayer):
    ...
    def _get_delta_activations(
        ...
        return F.conv2d(
            ...
            dilation=base_layer._dilation,
            groups=base_layer._groups,
        )
...
class HadaWeight(paddle.autograd.PyLayer):
    @staticmethod
    def forward(ctx, w1a, w1b, w2a, w2b, scale=paddle.to_tensor(1)):
    ...
    def backward(ctx, grad_out):
        (w1a, w1b, w2a, w2b, scale) = ctx.saved_tensor()
...
class HadaWeightCP(paddle.autograd.PyLayer):
    @staticmethod
    def forward(ctx, t1, w1a, w1b, t2, w2a, w2b, scale=paddle.to_tensor(1)):
        ctx.save_for_backward(t1, w1a, w1b, t2, w2a, w2b, scale)

        rebuild1 = paddle.einsum("i j k l, j r, i p -> p r k l", t1, w1b, w1a)
        rebuild2 = paddle.einsum("i j k l, j r, i p -> p r k l", t2, w2b, w2a)

        return rebuild1 * rebuild2 * scale

    @staticmethod
    def backward(ctx, grad_out):
        (t1, w1a, w1b, t2, w2a, w2b, scale) = ctx.saved_tensor()
        grad_out = grad_out * scale

        temp = paddle.einsum("i j k l, j r -> i r k l", t2, w2b)
        rebuild = paddle.einsum("i j k l, i r -> r j k l", temp, w2a)

        grad_w = rebuild * grad_out
        del rebuild

        grad_w1a = paddle.einsum("r j k l, i j k l -> r i", temp, grad_w)
        grad_temp = paddle.einsum("i j k l, i r -> r j k l", grad_w, w1a.T)
        del grad_w, temp

        grad_w1b = paddle.einsum("i r k l, i j k l -> r j", t1, grad_temp)
        grad_t1 = paddle.einsum("i j k l, j r -> i r k l", grad_temp, w1b.T)
        del grad_temp

        temp = paddle.einsum("i j k l, j r -> i r k l", t1, w1b)
        rebuild = paddle.einsum("i j k l, i r -> r j k l", temp, w1a)

        grad_w = rebuild * grad_out
        del rebuild

        grad_w2a = paddle.einsum("r j k l, i j k l -> r i", temp, grad_w)
        grad_temp = paddle.einsum("i j k l, i r -> r j k l", grad_w, w2a.T)
        del grad_w, temp

        grad_w2b = paddle.einsum("i r k l, i j k l -> r j", t2, grad_temp)
        grad_t2 = paddle.einsum("i j k l, j r -> i r k l", grad_temp, w2b.T)
        del grad_temp
