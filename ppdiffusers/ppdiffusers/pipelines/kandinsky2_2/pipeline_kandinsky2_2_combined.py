...
class KandinskyV22CombinedPipeline(DiffusionPipeline):
    ...
    def __init__(
            ...
            movq: VQModel,
            prior_prior: PriorTransformer,
            prior_image_encoder: paddlenlp.transformers.
            CLIPVisionModelWithProjection,
            prior_text_encoder: paddlenlp.transformers.
...
class KandinskyV22Img2ImgCombinedPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if len(prompt) < image_embeds.shape[0] and image_embeds.shape[0] % len(
                prompt) == 0:
            prompt = image_embeds.shape[0] // len(prompt) * prompt
        if isinstance(image, (list, tuple)) and len(image) < image_embeds.shape[
                0] and image_embeds.shape[0] % len(image) == 0:
...
class KandinskyV22InpaintCombinedPipeline(DiffusionPipeline):
    ...
    def __call__(
            ...
            image: Union[paddle.Tensor, PIL.Image.Image, List[paddle.Tensor],
                         List[PIL.Image.Image]],
            mask_image: Union[paddle.Tensor, PIL.Image.Image, List[
                paddle.Tensor], List[PIL.Image.Image]],
            negative_prompt: Optional[Union[str, List[str]]]=None,
            num_inference_steps: int=100,
            guidance_scale: float=4.0,
            num_images_per_prompt: int=1,
        ...
        prompt = [prompt] if not isinstance(prompt, (list, tuple)) else prompt
        image = [image] if isinstance(prompt, PIL.Image.Image) else image
        mask_image = [mask_image] if isinstance(mask_image,
                                                PIL.Image.Image) else mask_image
        if len(prompt) < image_embeds.shape[0] and image_embeds.shape[0] % len(
                prompt) == 0:
            prompt = image_embeds.shape[0] // len(prompt) * prompt
        if isinstance(image, (list, tuple)) and len(image) < image_embeds.shape[
                0] and image_embeds.shape[0] % len(image) == 0:
            image = image_embeds.shape[0] // len(image) * image
        if isinstance(mask_image,
                      (list, tuple)) and len(mask_image) < image_embeds.shape[
                          0] and image_embeds.shape[0] % len(mask_image) == 0:
