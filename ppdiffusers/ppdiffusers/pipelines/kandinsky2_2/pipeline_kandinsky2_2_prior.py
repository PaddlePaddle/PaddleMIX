...
class KandinskyV22PriorPipeline(DiffusionPipeline):
    ...
    def interpolate(self,
            ...
            if isinstance(cond, str):
                ...
                if isinstance(cond, PIL.Image.Image):
                    cond = self.image_processor(
                        cond, return_tensors='pt').pixel_values[0].unsqueeze(
                            axis=0).to(dtype=self.image_encoder.dtype,
                                       device=device)
                image_emb = self.image_encoder(cond)['image_embeds'].tile(
                    repeat_times=[num_images_per_prompt, 1]).unsqueeze(axis=0)
            else:
                raise ValueError(
        ...
        zero_image_emb = (out_zero.negative_image_embeds
                          if negative_prompt == '' else out_zero.image_embeds)
