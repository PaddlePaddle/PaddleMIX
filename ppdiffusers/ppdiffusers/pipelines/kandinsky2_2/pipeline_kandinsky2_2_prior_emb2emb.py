...
class KandinskyV22PriorEmb2EmbPipeline(DiffusionPipeline):
    ...
    def _encode_image(self,
        ...
        if not isinstance(image, paddle.Tensor):
            image = self.image_processor(
                image, return_tensors='pt').pixel_values.to(
    ...
    def __call__(self,
        ...
        prompt_embeds, text_encoder_hidden_states, text_mask = (
            self._encode_prompt(prompt, device, num_images_per_prompt,
        ...
        if isinstance(image, paddle.Tensor) and image.ndim == 2:
            ...
            raise ValueError(
                f' if pass `image` as pytorch tensor, or a list of pytorch tensor, please make sure each tensor has shape [batch_size, channels, height, width], currently {image[0].unsqueeze(axis=0).shape}'
            )
        else:
            image_embeds = self._encode_image(image, device,
                                              num_images_per_prompt)
