...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> from ppdiffusers import KandinskyV22Pipeline, KandinskyV22PriorEmb2EmbPipeline
        >>> import paddle
...
EXAMPLE_INTERPOLATE_DOC_STRING = """
    Examples:
        ```py
        >>> from ppdiffusers import KandinskyV22PriorEmb2EmbPipeline, KandinskyV22Pipeline
...
class KandinskyV22PriorEmb2EmbPipeline(DiffusionPipeline):
    ...
    def interpolate(self,
                    images_and_prompts: List[Union[str, PIL.Image.Image,
                                                   paddle.Tensor]],
                    weights: List[float],
        ...
        for cond, weight in zip(images_and_prompts, weights):
            if isinstance(cond, str):
                image_emb = self(
                    ...
                    guidance_scale=guidance_scale).image_embeds.unsqueeze(0)
            elif isinstance(cond, (PIL.Image.Image, paddle.Tensor)):
                image_emb = self._encode_image(
                    cond,
                    num_images_per_prompt=num_images_per_prompt).unsqueeze(
                        axis=0)
            else:
                raise ValueError(
        ...
        return KandinskyPriorPipelineOutput(
            image_embeds=image_emb,
            negative_image_embeds=paddle.randn(
                shape=image_emb.shape, dtype=image_emb.dtype))

    def _encode_image(self,
                      image: Union[paddle.Tensor, List[PIL.Image.Image]],
                      num_images_per_prompt):
        if not isinstance(image, paddle.Tensor):
            image = self.image_processor(
                image, return_tensors='pd').pixel_values.cast(
                    dtype=self.image_encoder.dtype)
        image_emb = self.image_encoder(image)['image_embeds']
        image_emb = image_emb.repeat_interleave(
    ...
    def _encode_prompt(self,
        ...
        text_encoder_hidden_states = (
            text_encoder_hidden_states.repeat_interleave(
                repeats=num_images_per_prompt, axis=0))
        text_mask = text_mask.repeat_interleave(
            repeats=num_images_per_prompt, axis=0)
        if do_classifier_free_guidance:
            ...
            uncond_text_encoder_hidden_states = (
                uncond_text_encoder_hidden_states.tile(
    ...
    def __call__(self,
                 ...
                 image: Union[paddle.Tensor, List[paddle.Tensor],
        ...
        elif not isinstance(negative_prompt,
                            list) and negative_prompt is not None:
            raise ValueError(
        ...
        if not isinstance(image, List):
            image = [image]
        if isinstance(image[0], paddle.Tensor):
            image = paddle.concat(x=image, axis=0)
        if isinstance(image, paddle.Tensor) and image.ndim == 2:
            image_embeds = image.repeat_interleave(
                repeats=num_images_per_prompt, axis=0)
        elif isinstance(image, paddle.Tensor) and image.ndim != 4:
            raise ValueError(
                f' if pass `image` as paddle tensor, or a list of paddle tensor, please make sure each tensor has shape [batch_size, channels, height, width], currently {image[0].unsqueeze(axis=0).shape}'
            )
        else:
        ...
        latents = self.prepare_latents(
            ...
            num_images_per_prompt, prompt_embeds.dtype, generator)
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if do_classifier_free_guidance:
                (predicted_image_embedding_uncond,
                 ...
                 ) = predicted_image_embedding.chunk(chunks=2)
                predicted_image_embedding = (
                    predicted_image_embedding_uncond + guidance_scale *
                    (predicted_image_embedding_text -
                     predicted_image_embedding_uncond))
