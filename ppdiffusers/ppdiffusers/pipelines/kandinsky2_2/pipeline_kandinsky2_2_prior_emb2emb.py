...
class KandinskyV22PriorEmb2EmbPipeline(DiffusionPipeline):
    ...
    def interpolate(
        ...
        for cond, weight in zip(images_and_prompts, weights):
            if isinstance(cond, str):
                ...
                image_emb = self._encode_image(cond, num_images_per_prompt=num_images_per_prompt).unsqueeze(axis=0)
            else:
                raise ValueError(
        ...
        return KandinskyPriorPipelineOutput(
            image_embeds=image_emb, negative_image_embeds=paddle.randn(shape=image_emb.shape, dtype=image_emb.dtype)
        )

    def _encode_image(self, image: Union[paddle.Tensor, List[PIL.Image.Image]], num_images_per_prompt):
        if not isinstance(image, paddle.Tensor):
            image = self.image_processor(image, return_tensors="pd").pixel_values.cast(dtype=self.image_encoder.dtype)
        image_emb = self.image_encoder(image)["image_embeds"]
        image_emb = image_emb.repeat_interleave(repeats=num_images_per_prompt, axis=0)
        return image_emb
    ...
    def __call__(
        ...
        image: Union[paddle.Tensor, List[paddle.Tensor], PIL.Image.Image, List[PIL.Image.Image]],
        strength: float = 0.3,
        ...
        if isinstance(image, paddle.Tensor) and image.ndim == 2:
            ...
            image_embeds = image.repeat_interleave(repeats=num_images_per_prompt, axis=0)
