    ...
    def __init__(
        ...
        **kwargs
        ...
        self.norm1 = paddle.nn.GroupNorm(
            num_groups=groups, num_channels=self.in_channels_prod, epsilon=eps, weight_attr=True, bias_attr=True
        )
        self.conv1 = paddle.nn.Conv2D(
            in_channels=self.in_channels_prod, out_channels=out_channels_prod, kernel_size=1, padding=0
        )
        if temb_channels is not None:
            self.time_emb_proj = paddle.nn.Linear(in_features=temb_channels, out_features=out_channels_prod)
        else:
            self.time_emb_proj = None
        self.norm2 = paddle.nn.GroupNorm(
            num_groups=groups_out, num_channels=out_channels_prod, epsilon=eps, weight_attr=True, bias_attr=True
        )
        self.dropout = paddle.nn.Dropout(p=dropout)
        self.conv2 = paddle.nn.Conv2D(
            in_channels=out_channels_prod, out_channels=out_channels_prod, kernel_size=1, padding=0
        )
        ...
        if self.use_in_shortcut:
            self.conv_shortcut = paddle.nn.Conv2D(
                in_channels=self.in_channels_prod, out_channels=out_channels_prod, kernel_size=1, stride=1, padding=0
            )
    ...
    def __init__(
        ...
        if add_upsample:
            self.upsamplers = paddle.nn.LayerList(
                sublayers=[LinearMultiDim(out_channels, use_conv=True, out_channels=out_channels)]
            )
        else:
