...
class GLIGENTextBoundingboxProjection(nn.Layer):
    def __init__(self, positive_len, out_dim, feature_type, fourier_freqs=8):
        ...
        if feature_type == "text-only":
            ...
            self.null_text_feature = nn.Parameter(
                paddle.zeros(
                    [
                        self.positive_len,
                    ]
            ...
            self.null_image_feature = nn.Parameter(
                paddle.zeros(
                    [
                        self.positive_len,
                    ]
        ...
        self.null_position_feature = nn.Parameter(
            paddle.zeros(
                [
                    self.position_dim,
                ]
...
# Copied from ppdiffusers.models.unet_2d_blocks.UNetMidBlock2D with UNetMidBlock2D->UNetMidBlockFlat, ResnetBlock2D->ResnetBlockFlat
class UNetMidBlockFlat(nn.Layer):
    """
...
class UNetMidBlockFlatSimpleCrossAttn(nn.Layer):
    def __init__(
        ...
        for _ in range(num_layers):
            processor = (
                AttnAddedKVProcessor2_5() if hasattr(F, "scaled_dot_product_attention") else AttnAddedKVProcessor()
            )
