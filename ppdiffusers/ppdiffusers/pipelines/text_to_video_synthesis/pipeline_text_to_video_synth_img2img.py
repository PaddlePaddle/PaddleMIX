...
def preprocess_video(video):
    ...
    if isinstance(video, supported_formats):
        video = [video]
    elif not (isinstance(video, list) and all(
            isinstance(i, supported_formats) for i in video)):
        raise ValueError(
    ...
    if isinstance(video[0], np.ndarray):
        video = np.concatenate(
            video, axis=0) if video[0].ndim == 5 else np.stack(
                video, axis=0)
...
class VideoToVideoSDPipeline(DiffusionPipeline, TextualInversionLoaderMixin,
                             LoraLoaderMixin):
    """
    ...
    def prepare_latents(self,
        ...
        if video.shape[1] == 4:
            ...
            if isinstance(generator, list) and len(generator) != batch_size:
                ...
                init_latents = [
                    self.vae.encode(video[i:i + 1]).latent_dist.sample(
                        generator[i]) for i in range(batch_size)
                ]
                init_latents = paddle.concat(x=init_latents, axis=0)
            else:
                init_latents = self.vae.encode(video).latent_dist.sample(
                    generator)
        ...
        latents = latents[(None), :].reshape((bsz, frames, latents.shape[
            1]) + latents.shape[2:]).transpose(perm=[0, 2, 1, 3, 4])
        return latents
