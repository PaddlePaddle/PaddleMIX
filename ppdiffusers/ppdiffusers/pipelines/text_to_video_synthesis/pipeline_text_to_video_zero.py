...
def warp_single_latent(latent, reference_flow):
    ...
    coords_t0 = paddle.nn.functional.interpolate(
        x=coords_t0, size=(h, w), mode='bilinear')
    coords_t0 = paddle.transpose(x=coords_t0, perm=(0, 2, 3, 1))
    warped = paddle.nn.functional.grid_sample(
...
class TextToVideoZeroPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        prompt_embeds = prompt_embeds[:, None].tile(
            repeat_times=[1, video_length, 1,
                          1]).reshape(b * video_length, l, d)
