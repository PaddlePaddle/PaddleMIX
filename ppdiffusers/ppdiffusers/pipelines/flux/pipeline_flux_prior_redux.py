...
class FluxPriorReduxPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        if hasattr(self, "text_encoder") and self.text_encoder is not None:
            ...
            prompt_embeds = paddle.zeros((batch_size, 512, 4096), dtype=image_embeds.dtype)
            # pooled_prompt_embeds is 768, clip text encoder hidden size
            pooled_prompt_embeds = paddle.zeros((batch_size, 768), dtype=image_embeds.dtype)
        ...
        pooled_prompt_embeds = paddle.sum(pooled_prompt_embeds, axis=0, keepdim=True)
