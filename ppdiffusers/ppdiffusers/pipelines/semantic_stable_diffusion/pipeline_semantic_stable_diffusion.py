...
class SemanticStableDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        text_embeddings = self.text_encoder(text_input_ids.to(self.place))[0]
        ...
        if enable_edit_guidance:
            if editing_prompt_embeddings is None:
                ...
                edit_concepts = self.text_encoder(
                    edit_concepts_input_ids.to(self.place))[0]
            else:
                edit_concepts = editing_prompt_embeddings.to(self.place).tile(
        ...
        if do_classifier_free_guidance:
            ...
            uncond_embeddings = self.text_encoder(
                uncond_input.input_ids.to(self.place))[0]
        ...
        latents = self.prepare_latents(
            ...
            width, text_embeddings.dtype, self.place, generator, latents)
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            if do_classifier_free_guidance:
                ...
                if enable_edit_guidance:
                    ...
                    for c, noise_pred_edit_concept in enumerate(
                        ...
                        tmp_weights = paddle.full_like(
                            x=tmp_weights, fill_value=edit_weight_c)
                        ...
                        if noise_guidance_edit_tmp.dtype == 'float32':
                            tmp = paddle.quantile(
                                x=paddle.abs(x=noise_guidance_edit_tmp).flatten(
                                    start_axis=2),
                                q=edit_threshold_c,
                                axis=2,
                                keepdim=False)
                    ...
                    if len(noise_pred_edit_concepts) > warmup_inds.shape[0] > 0:
                        ...
                        concept_weights_tmp = (concept_weights_tmp /
                                               concept_weights_tmp.sum(axis=0))
                        noise_guidance_edit_tmp = paddle.index_select(
                            x=noise_guidance_edit.to(self.place),
                            axis=0,
                            index=warmup_inds)
                        ...
                        concept_weights = concept_weights.to(self.place)
                        noise_guidance_edit = noise_guidance_edit.to(self.place)
                    concept_weights = paddle.where(
                        condition=concept_weights < 0,
                        x=paddle.zeros_like(x=concept_weights),
                        y=concept_weights)
                ...
                if sem_guidance is not None:
                    edit_guidance = sem_guidance[i].to(self.place)
        ...
        if not output_type == 'latent':
            ...
            image, has_nsfw_concept = self.run_safety_checker(
                image, self.place, text_embeddings.dtype)
        else:
            image = latents
            has_nsfw_concept = None
