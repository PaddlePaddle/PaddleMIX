...
def sample_pmf(pmf: paddle.Tensor, n_samples: int) -> paddle.Tensor:
    ...
    *shape, support_size, last_dim = pmf.shape
    ...
    cdf = paddle.cumsum(pmf.reshape([-1, support_size]), 1)
    inds = paddle.searchsorted(cdf.cast(paddle.float32), paddle.rand([cdf.shape[0], n_samples]).cast(paddle.float32))

    return inds.reshape([*shape, n_samples, 1]).clip(0, support_size - 1)
...
def posenc_nerf(x: paddle.Tensor, min_deg: int = 0, max_deg: int = 15) -> paddle.Tensor:
    ...
    scales = 2.0 ** paddle.arange(min_deg, max_deg, dtype=x.dtype)
    *shape, dim = x.shape
    xb = (x.reshape(-1, 1, dim) * scales.reshape([1, -1, 1])).reshape([*shape, -1])
    assert xb.shape[-1] == dim * (max_deg - min_deg)
    emb = paddle.concat([xb, xb + math.pi / 2.0], axis=-1).sin()
    return paddle.concat([x, emb], axis=-1)
...
def encode_direction(position, direction=None):
    if direction is None:
        return paddle.zeros_like(posenc_nerf(position, min_deg=0, max_deg=8))
    else:
...
def integrate_samples(volume_range, ts, density, channels):
    ...
    mass = paddle.cumsum(ddensity, -2)
    transmittance = paddle.exp(-mass[..., -1, :])

    alphas = 1.0 - paddle.exp(-ddensity)
    Ts = paddle.exp(paddle.concat([paddle.zeros_like(mass[..., :1, :]), -mass[..., :-1, :]], axis=-2))
    # This is the probability of light hitting and reflecting off of
    # something at depth [..., i, :].
    weights = alphas * Ts

    # 2. Integrate channels
    channels = paddle.sum(channels * weights, axis=-2)

    return channels, weights, transmittance


def volume_query_points(volume, grid_size):
    indices = paddle.arange(grid_size**3)
    zs = indices % grid_size
    ys = paddle.floor_divide(indices, grid_size) % grid_size  # , rounding_mode="trunc"
    xs = paddle.floor_divide(indices, grid_size**2) % grid_size  # , rounding_mode="trunc"
    combined = paddle.stack([xs, ys, zs], axis=1)
    return (combined.cast("float32") / (grid_size - 1)) * (volume.bbox_max - volume.bbox_min) + volume.bbox_min


def _convert_srgb_to_linear(u: paddle.Tensor):
    return paddle.where(u <= 0.04045, u / 12.92, ((u + 0.055) / 1.055) ** 2.4)


def _create_flat_edge_indices(
    flat_cube_indices: paddle.Tensor,
...
class VoidNeRFModel(nn.Layer):
    ...
    def __init__(self, background, channel_scale=255.0):
        super().__init__()
        background = nn.Parameter(paddle.to_tensor(np.array(background)).cast(dtype=paddle.float32) / channel_scale)
    ...
    def forward(self, position):
        ...
        background = paddle.broadcast_to(
            background.reshape([background.shape[0], *ones, n_channels]), [*shape, n_channels]
        )
...
class VolumeRange:
    ...
    def partition(self, ts):
        ...
        mids = (ts[..., 1:, :] + ts[..., :-1, :]) * 0.5
        lower = paddle.concat([self.t0[..., None, :], mids], axis=-2)
        upper = paddle.concat([mids, self.t1[..., None, :]], axis=-2)
        delta = upper - lower
        assert lower.shape == upper.shape == delta.shape == ts.shape
        return lower, upper, delta


class BoundingBoxVolume(nn.Layer):
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.bbox_min = paddle.to_tensor(bbox_min)
        self.bbox_max = paddle.to_tensor(bbox_max)
        self.bbox = paddle.stack([self.bbox_min, self.bbox_max])
    ...
    def intersect(
        ...
        direction: paddle.Tensor,
        t0_lower: Optional[paddle.Tensor] = None,
        epsilon=1e-6,
    ):
        """
        Args:
        ...
        def _safe_divide(a, b, epsilon=1e-6):
            return a / paddle.where(b < 0, b - epsilon, b + epsilon)
        ...
        if t0_lower is not None:
            ...
            t0 = paddle.maximum(t0, t0_lower)

        intersected = t0 + self.min_t_range < t1
        t0 = paddle.where(intersected, t0, paddle.zeros_like(t0))
        t1 = paddle.where(intersected, t1, paddle.ones_like(t1))

        return VolumeRange(t0=t0, t1=t1, intersected=intersected)


class StratifiedRaySampler(nn.Layer):
    """
    ...
    def sample(
        ...
        ts = paddle.linspace(0, 1, n_samples).reshape([*ones, n_samples]).cast(t0.dtype)

        if self.depth_mode == "linear":
            ts = t0 * (1.0 - ts) + t1 * ts
        elif self.depth_mode == "geometric":
            ts = (t0.clip(epsilon).log() * (1.0 - ts) + t1.clip(epsilon).log() * ts).exp()
            ...
            ts = 1.0 / (1.0 / t0.clip(epsilon) * (1.0 - ts) + 1.0 / t1.clip(epsilon) * ts)

        mids = 0.5 * (ts[..., 1:] + ts[..., :-1])
        upper = paddle.concat([mids, t1], axis=-1)
        lower = paddle.concat([t0, mids], axis=-1)
        # yiyi notes: add a random seed here for testing, don't forget to remove
        # paddle.seed(0)
        t_rand = paddle.randn(ts.shape, dtype=ts.dtype)

        ts = lower + (upper - lower) * t_rand
        return ts.unsqueeze(-1)


class ImportanceRaySampler(nn.Layer):
    """
    ...
    def sample(self, t0: paddle.Tensor, t1: paddle.Tensor, n_samples: int) -> paddle.Tensor:
        ...
        if self.blur_pool:
            padded = paddle.concat([weights[..., :1, :], weights, weights[..., -1:, :]], axis=-2)
            maxes = paddle.maximum(padded[..., :-1, :], padded[..., 1:, :])
        ...
        t_rand = paddle.rand(inds.shape)
        lower_ = paddle.take_along_axis(lower, axis=-2, indices=inds)
...
class MeshDecoder(nn.Layer):
    ...
    def __init__(self):
        super().__init__()
        cases = paddle.zeros([256, 5, 3], dtype=paddle.int64)
        masks = paddle.zeros([256, 5], dtype=paddle.bool)
    ...
    def forward(self, field: paddle.Tensor, min_point: paddle.Tensor, size: paddle.Tensor):
        ...
        corner_coords = paddle.empty([*grid_size, 3], dtype=field.dtype)
        corner_coords[range(grid_size[0]), :, :, 0] = paddle.arange(grid_size[0], dtype=field.dtype)[:, None, None]
        corner_coords[:, range(grid_size[1]), :, 1] = paddle.arange(grid_size[1], dtype=field.dtype)[:, None]
        corner_coords[:, :, range(grid_size[2]), 2] = paddle.arange(grid_size[2], dtype=field.dtype)
        ...
        cube_indices = paddle.zeros([grid_size[0] - 1, grid_size[1] - 1, grid_size[2] - 1, 3], dtype=paddle.int64)
        cube_indices[range(grid_size[0] - 1), :, :, 0] = paddle.arange(grid_size[0] - 1)[:, None, None]
        cube_indices[:, range(grid_size[1] - 1), :, 1] = paddle.arange(grid_size[1] - 1)[:, None]
        cube_indices[:, :, range(grid_size[2] - 1), 2] = paddle.arange(grid_size[2] - 1)
        ...
        flat_bitmasks = bitmasks.reshape([-1]).cast(
            "int64"
        ...
        global_tris = paddle.take_along_axis(
            edge_indices, axis=1, indices=local_tris.reshape([local_tris.shape[0], -1])
        ...
        old_index_to_new_index[used_vertex_indices] = paddle.arange(len(used_vertex_indices), dtype=paddle.int64)
        ...
        v1 = paddle.floor(used_edge_midpoints).cast(paddle.int64)
        v2 = paddle.ceil(used_edge_midpoints).cast(paddle.int64)
        s1 = field[v1[:, 0], v1[:, 1], v1[:, 2]]
        s2 = field[v2[:, 0], v2[:, 1], v2[:, 2]]
        p1 = (v1.cast("float32") / (grid_size_tensor - 1)) * size + min_point
        p2 = (v2.cast("float32") / (grid_size_tensor - 1)) * size + min_point
...
class MLPNeRSTFModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.mlp = nn.LayerList([nn.Linear(d_in, d_out) for d_in, d_out in zip(input_widths, output_widths)])
    ...
    def forward(self, *, position, direction, ts, nerf_level="coarse", rendering_mode="nerf"):
        ...
        for i, layer in enumerate(self.mlp):
            if i == self.config.insert_direction_at:  # 4 in the config
                ...
                h = paddle.concat([h, h_direction], axis=-1)
...
class ShapERenderer(ModelMixin, ConfigMixin):
    ...
    def render_rays(self, rays, sampler, n_samples, prev_model_out=None, render_with_direction=False):
        ...
        if prev_model_out is not None:
            ...
            ts = paddle.sort(paddle.concat([ts, prev_model_out.ts], axis=-2), axis=-2)

        batch_size, *_shape, _t0_dim = vrange.t0.shape
        _, *ts_shape, _ts_dim = ts.shape

        # 2. Get the points along the ray and query the model
        directions = paddle.broadcast_to(direction.unsqueeze(-2), [batch_size, *ts_shape, 3])
        ...
        transmittance = paddle.where(vrange.intersected, transmittance, paddle.ones_like(transmittance))
        channels = paddle.where(vrange.intersected, channels, paddle.zeros_like(channels))
    ...
    def decode_to_image(
        ...
        for name, param in self.mlp.state_dict().items():
            if f"nerstf.{name}" in projected_params.keys():
                param.copy_(projected_params[f"nerstf.{name}"].squeeze(0), False)
        ...
        images = paddle.concat(images, axis=1)
        images = images.reshape([*camera.shape, camera.height, camera.width, -1]).squeeze(0)
    ...
    def decode_to_mesh(
        ...
        query_positions = query_points[None].tile([1, 1, 1]).cast(dtype=self.mlp.dtype)
        ...
        fields = paddle.concat(fields, axis=1)
        fields = fields.cast("float32")
        ...
        mesh_mask = paddle.to_tensor(mesh_mask)
        ...
        texture_query_positions = paddle.stack(
            [m.verts[paddle.arange(0, max_vertices) % len(m.verts)] for m in raw_meshes],
            axis=0,
        )
        ...
        textures = paddle.concat(textures, axis=1)

        textures = _convert_srgb_to_linear(textures)
        textures = textures.cast("float32")
