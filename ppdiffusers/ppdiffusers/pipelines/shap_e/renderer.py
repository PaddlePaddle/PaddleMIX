...
def sample_pmf(pmf: paddle.Tensor, n_samples: int) -> paddle.Tensor:
    ...
    *shape, support_size, last_dim = pmf.shape
    ...
    inds = paddle.searchsorted(
        sorted_sequence=cdf,
        values=paddle.rand(shape=[cdf.shape[0], n_samples]))
    return inds.reshape([*shape, n_samples, 1]).clip(
        min=0, max=support_size - 1)


def posenc_nerf(x: paddle.Tensor, min_deg: int=0,
                max_deg: int=15) -> paddle.Tensor:
    """
    ...
    *shape, dim = x.shape
    xb = (x.reshape(-1, 1, dim) * scales.reshape([1, -1, 1])).reshape(*shape,
                                                                      -1)
...
def volume_query_points(volume, grid_size):
    ...
    ys = paddle.trunc(paddle.divide(indices, paddle.to_tensor(
        grid_size))) % grid_size
    xs = paddle.trunc(paddle.divide(indices, paddle.to_tensor(grid_size**
                                                              2))) % grid_size
    combined = paddle.stack(x=[xs, ys, zs], axis=1)
    return combined.astype(dtype='float32') / (grid_size - 1) * (
        volume.bbox_max - volume.bbox_min) + volume.bbox_min


def _convert_srgb_to_linear(u: paddle.Tensor):
    return paddle.where(
        condition=u <= 0.04045, x=u / 12.92, y=((u + 0.055) / 1.055)**2.4)


def _create_flat_edge_indices(flat_cube_indices: paddle.Tensor,
    ...
    return paddle.stack(
        x=[
            ...
            flat_cube_indices[:, (2)],
            flat_cube_indices[:, (0)] * grid_size[1] * grid_size[2] +
            (flat_cube_indices[:, (1)] + 1
             ) * grid_size[2] + flat_cube_indices[:, (2)],
            flat_cube_indices[:, (0)] * grid_size[1] * grid_size[2] +
            flat_cube_indices[:, (1)] * grid_size[2] + flat_cube_indices[:, (2)]
            + 1, flat_cube_indices[:, (0)] * grid_size[1] * grid_size[2] +
            (flat_cube_indices[:, (1)] + 1
             ) * grid_size[2] + flat_cube_indices[:, (2)] + 1,
            y_offset + flat_cube_indices[:, (0)] *
            (grid_size[1] - 1) * grid_size[2] + flat_cube_indices[:, (1)] *
            grid_size[2] + flat_cube_indices[:, (2)],
            y_offset + (flat_cube_indices[:, (0)] + 1) *
            (grid_size[1] - 1) * grid_size[2] + flat_cube_indices[:, (1)] *
            grid_size[2] + flat_cube_indices[:, (2)],
            y_offset + flat_cube_indices[:, (0)] *
            (grid_size[1] - 1) * grid_size[2] + flat_cube_indices[:, (1)] *
            ...
            (grid_size[1] - 1) * grid_size[2] + flat_cube_indices[:, (1)] *
            grid_size[2] + flat_cube_indices[:, (2)] + 1,
            z_offset + flat_cube_indices[:, (0)] * grid_size[1] *
            (grid_size[2] - 1) + flat_cube_indices[:, (1)] *
            (grid_size[2] - 1) + flat_cube_indices[:, (2)],
            z_offset + (flat_cube_indices[:, (0)] + 1) * grid_size[1] *
            (grid_size[2] - 1) + flat_cube_indices[:, (1)] *
            (grid_size[2] - 1) + flat_cube_indices[:, (2)],
            z_offset + flat_cube_indices[:, (0)] * grid_size[1] *
            (grid_size[2] - 1) + (flat_cube_indices[:, (1)] + 1) *
            (grid_size[2] - 1) + flat_cube_indices[:, (2)],
            z_offset + (flat_cube_indices[:, (0)] + 1) * grid_size[1] *
            (grid_size[2] - 1) + (flat_cube_indices[:, (1)] + 1) * (
                grid_size[2] - 1) + flat_cube_indices[:, (2)]
...
class VoidNeRFModel(paddle.nn.Layer):
    ...
    def forward(self, position):
        ...
        background = paddle.broadcast_to(
            x=background.reshape([background.shape[0], *ones, n_channels]),
    ...
    def intersect(self,
        ...
        def _safe_divide(a, b, epsilon=1e-06):
            return a / paddle.where(
                condition=b < 0, x=b - epsilon, y=b + epsilon)
        ...
        t0 = paddle.where(
            condition=intersected, x=t0, y=paddle.zeros_like(x=t0))
...
class StratifiedRaySampler(paddle.nn.Layer):
    ...
    def sample(self,
               ...
               t1: paddle.Tensor,
               n_samples: int,
               epsilon: float=0.001) -> paddle.Tensor:
        """
        Args:
...
class ImportanceRaySampler(paddle.nn.Layer):
    ...
    @paddle.no_grad()
    def sample(self, t0: paddle.Tensor, t1: paddle.Tensor,
               n_samples: int) -> paddle.Tensor:
        """
        Args:
            t0: start time has shape [batch_size, *shape, 1]
            t1: finish time has shape [batch_size, *shape, 1]
        ...
        if self.blur_pool:
            padded = paddle.concat(
                ...
                axis=-2)
            maxes = paddle.maximum(
                x=padded[(...), :-1, :], y=padded[(...), 1:, :])
            weights = 0.5 * (maxes[(...), :-1, :] + maxes[(...), 1:, :])
        weights = weights + self.alpha
        pmf = weights / weights.sum(axis=-2, keepdim=True)
        inds = sample_pmf(pmf, n_samples)
        assert inds.shape == (batch_size, *shape, n_samples, 1)
        assert (inds >= 0).astype('bool').all() and (
            inds < n_coarse_samples).astype('bool').all()
...
class MeshDecoder(paddle.nn.Layer):
    ...
    def forward(self,
        ...
        corner_coords[:, (range(grid_size[1])), :, (1)] = paddle.arange(
        ...
        cube_indices = paddle.zeros(
            ...
            dtype='int64')
        cube_indices[(range(grid_size[0] - 1)), :, :, (0)] = paddle.arange(
            end=grid_size[0] - 1)[:, (None), (None)]
        cube_indices[:, (range(grid_size[1] - 1)), :, (1)] = paddle.arange(
            end=grid_size[1] - 1)[:, (None)]
...
class MLPNeRSTFModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.mlp = paddle.nn.LayerList(sublayers=[
            paddle.nn.Linear(
    ...
    def map_indices_to_keys(self, output):
        ...
        mapped_output = {
            k: output[(...), start:end]
            for k, (start, end) in h_map.items()
        }
...
class ChannelsProj(paddle.nn.Layer):
    def __init__(self, *, vectors: int, channels: int, d_latent: int):
        ...
        self.proj = paddle.nn.Linear(
            in_features=d_latent, out_features=vectors * channels)
    ...
    def forward(self, x: paddle.Tensor) -> paddle.Tensor:
        x_bvd = x
        w_vcd = self.proj.weight.reshape(
...
class ShapEParamsProjModel(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
                 *,
                 param_names: Tuple[str]=(
                     'nerstf.mlp.0.weight', 'nerstf.mlp.1.weight',
                     'nerstf.mlp.2.weight', 'nerstf.mlp.3.weight'),
                 param_shapes: Tuple[Tuple[int]]=((256, 93), (256, 256),
                                                  (256, 256), (256, 256)),
                 d_latent: int=1024):
        super().__init__()
        ...
        for k, (vectors, channels) in zip(param_names, param_shapes):
            self.projections[_sanitize_name(k)] = ChannelsProj(
                vectors=vectors, channels=channels, d_latent=d_latent)
    ...
    def forward(self, x: paddle.Tensor):
        ...
        for k, shape in zip(self.config.param_names, self.config.param_shapes):
            ...
            x_bvd = x[:, start:end]
            out[k] = self.projections[_sanitize_name(k)](x_bvd).reshape(
                len(x), *shape)
            start = end
        return out
...
class ShapERenderer(ModelMixin, ConfigMixin):
    ...
    def __init__(self,
        ...
        self.volume = BoundingBoxVolume(
            bbox_max=[1.0, 1.0, 1.0], bbox_min=[-1.0, -1.0, -1.0])
    ...
    def render_rays(self,
        ...
        transmittance = paddle.where(
            ...
            x=transmittance,
            y=paddle.ones_like(x=transmittance))
        channels = paddle.where(
            condition=vrange.intersected,
            x=channels,
    ...
    def decode_to_image(self,
        ...
        for idx in range(n_batches):
            ...
            channels, _, _ = self.render_rays(
                ...
                fine_sampler,
                n_fine_samples,
                prev_model_out=coarse_model_out)
            images.append(channels)
        images = paddle.concat(x=images, axis=1)
        images = images.reshape(
            [*camera.shape, camera.height, camera.width, -1]).squeeze(axis=0)
        return images
    ...
    def decode_to_mesh(self,
        ...
        for idx in range(0, query_positions.shape[1], query_batch_size):
            ...
            model_out = self.mlp(position=query_batch,
                                 direction=None,
                                 ts=None,
                                 nerf_level='fine',
                                 rendering_mode='stf')
        ...
        mesh_mask = []
        for field in fields:
            raw_mesh = self.mesh_decoder(
                field, self.volume.bbox_min,
        ...
        texture_query_positions = paddle.stack(
            x=[
                m.verts[paddle.arange(
                    start=0, end=max_vertices) % len(m.verts)]
                for m in raw_meshes
            ],
            axis=0)
        ...
        for idx in range(0, texture_query_positions.shape[1], query_batch_size):
            ...
            texture_model_out = self.mlp(position=query_batch,
                                         direction=None,
                                         ts=None,
                                         nerf_level='fine',
                                         rendering_mode='stf')
        ...
        for m, texture in zip(raw_meshes, textures):
            ...
            m.vertex_channels = dict(
                zip(texture_channels, texture.unbind(axis=-1)))
        return raw_meshes[0]
