...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import paddle
        >>> from ppdiffusers import StableDiffusionGLIGENTextImagePipeline
        >>> from ppdiffusers.utils import load_image

        >>> # Insert objects described by image at the region defined by bounding boxes
        >>> pipe = StableDiffusionGLIGENTextImagePipeline.from_pretrained(
        ...     "anhnct/Gligen_Inpainting_Text_Image", paddle_dtype=paddle.float16
        ... )
        ...
        >>> pipe = StableDiffusionGLIGENTextImagePipeline.from_pretrained(
        ...     "anhnct/Gligen_Text_Image", paddle_dtype=paddle.float16
        ... )
...
class StableDiffusionGLIGENTextImagePipeline(DiffusionPipeline):
    ...
    def draw_inpaint_mask_from_boxes(self, boxes, size):
        ...
        """
        inpaint_mask = paddle.ones([size[0], size[1]])
        for box in boxes:
    ...
    def complete_mask(self, has_mask, max_objs):
        ...
        """
        mask = paddle.ones([1, max_objs]).cast(self.text_encoder.dtype)
        if has_mask is None:
            return mask
    ...
    def get_clip_feature(self, input, normalize_constant, is_image=False):
        ...
        if is_image:
            if input is None:
                return None
            inputs = self.processor(images=[input], return_tensors="pd")
            inputs["pixel_values"] = inputs["pixel_values"].cast(self.image_encoder.dtype)
            ...
            if input is None:
                return None
            inputs = self.tokenizer(input, return_tensors="pd", padding=True)
    ...
    def get_cross_attention_kwargs_with_grounded(
        ...
        boxes = paddle.zeros([max_objs, 4], dtype=self.text_encoder.dtype)
        masks = paddle.zeros(
            [
                max_objs,
            ],
        ...
        phrases_masks = paddle.zeros(
            [
                max_objs,
            ],
            dtype=self.text_encoder.dtype,
        )
        image_masks = paddle.zeros(
            [
                max_objs,
            ],
            dtype=self.text_encoder.dtype,
        )
        phrases_embeddings = paddle.zeros([max_objs, hidden_size], dtype=self.text_encoder.dtype)
        image_embeddings = paddle.zeros([max_objs, hidden_size], dtype=self.text_encoder.dtype)
        ...
        for idx, (box, text_feature, image_feature) in enumerate(zip(gligen_boxes, text_features, image_features)):
            boxes[idx] = paddle.to_tensor(box)
            masks[idx] = 1
    ...
    def __call__(
        ...
        if gligen_inpaint_image is not None:
            ...
            gligen_inpaint_image = gligen_inpaint_image.cast(dtype=self.vae.dtype)
            ...
            gligen_inpaint_mask = gligen_inpaint_mask.cast(dtype=gligen_inpaint_latent.dtype)
            gligen_inpaint_mask = gligen_inpaint_mask[None, None]
            gligen_inpaint_mask_addition = paddle.concat(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if latents.shape[1] != 4:
                    latents = paddle.randn(latents[:, :4].shape, dtype=latents.dtype)

                if gligen_inpaint_image is not None:
                    gligen_inpaint_latent_with_noise = (
                        self.scheduler.add_noise(
                            gligen_inpaint_latent,
                            paddle.randn(gligen_inpaint_latent.shape, dtype=gligen_inpaint_latent.dtype),
                            paddle.to_tensor([t]),
                        )
                ...
                if gligen_inpaint_image is not None:
                    latent_model_input = paddle.concat((latent_model_input, gligen_inpaint_mask_addition), axis=1)
