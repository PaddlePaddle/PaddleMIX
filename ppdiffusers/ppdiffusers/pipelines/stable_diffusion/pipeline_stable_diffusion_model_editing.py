...
from paddlenlp.transformers import CLIPTextModel, CLIPTokenizer, CLIPFeatureExtractor
    ...
    def edit_model(self,
        ...
        for old_text, new_text in zip(old_texts, new_texts):
            ...
            text_embeddings = self.text_encoder(
                text_input.input_ids.to(self.place))[0]
        ...
        for old_text, new_text in zip(old_texts, new_texts):
            ...
            tokens_a = [(self.tokenizer.encode('a ')[1]
                         if self.tokenizer.decode(t) == 'an' else t)
                        for t in tokens_a]
            tokens_b = [(self.tokenizer.encode('a ')[1]
                         if self.tokenizer.decode(t) == 'an' else t)
                        for t in tokens_b]
        ...
        for layer_num in range(len(self.projection_matrices)):
            ...
            mat2 = lamb * paddle.eye(
                num_rows=self.projection_matrices[layer_num].weight.shape[1])
            for context, values in zip(contexts, valuess):
                context_vector = context.reshape(context.shape[0],
                                                 context.shape[1], 1)
                context_vector_T = context.reshape(context.shape[0], 1,
                                                   context.shape[1])
