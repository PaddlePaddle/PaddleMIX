...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import paddle
        >>> from ppdiffusers import StableDiffusionGLIGENPipeline
        >>> from ppdiffusers.utils import load_image

        >>> # Insert objects described by text at the region defined by bounding boxes
        >>> pipe = StableDiffusionGLIGENPipeline.from_pretrained(
        ...     "masterful/gligen-1-4-inpainting-text-box", variant="fp16", paddle_dtype=paddle.float16
        ... )
        ...
        >>> # insert objects described by text at the region defined by bounding boxes
        >>> pipe = StableDiffusionGLIGENPipeline.from_pretrained(
        ...     "masterful/gligen-1-4-generation-text-box", variant="fp16", paddle_dtype=paddle.float16
        ... )
...
class StableDiffusionGLIGENPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        tokenizer_inputs = self.tokenizer(gligen_phrases, padding=True, return_tensors="pd")
        ...
        boxes[:n_objs] = paddle.to_tensor(gligen_boxes)
        text_embeddings = paddle.zeros([max_objs, self.unet.cross_attention_dim], dtype=self.text_encoder.dtype)
        ...
        if do_classifier_free_guidance:
            ...
            boxes = paddle.concat([boxes] * 2)
            text_embeddings = paddle.concat([text_embeddings] * 2)
            masks = paddle.concat([masks] * 2)
        ...
        if gligen_inpaint_image is not None:
            ...
            gligen_inpaint_mask = gligen_inpaint_mask.cast(
                dtype=gligen_inpaint_latent.dtype,
            )
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if latents.shape[1] != 4:
                    latents = paddle.randn(latents[:, :4], dtype=latents.dtype)
