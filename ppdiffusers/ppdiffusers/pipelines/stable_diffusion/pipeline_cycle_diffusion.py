...
def posterior_sample(scheduler, latents, timestep, clean_latents, generator,
    ...
    e_t = (latents - alpha_prod_t**0.5 * clean_latents) / (
        1 - alpha_prod_t)**0.5
...
def compute_noise(scheduler, prev_latents, latents, timestep, noise_pred, eta):
    ...
    if scheduler.config.clip_sample:
        pred_original_sample = paddle.clip(
            x=pred_original_sample, min=-1, max=1)
    variance = scheduler._get_variance(timestep, prev_timestep)
    std_dev_t = eta * variance**0.5
    ...
    def prepare_latents(self,
        ...
        if batch_size > init_latents.shape[
            ...
            init_latents = paddle.concat(
                x=[init_latents] * additional_image_per_prompt *
                num_images_per_prompt,
                axis=0)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                source_latent_model_input = paddle.concat(
                    x=[source_latents] *
                    2) if do_classifier_free_guidance else source_latents
                ...
                if do_classifier_free_guidance:
                    concat_latent_model_input = paddle.stack(
                        x=[
                            ...
                            source_latent_model_input[1], latent_model_input[1]
                        ],
                        axis=0)
                    concat_prompt_embeds = paddle.stack(
                        x=[
                            source_prompt_embeds[0], prompt_embeds[0],
                            source_prompt_embeds[1], prompt_embeds[1]
                        ],
                        axis=0)
                else:
                    concat_latent_model_input = paddle.concat(
                        x=[source_latent_model_input, latent_model_input],
                        axis=0)
                    concat_prompt_embeds = paddle.concat(
                        x=[source_prompt_embeds, prompt_embeds], axis=0)
                ...
                if do_classifier_free_guidance:
                    ...
                    source_noise_pred = (
                        source_noise_pred_uncond + source_guidance_scale *
                        (source_noise_pred_text - source_noise_pred_uncond))
                else:
                    source_noise_pred, noise_pred = concat_noise_pred.chunk(
                        chunks=2, axis=0)
