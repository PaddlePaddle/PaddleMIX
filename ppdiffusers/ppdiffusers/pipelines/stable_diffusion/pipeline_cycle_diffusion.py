...
def compute_noise(scheduler, prev_latents, latents, timestep, noise_pred, eta):
    ...
    if scheduler.config.clip_sample:
        pred_original_sample = paddle.clip(x=pred_original_sample, min=-1, max=1)
    ...
    def prepare_latents(self, image, timestep, batch_size, num_images_per_prompt, dtype, generator=None):
        ...
        if batch_size > init_latents.shape[0] and batch_size % init_latents.shape[0] == 0:
            ...
            init_latents = paddle.concat(
                x=[init_latents] * additional_image_per_prompt * num_images_per_prompt, axis=0
            )
            ...
            init_latents = paddle.concat(x=[init_latents] * num_images_per_prompt, axis=0)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                source_latent_model_input = (
                    paddle.concat(x=[source_latents] * 2) if do_classifier_free_guidance else source_latents
                )
                ...
                if do_classifier_free_guidance:
                    ...
                    concat_prompt_embeds = paddle.stack(
                        x=[source_prompt_embeds[0], prompt_embeds[0], source_prompt_embeds[1], prompt_embeds[1]],
                        axis=0,
                    )
                else:
                    concat_latent_model_input = paddle.concat(
                        x=[source_latent_model_input, latent_model_input], axis=0
                    )
                    concat_prompt_embeds = paddle.concat(x=[source_prompt_embeds, prompt_embeds], axis=0)
                ...
                if do_classifier_free_guidance:
                    (
                        ...
                        noise_pred_text,
                    ) = concat_noise_pred.chunk(chunks=4, axis=0)
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
                    source_noise_pred = source_noise_pred_uncond + source_guidance_scale * (
                        source_noise_pred_text - source_noise_pred_uncond
                    )
                else:
                    source_noise_pred, noise_pred = concat_noise_pred.chunk(chunks=2, axis=0)
