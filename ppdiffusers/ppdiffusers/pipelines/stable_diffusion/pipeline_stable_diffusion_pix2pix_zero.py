...
class StableDiffusionPix2PixZeroPipeline(DiffusionPipeline):
    ...
    def generate_caption(self, images):
        ...
        inputs = self.caption_processor(images, text, return_tensors="pd").to(dtype=self.caption_generator.dtype)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # optimizer
                opt = paddle.optimizer.SGD([x_in], learning_rate=cross_attention_guidance_amount)
