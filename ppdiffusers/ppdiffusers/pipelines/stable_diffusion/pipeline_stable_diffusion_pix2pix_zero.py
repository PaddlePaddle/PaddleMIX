...
class Pix2PixZeroL2Loss:
    ...
    def compute_loss(self, predictions, targets):
        self.loss += ((predictions - targets) ** 2).sum(axis=(1, 2)).mean(axis=0)
...
class Pix2PixZeroAttnProcessor:
    ...
    def __call__(
        self, attn: Attention, hidden_states, encoder_hidden_states=None, attention_mask=None, timestep=None, loss=None
    ):
...
class StableDiffusionPix2PixZeroPipeline(DiffusionPipeline):
    ...
    def generate_caption(self, images):
        ...
        inputs = self.caption_processor(images, text, return_tensors="pd").cast(dtype=self.caption_generator.dtype)
    ...
    def construct_direction(self, embs_source: paddle.Tensor, embs_target: paddle.Tensor):
        ...
        return (embs_target.mean(axis=0) - embs_source.mean(axis=0)).unsqueeze(axis=0)
    ...
    def kl_divergence(self, hidden_states):
        ...
        return var + mean**2 - 1 - paddle.log(x=var + 1e-07)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet(
                    latent_model_input, t, encoder_hidden_states=prompt_embeds, cross_attention_kwargs={"timestep": t}
                ).sample
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latents = x_in.detach().chunk(chunks=2)[0]
    ...
    def invert(
        ...
        prompt_embeds = self._encode_prompt(
            prompt, num_images_per_prompt, do_classifier_free_guidance, prompt_embeds=prompt_embeds
        )
