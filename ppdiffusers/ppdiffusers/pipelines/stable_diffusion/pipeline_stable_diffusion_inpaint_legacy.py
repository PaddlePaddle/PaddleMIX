...
def preprocess_mask(mask, batch_size, scale_factor=8):
    if not isinstance(mask, paddle.Tensor):
        ...
        if mask.shape[3] in valid_mask_channel_sizes:
            mask = mask.transpose(perm=[0, 3, 1, 2])
        ...
        mask = paddle.nn.functional.interpolate(x=mask, size=(h // scale_factor, w // scale_factor))
        return mask
...
class StableDiffusionInpaintPipelineLegacy(
    ...
    def __init__(
        ...
        super().__init__()
        deprecation_message = f"The class {self.__class__} is deprecated and will be removed in v1.0.0. You can achieve exactly the same functionalityby loading your model into `StableDiffusionInpaintPipeline` instead. See https://github.com/huggingface/diffusers/pull/3533for more information."
            ...
            for i, t in enumerate(timesteps):
                ...
                if add_predicted_noise:
                    init_latents_proper = self.scheduler.add_noise(
                        init_latents_orig, noise_pred_uncond, paddle.to_tensor(data=[t])
                    )
                else:
                    init_latents_proper = self.scheduler.add_noise(
                        init_latents_orig, noise, paddle.to_tensor(data=[t])
                    )
