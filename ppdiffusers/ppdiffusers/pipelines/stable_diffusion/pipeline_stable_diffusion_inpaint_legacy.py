...
def preprocess_mask(mask, batch_size, scale_factor=8):
    if not isinstance(mask, paddle.Tensor):
        ...
        mask = paddle.nn.functional.interpolate(
            x=mask, size=(h // scale_factor, w // scale_factor))
        return mask
...
class StableDiffusionInpaintPipelineLegacy(
    ...
    def __init__(self,
        ...
        super().__init__()
        deprecation_message = (
            f'The class {self.__class__} is deprecated and will be removed in v1.0.0. You can achieve exactly the same functionalityby loading your model into `StableDiffusionInpaintPipeline` instead. See https://github.com/huggingface/diffusers/pull/3533for more information.'
        )
        deprecate(
            'legacy is outdated',
            '1.0.0',
