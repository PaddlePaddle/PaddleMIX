...
class StableDiffusionPanoramaPipeline(
    ...
    def decode_latents_with_padding(self, latents, padding=8):
        ...
        latents = paddle.concat(
            x=(latents_right, latents, latents_left), axis=-1)
        image = self.vae.decode(latents, return_dict=False)[0]
    ...
    def get_views(self,
        ...
        panorama_width /= 8
        num_blocks_height = (
            panorama_height - window_size
        ) // stride + 1 if panorama_height > window_size else 1
        if circular_padding:
            num_blocks_width = (panorama_width // stride
                                if panorama_width > window_size else 1)
        else:
            num_blocks_width = (
                panorama_width - window_size
            ...
            for i, t in enumerate(timesteps):
                ...
                for j, batch_view in enumerate(views_batch):
                    ...
                    if circular_padding:
                        ...
                        latents_for_view = paddle.concat(x=[
                        ...
                        if circular_padding and w_end > latents.shape[3]:
                            ...
                            value[:, :, h_start:h_end,
                                  w_start:] += latents_view_denoised[:, :,
                                                                     ...
                                                                     shape[3] -
                                                                     w_start]
                            value[:, :, h_start:h_end, :w_end - latents.shape[
                                3]] += latents_view_denoised[:, :, h_start:
                                                             h_end,
                                                             latents.shape[
                                                                 3] - w_start:]
                            count[:, :, h_start:h_end, w_start:] += 1
                            count[:, :, h_start:h_end, :w_end - latents.shape[
                                3]] += 1
                        else:
