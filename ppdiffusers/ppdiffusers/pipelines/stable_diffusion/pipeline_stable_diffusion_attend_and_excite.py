    ...
    def aggregate_attention(self, from_where: List[str]) -> paddle.Tensor:
        ...
        for location in from_where:
            for item in attention_maps[location]:
                cross_maps = item.reshape(-1, self.attn_res[0],
                                          self.attn_res[1], item.shape[-1])
...
class StableDiffusionAttendAndExcitePipeline(DiffusionPipeline,
    ...
    def _compute_max_attention_per_index(
        ...
        attention_for_text = paddle.nn.functional.softmax(
            x=attention_for_text, axis=-1)
        indices = [(index - 1) for index in indices]
        max_indices_list = []
        for i in indices:
            image = attention_for_text[:, :, i]
            smoothing = GaussianSmoothing().to(attention_maps.place)
            input = paddle.nn.functional.pad(
                image.unsqueeze(axis=0).unsqueeze(axis=0), (1, 1, 1, 1),
                mode='reflect')
            image = smoothing(input).squeeze(axis=0).squeeze(axis=0)
    ...
    def _update_latent(latents: paddle.Tensor,
        ...
        grad_cond = paddle.grad(
            outputs=out_92, inputs=[latents], retain_graph=True)[0]
    ...
    def __call__(
        ...
        text_embeddings = prompt_embeds[
            batch_size *
            num_images_per_prompt:] if do_classifier_free_guidance else prompt_embeds
            ...
            for i, t in enumerate(timesteps):
                with paddle.enable_grad():
                    ...
                    for latent, index, text_embedding in zip(latents, indices,
                        ...
                        text_embedding = text_embedding.unsqueeze(axis=0)
                        self.unet(
                            latent,
                            t,
...
class GaussianSmoothing(paddle.nn.Layer):
    """
    Arguments:
    ...
    def __init__(self,
        ...
        meshgrids = paddle.meshgrid([
            paddle.arange(end=size).astype('float32') for size in kernel_size
        ])
        for size, std, mgrid in zip(kernel_size, sigma, meshgrids):
            mean = (size - 1) / 2
            kernel *= 1 / (std * math.sqrt(2 * math.pi)) * paddle.exp(x=-(
                (mgrid - mean) / (2 * std))**2)
        kernel = kernel / paddle.sum(x=kernel)
        kernel = kernel.reshape([1, 1, *kernel.shape])
        kernel = kernel.tile(
            repeat_times=[channels, *([1] * (kernel.dim() - 1))])
        self.register_buffer(name='weight', tensor=kernel)
        self.groups = channels
        if dim == 1:
            self.conv = paddle.nn.functional.conv1d
        elif dim == 2:
            self.conv = paddle.nn.functional.conv2d
        elif dim == 3:
            self.conv = paddle.nn.functional.conv3d
        else:
            raise RuntimeError(
    ...
    def forward(self, input):
        ...
        """
        return self.conv(
            input, weight=self.weight.to(input.dtype), groups=self.groups)
