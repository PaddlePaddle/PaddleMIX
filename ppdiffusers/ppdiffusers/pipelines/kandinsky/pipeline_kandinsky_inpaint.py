...
class KandinskyInpaintPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        Args:
            ...
            image (`paddle.Tensor`, `PIL.Image.Image` or `np.ndarray`):
        ...
        mask_image = paddle.nn.functional.interpolate(
            x=mask_image, size=image_shape, mode='nearest')
        mask_image = prepare_mask(mask_image)
        masked_image = image * mask_image
        mask_image = mask_image.repeat_interleave(
            repeats=num_images_per_prompt, axis=0)
        masked_image = masked_image.repeat_interleave(
            repeats=num_images_per_prompt, axis=0)
        if do_classifier_free_guidance:
        ...
        latents = self.prepare_latents(
            (batch_size, num_channels_latents, sample_height,
             sample_width), text_encoder_hidden_states.dtype, device, generator,
        ...
        for i, t in enumerate(self.progress_bar(timesteps_tensor)):
            ...
            latent_model_input = paddle.concat(
                x=[latent_model_input, masked_image, mask_image], axis=1)
            added_cond_kwargs = {
