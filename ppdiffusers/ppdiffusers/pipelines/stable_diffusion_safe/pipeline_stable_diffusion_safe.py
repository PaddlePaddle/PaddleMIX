...
class StableDiffusionPipelineSafe(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        if do_classifier_free_guidance:
            ...
            if enable_safety_guidance:
                ...
                safety_embeddings = safety_embeddings.tile(repeat_times=[batch_size, num_images_per_prompt, 1])
                ...
                prompt_embeds = paddle.concat(x=[negative_prompt_embeds, prompt_embeds, safety_embeddings])
            else:
    ...
    def perform_safety_guidance(
        ...
        if enable_safety_guidance:
            ...
            # Equation 6
            scale = paddle.clip(
                x=paddle.abs(x=noise_pred_text - noise_pred_safety_concept) * sld_guidance_scale, max=1.0
            )

            # Equation 6
            safety_concept_scale = paddle.where(
                condition=noise_pred_text - noise_pred_safety_concept >= sld_threshold,
                x=paddle.zeros_like(x=scale),
                y=scale,
            )

            # Equation 4
            noise_guidance_safety = (noise_pred_safety_concept - noise_pred_uncond) * safety_concept_scale
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                latent_model_input = (
                    paddle.concat(x=[latents] * (3 if enable_safety_guidance else 2))
                    if do_classifier_free_guidance
                    else latents
                )
                ...
                # perform guidance
                if do_classifier_free_guidance:
                    noise_pred_out = noise_pred.chunk(chunks=3 if enable_safety_guidance else 2)
