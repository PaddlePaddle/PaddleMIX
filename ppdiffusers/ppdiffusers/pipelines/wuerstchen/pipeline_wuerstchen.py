...
class WuerstchenDecoderPipeline(DiffusionPipeline):
    ...
    def encode_prompt(
        ...
        if do_classifier_free_guidance:
            ...
            negative_prompt_embeds_text_encoder_output = self.text_encoder(
                uncond_input.input_ids, attention_mask=uncond_input.attention_mask
            )
    ...
    def __call__(
        self,
        image_embeddings: Union[paddle.Tensor, List[paddle.Tensor]],
        ...
        **kwargs,
        ...
        Args:
            image_embedding (`paddle.Tensor` or `List[paddle.Tensor]`):
        ...
        if isinstance(image_embeddings, list):
            image_embeddings = paddle.concat(image_embeddings, axis=0)
        if isinstance(image_embeddings, np.ndarray):
            image_embeddings = paddle.to_tensor(image_embeddings).cast(dtype=dtype)
        if not isinstance(image_embeddings, paddle.Tensor):
            raise TypeError(
                f"'image_embeddings' must be of type 'paddle.Tensor' or 'np.array', but got {type(image_embeddings)}."
            )
        ...
        text_encoder_hidden_states = (
            paddle.concat([prompt_embeds, negative_prompt_embeds])
        ...
        for i, t in enumerate(self.progress_bar(timesteps[:-1])):
            ...
            effnet = (
                paddle.concat([image_embeddings, paddle.zeros_like(image_embeddings)])
                if self.do_classifier_free_guidance
                else image_embeddings
            )
            # 7. Denoise latents
            predicted_latents = self.decoder(
                paddle.concat([latents] * 2) if self.do_classifier_free_guidance else latents,
                r=paddle.concat([ratio] * 2) if self.do_classifier_free_guidance else ratio,
                effnet=effnet,
            ...
            if self.do_classifier_free_guidance:
                ...
                predicted_latents = paddle.lerp(predicted_latents_uncond, predicted_latents_text, self.guidance_scale)
        ...
        if not output_type == "latent":
            ...
            images = self.vqgan.decode(latents).sample.clip(0, 1)
            if output_type == "np":
                images = images.transpose([0, 2, 3, 1]).cast("float32").cpu().numpy()
