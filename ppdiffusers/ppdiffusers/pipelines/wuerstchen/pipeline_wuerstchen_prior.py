...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import paddle
        >>> from ppdiffusers import WuerstchenPriorPipeline
...
class WuerstchenPriorPipelineOutput(BaseOutput):
    ...
    Args:
        image_embeddings (`paddle.Tensor` or `np.ndarray`)
            Prior image embeddings for text prompt

    """

    image_embeddings: Union[paddle.Tensor, np.ndarray]
    ...
    def __call__(
        ...
        **kwargs,
        ...
        for i, t in enumerate(self.progress_bar(timesteps[:-1])):
            ratio = t.expand((latents.shape[0],)).cast(dtype)
            ...
            if self.do_classifier_free_guidance:
                ...
                predicted_image_embedding = paddle.lerp(
        ...
        if output_type == "np":
            latents = latents.cast("float32").cpu().numpy()
