...
class WuerstchenDiffNeXt(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.effnet_mappers = nn.LayerList(
            [
        ...
        self.seq_norm = nn.LayerNorm(c_cond, epsilon=1e-6, **norm_elementwise_affine_kwargs)

        self.embedding = nn.Sequential(
            nn.PixelUnshuffle(patch_size),
            nn.Conv2D(c_in * (patch_size**2), c_hidden[0], kernel_size=1),
            WuerstchenLayerNorm(c_hidden[0], epsilon=1e-6, **norm_elementwise_affine_kwargs),
        )
        ...
        for i in range(len(c_hidden)):
            ...
            if i > 0:
                down_block.append(
                    nn.Sequential(
                        WuerstchenLayerNorm(c_hidden[i - 1], epsilon=1e-6, **norm_elementwise_affine_kwargs),
    ...
    def _init_weights(self, m):
        if not lazy_init_helper()._state:
            # General init
            ...
            for j, block in enumerate(up_block):
                if isinstance(block, ResBlockStageB):
                    ...
                    if effnet_c is not None:
                        if skip is not None:
                            skip = paddle.concat([skip, effnet_c], axis=1)
                        else:
                            skip = effnet_c
                    x = block(x, skip)
                elif isinstance(block, AttnBlock):
    ...
    def forward(self, x, r, effnet, clip=None, x_cat=None, eps=1e-3, return_noise=True):
        if x_cat is not None:
            x = paddle.concat([x, x_cat], axis=1)
...
class ResBlockStageB(nn.Layer):
    ...
    def forward(self, x, x_skip=None):
        ...
        x = self.channelwise(x.transpose([0, 2, 3, 1])).transpose([0, 3, 1, 2])
        return x + x_res
