...
class MixingResidualBlock(nn.Layer):
    """
    ...
    def __init__(self, inp_channels, embed_dim):
        ...
        self.norm1 = nn.LayerNorm(inp_channels, epsilon=1e-6, **norm_elementwise_affine_kwargs)
        self.depthwise = nn.Sequential(
            nn.Pad2D(1, mode="replicate"), nn.Conv2D(inp_channels, inp_channels, kernel_size=3, groups=inp_channels)
        )

        # channelwise
        self.norm2 = nn.LayerNorm(inp_channels, epsilon=1e-6, **norm_elementwise_affine_kwargs)
    ...
    def forward(self, x):
        mods = self.gammas
        x_temp = self.norm1(x.transpose([0, 2, 3, 1])).transpose([0, 3, 1, 2]) * (1 + mods[0]) + mods[1]
        x = x + self.depthwise(x_temp) * mods[2]
        x_temp = self.norm2(x.transpose([0, 2, 3, 1])).transpose([0, 3, 1, 2]) * (1 + mods[3]) + mods[4]
        x = x + self.channelwise(x_temp.transpose([0, 2, 3, 1])).transpose([0, 3, 1, 2]) * mods[5]
        return x
...
class PaellaVQModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        down_blocks.append(
            nn.Sequential(
                nn.Conv2D(c_levels[-1], latent_channels, kernel_size=1, bias_attr=False),
    ...
    @apply_forward_hook
    def decode(
        self, h: paddle.Tensor, force_not_quantize: bool = True, return_dict: bool = True
