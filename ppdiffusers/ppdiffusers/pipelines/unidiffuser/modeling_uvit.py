...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    with paddle.no_grad():
        ...
        # [2l-1, 2u-1].
        tensor = tensor.uniform_(min=2 * l - 1, max=2 * u - 1)
...
class SkipBlock(nn.Layer):
    def __init__(self, dim: int):
        ...
        # Use nn.LayerNorm for now, following the original code
...
class UTransformerBlock(nn.Layer):
    """
...
# Modified from ppdiffusers.models.attention.BasicTransformerBlock
class UniDiffuserBlock(nn.Layer):
    """
...
class UTransformer2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.transformer_in_blocks = nn.LayerList(
            [
                block_cls(
                    inner_dim,
                    num_attention_heads,
        ...
        self.transformer_out_blocks = nn.LayerList(
            [
                nn.LayerDict(
                    {
...
class UniDiffuserModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if self.use_data_type_embedding:
            ...
            self.data_type_pos_embed_token = paddle.create_parameter(
    ...
    def forward(
        ...
        vae_hidden_states = self.vae_img_in(latent_image_embeds.cast(self.dtype))
        clip_hidden_states = self.clip_img_in(image_embeds.cast(self.dtype))
        text_hidden_states = self.text_in(prompt_embeds.cast(self.dtype))
        ...
        timestep_img = timestep_img * paddle.ones([batch_size], dtype=timestep_img.dtype)
        ...
        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
        timestep_text = timestep_text * paddle.ones([batch_size], dtype=timestep_text.dtype)
        ...
        if self.use_data_type_embedding:
            ...
            # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
            data_type = data_type * paddle.ones([batch_size], dtype=data_type.dtype)
        ...
        if self.use_data_type_embedding:
            pos_embed = paddle.concat(
                [self.pos_embed[:, : 1 + 1, :], self.data_type_pos_embed_token, self.pos_embed[:, 1 + 1 :, :]], axis=1
            )
        else:
