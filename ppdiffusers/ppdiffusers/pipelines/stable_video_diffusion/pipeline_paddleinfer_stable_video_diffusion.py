...
# from .paddle_infer_svd_utils import PaddleInferDiffusionSVDPipelineMixin
...
class PaddleInferStableVideoDiffusionPipeline(DiffusionPipeline):
    r"""
    ...
    def _encode_image(self, image, num_videos_per_prompt, do_classifier_free_guidance):
        ...
        if not isinstance(image, paddle.Tensor):
            ...
            image = self.image_processor.numpy_to_pd(image)
        ...
        # )
        image_embeddings = self.image_encoder(pixel_values=image)[0]
        image_embeddings = image_embeddings.unsqueeze(1)

        # duplicate image embeddings for each generation per prompt, using mps friendly method
        bs_embed, seq_len, _ = image_embeddings.shape
        image_embeddings = image_embeddings.tile([1, num_videos_per_prompt, 1])
        image_embeddings = image_embeddings.reshape([bs_embed * num_videos_per_prompt, seq_len, -1])

        if do_classifier_free_guidance:
            negative_image_embeddings = paddle.zeros_like(image_embeddings)

            # For classifier free guidance, we need to do two forward passes.
            # Here we concatenate the unconditional and text embeddings into a single batch
            # to avoid doing two forward passes
            image_embeddings = paddle.concat([negative_image_embeddings, image_embeddings])
    ...
    def _encode_vae_image(
        ...
        image_latents = self.vae_encoder(z=image)[0]

        if do_classifier_free_guidance:
            negative_image_latents = paddle.zeros_like(image_latents)

            # For classifier free guidance, we need to do two forward passes.
            # Here we concatenate the unconditional and text embeddings into a single batch
            # to avoid doing two forward passes
            image_latents = paddle.concat([negative_image_latents, image_latents])
    ...
    def _get_add_time_ids(
        ...
        if do_classifier_free_guidance:
            add_time_ids = paddle.concat([add_time_ids, add_time_ids])
    ...
    def decode_latents(self, latents, num_frames, decode_chunk_size=14):
        ...
        for i in range(0, latents.shape[0], decode_chunk_size):
            ...
            frame = self.vae_decoder(z=latents[i : i + decode_chunk_size], num_frames=num_frames_in)[0]
            frames.append(frame)
    ...
    def __call__(
        ...
        pipe = StableVideoDiffusionPipeline.from_pretrained("stabilityai/stable-video-diffusion-img2vid-xt", paddle_dtype=paddle.float16, variant="fp16")
        ...
        image_embeddings = self._encode_image(image, num_videos_per_prompt, do_classifier_free_guidance)
