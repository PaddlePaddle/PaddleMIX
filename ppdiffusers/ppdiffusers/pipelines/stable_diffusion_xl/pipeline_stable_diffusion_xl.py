...
class StableDiffusionXLPipeline(DiffusionPipeline, FromSingleFileMixin,
    ...
    def __call__(
        ...
        add_time_ids = add_time_ids.to(device).tile(
            repeat_times=[batch_size * num_images_per_prompt, 1])
        num_warmup_steps = max(
            len(timesteps) - num_inference_steps * self.scheduler.order, 0)
        if denoising_end is not None and type(
                denoising_end
    ...
    def load_lora_weights(self,
        ...
        text_encoder_state_dict = {
            k: v
            for k, v in state_dict.items() if 'text_encoder.' in k
        }
        ...
        text_encoder_2_state_dict = {
            k: v
            for k, v in state_dict.items() if 'text_encoder_2.' in k
        }
    ...
    def save_lora_weights(self,
                          ...
                          unet_lora_layers: Dict[str, Union[
                              paddle.nn.Layer, paddle.Tensor]]=None,
                          text_encoder_lora_layers: Dict[str, Union[
                              paddle.nn.Layer, paddle.Tensor]]=None,
                          text_encoder_2_lora_layers: Dict[str, Union[
                              paddle.nn.Layer, paddle.Tensor]]=None,
        ...
        def pack_weights(layers, prefix):
            layers_weights = layers.state_dict() if isinstance(
                layers, paddle.nn.Layer) else layers
            layers_state_dict = {
        ...
        if text_encoder_lora_layers and text_encoder_2_lora_layers:
            state_dict.update(
                pack_weights(text_encoder_lora_layers, 'text_encoder'))
            state_dict.update(
