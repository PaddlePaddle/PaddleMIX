...
from ...utils import deprecate, is_accelerate_available, is_accelerate_version, is_invisible_watermark_available, logging, randn_tensor
    ...
    def __call__(
        ...
        if do_classifier_free_guidance:
            prompt_embeds = paddle.concat(
                x=[prompt_embeds, negative_prompt_embeds], axis=0)
            add_text_embeds = paddle.concat(
                x=[add_text_embeds, negative_pooled_prompt_embeds], axis=0)
            add_neg_time_ids = add_neg_time_ids.tile(
                repeat_times=[batch_size * num_images_per_prompt, 1])
            add_time_ids = paddle.concat(
                x=[add_time_ids, add_neg_time_ids], axis=0)
        add_text_embeds = paddle.concat(
            x=(add_text_embeds, add_text_embeds[:original_add_text_embeds_len]),
            axis=0)
        add_time_ids = paddle.concat(
            x=(add_time_ids, add_time_ids.clone()[:original_add_time_ids]),
            axis=0)
        prompt_embeds = paddle.concat(
            x=(prompt_embeds,
