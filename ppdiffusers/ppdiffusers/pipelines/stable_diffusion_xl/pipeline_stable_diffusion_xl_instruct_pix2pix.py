...
EXAMPLE_DOC_STRING = """
    Examples:
        ```py
        >>> import paddle
        >>> from ppdiffusers import StableDiffusionXLInstructPix2PixPipeline
        ...
        ...     "diffusers/sdxl-instructpix2pix-768", paddle_dtype=paddle.float16
        ... )
...
class StableDiffusionXLInstructPix2PixPipeline(
    ...
    # Copied from ppdiffusers.pipelines.stable_diffusion.pipeline_stable_diffusion_instruct_pix2pix.StableDiffusionInstructPix2PixPipeline.check_inputs
    def check_inputs(
        self,
        prompt,
        callback_steps,
    ...
    def prepare_image_latents(
        ...
        if image.shape[1] == 4:
            ...
            if needs_upcasting:
                self.upcast_vae()
                image = image.cast(dtype=next(iter(self.vae.post_quant_conv.named_parameters()))[1].dtype)
        ...
        if image_latents.dtype != self.vae.dtype:
            image_latents = image_latents.cast(dtype=self.vae.dtype)
    ...
    # Copied from ppdiffusers.pipelines.stable_diffusion_xl.pipeline_stable_diffusion_xl.StableDiffusionXLPipeline.upcast_vae
    def upcast_vae(self):
    ...
    def __call__(
        ...
        if do_classifier_free_guidance:
            ...
            prompt_embeds = paddle.concat([prompt_embeds, negative_prompt_embeds, negative_prompt_embeds], axis=0)
            add_text_embeds = paddle.concat(
                [add_text_embeds, negative_pooled_prompt_embeds, negative_pooled_prompt_embeds], axis=0
            )
            add_time_ids = paddle.concat([add_time_ids, add_time_ids, add_time_ids], axis=0)
