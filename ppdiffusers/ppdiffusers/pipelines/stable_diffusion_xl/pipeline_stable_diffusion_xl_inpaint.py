...
def mask_pil_to_paddle(mask, height, width):
    ...
    if isinstance(mask, list) and isinstance(mask[0], PIL.Image.Image):
        mask = [
            i.resize(
                (width, height), resample=PIL.Image.LANCZOS) for i in mask
        ]
        mask = np.concatenate(
...
def prepare_mask_and_masked_image(image,
    ...
    if isinstance(image, paddle.Tensor):
        ...
        if isinstance(image, list) and isinstance(image[0], PIL.Image.Image):
            image = [
                i.resize(
                    (width, height), resample=PIL.Image.LANCZOS) for i in image
            ]
...
class StableDiffusionXLInpaintPipeline(DiffusionPipeline,
    ...
    def encode_prompt(
        ...
        zero_out_negative_prompt = (negative_prompt is None and
                                    self.config.force_zeros_for_empty_prompt)
        ...
        pooled_prompt_embeds = pooled_prompt_embeds.tile(repeat_times=[
    ...
    def check_inputs(self,
        ...
        elif prompt_2 is not None and (not isinstance(prompt_2, str) and
                                       not isinstance(prompt_2, list)):
            raise ValueError(
    ...
    def _get_add_time_ids(self, original_size, crops_coords_top_left,
                          target_size, aesthetic_score,
                          negative_aesthetic_score, dtype):
        ...
        add_neg_time_ids = paddle.to_tensor(
            data=[add_neg_time_ids], dtype=dtype)
    ...
    def __call__(
        ...
        self.check_inputs(prompt, prompt_2, height, width, strength,
                          callback_steps, negative_prompt, negative_prompt_2,
        ...
        if do_classifier_free_guidance:
            ...
            add_text_embeds = paddle.concat(
                x=[negative_pooled_prompt_embeds, add_text_embeds], axis=0)
            add_neg_time_ids = add_neg_time_ids.tile(
                repeat_times=[batch_size * num_images_per_prompt, 1])
        ...
        if (denoising_end is not None and denoising_start is not None and
                denoising_value_valid(denoising_end) and
                denoising_value_valid(denoising_start) and
            ...
            discrete_timestep_cutoff = int(
                round(self.scheduler.config.num_train_timesteps - denoising_end
                      * self.scheduler.config.num_train_timesteps))
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if num_channels_unet == 9:
                    latent_model_input = paddle.concat(
                        x=[latent_model_input, mask, masked_image_latents],
                        axis=1)
                added_cond_kwargs = {
                ...
                if num_channels_unet == 4:
                    ...
                    if i < len(timesteps) - 1:
                        ...
                        init_latents_proper = self.scheduler.add_noise(
                            init_latents_proper,
                            noise,
                            paddle.to_tensor(data=[noise_timestep]))
                    latents = (1 - init_mask
