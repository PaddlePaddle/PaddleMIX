...
class FastDeployStableDiffusionXLMegaPipeline(FastDeployStableDiffusionXLPipeline):
    ...
    def text2img(
        ...
        temp_pipeline = FastDeployStableDiffusionXLPipeline(
    ...
    def img2img(
        ...
        components = {name: component for name, component in self.components.items() if name in expected_components}
        temp_pipeline = FastDeployStableDiffusionXLImg2ImgPipeline(
            **components, requires_safety_checker=self.config.requires_safety_checker
        )
        temp_pipeline._progress_bar_config = self._progress_bar_config
        output = temp_pipeline(
    ...
    def inpaint(
        ...
        components = {name: component for name, component in self.components.items() if name in expected_components}
        temp_pipeline = FastDeployStableDiffusionXLInpaintPipeline(
            **components, requires_safety_checker=self.config.requires_safety_checker
        )
        temp_pipeline._progress_bar_config = self._progress_bar_config
        output = temp_pipeline(
