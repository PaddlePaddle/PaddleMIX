...
class UniDiffuserPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        if prompt_embeds is None:
            ...
            untruncated_ids = self.clip_tokenizer(prompt, padding="longest", return_tensors="pd").input_ids
            if (
    ...
    def encode_image_vae_latents(
        self, image, batch_size, num_prompts_per_image, dtype, do_classifier_free_guidance, generator=None
    ):
    ...
    def encode_image_clip_latents(self, image, batch_size, num_prompts_per_image, dtype, generator=None):
        # Map image to CLIP embedding.
        if not isinstance(image, (paddle.Tensor, PIL.Image.Image, list)):
            raise ValueError(
                f"`image` has to be of type `paddle.Tensor`, `PIL.Image.Image` or list but is {type(image)}"
            )
        preprocessed_image = self.image_processor.preprocess(image, return_tensors="pd")
    ...
    def prepare_image_vae_latents(
        self, batch_size, num_prompts_per_image, num_channels_latents, height, width, dtype, generator, latents=None
    ):
        shape = (
        ...
        if latents is None:
            ...
            latents = latents.tile(repeat_times=[num_prompts_per_image, 1, 1, 1])
            latents = latents.cast(dtype=dtype)
    ...
    def _split(self, x, height, width):
        ...
        img_vae, img_clip = x.split([img_vae_dim, self.image_encoder_projection_dim], axis=1)
        img_vae = paddle.reshape(x=img_vae, shape=(batch_size, self.num_channels_latents, latent_height, latent_width))
        img_clip = paddle.reshape(x=img_clip, shape=(batch_size, 1, self.image_encoder_projection_dim))
    ...
    def _split_joint(self, x, height, width):
        ...
        img_vae, img_clip, text = x.split([img_vae_dim, self.image_encoder_projection_dim, text_dim], axis=1)
        img_vae = paddle.reshape(x=img_vae, shape=(batch_size, self.num_channels_latents, latent_height, latent_width))
        img_clip = paddle.reshape(x=img_clip, shape=(batch_size, 1, self.image_encoder_projection_dim))
        text = paddle.reshape(x=text, shape=(batch_size, self.text_encoder_seq_len, self.text_intermediate_dim))
