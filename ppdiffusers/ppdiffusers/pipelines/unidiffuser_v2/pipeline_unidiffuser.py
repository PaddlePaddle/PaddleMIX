...
class UniDiffuserPipeline(DiffusionPipeline):
    ...
    def _infer_mode(self, prompt, prompt_embeds, image, latents, prompt_latents,
        ...
        image_latents_available = (vae_latents_available and
                                   clip_latents_available)
    ...
    def _infer_batch_size(self, mode, prompt, prompt_embeds, image,
                          num_images_per_prompt, num_prompts_per_image, latents,
        ...
        if prompt_embeds is None:
            ...
            if untruncated_ids.shape[-1] >= text_input_ids.shape[
                ...
                removed_text = self.clip_tokenizer.batch_decode(
                    untruncated_ids[:, self.clip_tokenizer.model_max_length - 1:
                                    -1])
                logger.warning(
                    f'The following part of your input was truncated because CLIP can only handle sequences up to {self.clip_tokenizer.model_max_length} tokens: {removed_text}'
                )
        ...
        if isinstance(generator, list):
            image_latents = [(
                self.vae.encode(image[i:i + 1]).latent_dist.sample(
                    generator=generator[i]) * self.vae.config.scaling_factor)
    ...
    def encode_image_clip_latents(self,
                                  image,
                                  batch_size,
                                  num_prompts_per_image,
                                  dtype,
        ...
        if isinstance(generator, list):
            image_latents = [
                self.image_encoder(**preprocessed_image[i:i + 1]).image_embeds
                for i in range(batch_size)
            ]
            image_latents = paddle.concat(x=image_latents, axis=0)
        else:
            image_latents = self.image_encoder(
                **preprocessed_image).image_embeds
        if batch_size > image_latents.shape[
                0] and batch_size % image_latents.shape[0] == 0:
            # expand image_latents for batch_size
            deprecation_message = (
    ...
    # Modified from ppdiffusers.pipelines.stable_diffusion.pipeline_stable_diffusion.StableDiffusionPipeline.decode_latents
    ...
    def _split(self, x, height, width):
        ...
        img_vae = paddle.reshape(
            ...
            shape=(batch_size, self.num_channels_latents, latent_height,
                   latent_width))
        img_clip = paddle.reshape(
            x=img_clip,
            shape=(batch_size, 1, self.image_encoder_projection_dim))
    ...
    def _combine(self, img_vae, img_clip):
        ...
        """
        img_vae = paddle.reshape(x=img_vae, shape=(img_vae.shape[0], -1))
        img_clip = paddle.reshape(x=img_clip, shape=(img_clip.shape[0], -1))
        return paddle.concat(x=[img_vae, img_clip], axis=-1)
    ...
    def _split_joint(self, x, height, width):
        ...
        text = paddle.reshape(
            x=text,
            shape=(batch_size, self.text_encoder_seq_len,
                   self.text_intermediate_dim))
    ...
    def _combine_joint(self, img_vae, img_clip, text):
        ...
        img_vae = paddle.reshape(x=img_vae, shape=(img_vae.shape[0], -1))
        img_clip = paddle.reshape(x=img_clip, shape=(img_clip.shape[0], -1))
        text = paddle.reshape(x=text, shape=(text.shape[0], -1))
        return paddle.concat(x=[img_vae, img_clip, text], axis=-1)
    ...
    def _get_noise_pred(self, mode, latents, t, prompt_embeds, img_vae,
        ...
        if mode == 'joint':
            ...
            img_vae_latents, img_clip_latents, text_latents = (
                self._split_joint(latents, height, width))
            ...
            return guidance_scale * x_out + (1.0 - guidance_scale
                                             ) * x_out_uncond
        elif mode == 'text2img':
    ...
    def check_inputs(self,
        ...
        if (height % self.vae_scale_factor != 0 or
                width % self.vae_scale_factor != 0):
            raise ValueError(
        ...
        if full_latents_available:
            individual_latents_available = (prompt_latents is not None or
                                            vae_latents is not None or
                                            clip_latents is not None)
            ...
            img_vae_dim = (self.num_channels_latents * latent_height *
                           latent_width)
            text_dim = (self.text_encoder_seq_len *
                        self.text_encoder_hidden_size)
            latents_dim = (
                img_vae_dim + self.image_encoder_projection_dim + text_dim)
        ...
        if prompt_latents_available:
            prompt_latents_expected_shape = (self.text_encoder_seq_len,
                                             self.text_encoder_hidden_size)
        ...
        if (mode == 'joint' and prompt_latents_available and
            ...
            if prompt_latents.shape[0] != vae_latents.shape[
                    0] or prompt_latents.shape[0] != clip_latents.shape[0]:
                raise ValueError(
    ...
    def __call__(
        ...
        self.check_inputs(mode, prompt, image, height, width, callback_steps,
                          ...
                          negative_prompt_embeds, latents, prompt_latents,
                          vae_latents, clip_latents)
        ...
        if latents is not None:
            ...
            vae_latents, clip_latents, prompt_latents = self._split_joint(
        ...
        gen_text = None
        if mode == 'joint':
            image_vae_latents, image_clip_latents, text_latents = (
                self._split_joint(latents, height, width))
