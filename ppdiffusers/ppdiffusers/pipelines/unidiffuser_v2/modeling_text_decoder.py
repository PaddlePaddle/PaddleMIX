...
class UniDiffuserTextDecoder(ModelMixin, ConfigMixin):
    ...
    _keys_to_ignore_on_load_unexpected = [
        'h\\.\\d+\\.attn\\.bias', 'h\\.\\d+\\.attn\\.masked_bias'
    ]
    ...
    def __init__(self,
        ...
        self.encode_prefix = paddle.nn.Linear(
            in_features=self.prefix_inner_dim,
            out_features=self.prefix_hidden_dim
        ) if self.prefix_hidden_dim is not None else paddle.nn.Identity()
        self.decode_prefix = paddle.nn.Linear(
            in_features=self.prefix_hidden_dim, out_features=n_embd
        ) if self.prefix_hidden_dim is not None else paddle.nn.Identity()
    ...
    def generate_captions(
        ...
        """
        features = paddle.split(
            x=features, num_or_sections=features.shape[0] // 1, axis=0)
        generated_tokens = []
    ...
    def generate_beam(self,
        ...
        for i in range(entry_length):
            ...
            if scores is None:
                ...
                generated = generated.expand(
                    shape=[beam_size, *generated.shape[1:]])
                next_tokens, scores = next_tokens.transpose(
                    perm=[1, 0]), scores.squeeze(axis=0)
                if tokens is None:
                    tokens = next_tokens
                else:
            ...
            next_token_embed = self.transformer.transformer.wte(
                next_tokens.squeeze()).reshape([generated.shape[0], 1, -1])
            generated = paddle.concat(x=(generated, next_token_embed), axis=1)
            is_stopped = is_stopped + next_tokens.equal(
                y=stop_token_index).squeeze()
        ...
        seq_lengths = paddle.to_tensor(
            data=[seq_lengths[i] for i in order], dtype=seq_lengths.dtype)
