    ...
    def __init__(
        ...
        if layer_norm:
            self.norm = paddle.nn.LayerNorm(
                normalized_shape=embed_dim, weight_attr=False, bias_attr=False, epsilon=1e-06
            )
        else:
            self.norm = None
        ...
        if self.use_pos_embed:
            ...
            self.register_buffer(
                name="pos_embed",
                tensor=paddle.to_tensor(data=pos_embed).astype(dtype="float32").unsqueeze(axis=0),
                persistable=False,
            )
...
class SkipBlock(paddle.nn.Layer):
    def __init__(self, dim: int):
        super().__init__()
        self.skip_linear = paddle.nn.Linear(in_features=2 * dim, out_features=dim)

        # Use paddle.nn.LayerNorm for now, following the original code
...
# Modified from ppdiffusers.models.attention.BasicTransformerBlock
...
class UniDiffuserBlock(paddle.nn.Layer):
    ...
    def forward(
        ...
        # Following the ppdiffusers transformer block implementation, put the LayerNorm on the
        # residual backbone
        # Pre-LayerNorm
        if self.pre_layer_norm:
            if self.use_ada_layer_norm:
...
class UTransformer2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.transformer_in_blocks = paddle.nn.LayerList(
            sublayers=[
                block_cls(
                    inner_dim,
                    num_attention_heads,
        ...
        self.transformer_out_blocks = paddle.nn.LayerList(
            sublayers=[
                paddle.nn.LayerDict(
                    sublayers={
...
class UniDiffuserModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.clip_img_in = paddle.nn.Linear(in_features=clip_img_dim, out_features=self.inner_dim)
        self.text_in = paddle.nn.Linear(in_features=text_dim, out_features=self.inner_dim)
        ...
        self.timestep_img_embed = (
            TimestepEmbedding(self.inner_dim, 4 * self.inner_dim, out_dim=self.inner_dim)
        ...
        out_74 = paddle.create_parameter(
            shape=paddle.zeros(shape=[1, self.num_tokens, self.inner_dim]).shape,
            dtype=paddle.zeros(shape=[1, self.num_tokens, self.inner_dim]).numpy().dtype,
            default_initializer=paddle.nn.initializer.Assign(paddle.zeros(shape=[1, self.num_tokens, self.inner_dim])),
        )
        ...
        if self.use_data_type_embedding:
            self.data_type_token_embedding = paddle.nn.Embedding(num_embeddings=2, embedding_dim=self.inner_dim)
            out_75 = paddle.create_parameter(
                ...
                default_initializer=paddle.nn.initializer.Assign(paddle.zeros(shape=[1, 1, self.inner_dim])),
            )
        ...
        self.vae_img_out = paddle.nn.Linear(in_features=self.inner_dim, out_features=patch_dim)
        self.clip_img_out = paddle.nn.Linear(in_features=self.inner_dim, out_features=clip_img_dim)
        self.text_out = paddle.nn.Linear(in_features=self.inner_dim, out_features=text_dim)
    ...
    def forward(
        ...
        num_text_tokens, num_img_tokens = text_hidden_states.shape[1], vae_hidden_states.shape[1]
        ...
        timestep_img = timestep_img * paddle.ones(shape=batch_size, dtype=timestep_img.dtype)
        ...
        if not paddle.is_tensor(x=timestep_text):
            timestep_text = paddle.to_tensor(data=[timestep_text], dtype="int64")

        # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
        timestep_text = timestep_text * paddle.ones(shape=batch_size, dtype=timestep_text.dtype)
        ...
        if self.use_data_type_embedding:
            ...
            # broadcast to batch dimension in a way that's compatible with ONNX/Core ML
            data_type = data_type * paddle.ones(shape=batch_size, dtype=data_type.dtype)
            data_type_token = self.data_type_token_embedding(data_type).unsqueeze(axis=1)
            ...
            hidden_states = paddle.concat(
                x=[timestep_img_token, timestep_text_token, text_hidden_states, clip_hidden_states, vae_hidden_states],
                axis=1,
            )
        ...
        if self.use_data_type_embedding:
            pos_embed = paddle.concat(
                x=[self.pos_embed[:, : 1 + 1, :], self.data_type_pos_embed_token, self.pos_embed[:, 1 + 1 :, :]],
                axis=1,
            )
        else:
        ...
        if self.use_data_type_embedding:
            (
                ...
                img_clip_out,
                img_vae_out,
            ) = hidden_states.split((1, 1, 1, num_text_tokens, 1, num_img_tokens), axis=1)
        else:
