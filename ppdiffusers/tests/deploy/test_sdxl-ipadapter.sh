...
cd $LOCAL_PATH/PaddleMIX/ppdiffusers/deploy/ipadapter/sdxl
...
python infer.py --model_dir static_model/stable-diffusion-xl-base-1.0-ipadapter --scheduler "euler" --backend paddle --device gpu --task_name all --width 512 --height 512 --inference_steps 30 --tune True --use_fp16 False

# 3. convert the model to tensorrt
python infer.py --model_dir static_model/stable-diffusion-xl-base-1.0-ipadapter --scheduler "euler" --backend paddle_tensorrt --device gpu --task_name all --width 512 --height 512 --inference_steps 50