...
class ActivationsTests(unittest.TestCase):
    ...
    def test_swish(self):
        ...
        self.assertEqual(act(paddle.to_tensor(data=-100, dtype='float32')).
            item(), 0)
        self.assertNotEqual(act(paddle.to_tensor(data=-1, dtype='float32'))
            .item(), 0)
        self.assertEqual(act(paddle.to_tensor(data=0, dtype='float32')).
            item(), 0)
        self.assertEqual(act(paddle.to_tensor(data=20, dtype='float32')).
            item(), 20)
    ...
    def test_mish(self):
        ...
        self.assertEqual(act(paddle.to_tensor(data=-200, dtype='float32')).
            item(), 0)
        self.assertNotEqual(act(paddle.to_tensor(data=-1, dtype='float32'))
            .item(), 0)
        self.assertEqual(act(paddle.to_tensor(data=0, dtype='float32')).
