...
from ppdiffusers import UNetSpatioTemporalConditionModel
...
class UNetSpatioTemporalConditionModelTests(ModelTesterMixin, UNetTesterMixin, unittest.TestCase):
    ...
    def dummy_input(self):
        ...
        noise = floats_tensor((batch_size, num_frames, num_channels) + sizes)
        time_step = paddle.to_tensor([10])
        encoder_hidden_states = floats_tensor((batch_size, 1, 32))
    ...
    def _get_add_time_ids(self, do_classifier_free_guidance=True):
        ...
        add_time_ids = add_time_ids.tile([1, 1])
