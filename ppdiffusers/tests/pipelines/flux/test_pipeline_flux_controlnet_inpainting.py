...
class FluxControlNetInpaintPipelineFastTests(unittest.TestCase, PipelineTesterMixin):
    ...
    params = frozenset([
        "prompt", "image", "mask_image", "control_image", "height", "width", "guidance_scale", 
    ...
    def get_dummy_inputs(self, seed=0):
        ...
        init_image = Image.new("RGB", (16, 16), 128)  # Gray image for inpainting
        mask_image = Image.new("RGB", (16, 16), 0)    # Black mask (areas to inpaint)
        # Create a white square in the center of the mask
        for x in range(4, 12):
            for y in range(4, 12):
    ...
    def test_flux_controlnet_inpaint_different_prompts(self):
    ...
    def test_flux_controlnet_inpaint_conditioning_scale(self):
        pipe = self.pipeline_class(**self.get_dummy_components())
    ...
    def test_flux_controlnet_inpaint_strength(self):
        pipe = self.pipeline_class(**self.get_dummy_components())
        ...
        # Generate with lower strength (less inpainting)
        inputs = self.get_dummy_inputs()
        inputs["strength"] = 0.3  # Less inpainting
        low_strength_output = pipe(**inputs).images[0]
        
        # Generate with higher strength (more inpainting)
        inputs = self.get_dummy_inputs()
        inputs["strength"] = 0.9  # More inpainting
    ...
    def test_flux_controlnet_inpaint_different_masks(self):
        pipe = self.pipeline_class(**self.get_dummy_components())
        
        # Generate with default mask
        inputs = self.get_dummy_inputs()
        default_output = pipe(**inputs).images[0]
        
        # Create a different mask - horizontal strip
        different_mask = Image.new("RGB", (16, 16), 0)
        for x in range(0, 16):
            for y in range(6, 10):
                different_mask.putpixel((x, y), (255, 255, 255))
        
        inputs = self.get_dummy_inputs()
        inputs["mask_image"] = different_mask
        different_mask_output = pipe(**inputs).images[0]
        
        # Different masks should produce different outputs
        max_diff = np.abs(default_output - different_mask_output).max()
        assert max_diff > 1e-6

    def test_flux_controlnet_inpaint_image_output_shape(self):
        pipe = self.pipeline_class(**self.get_dummy_components())
        inputs = self.get_dummy_inputs()
    ...
    def test_flux_multi_controlnet_inpaint(self):
        ...
        # Create mask with white square in center
        mask_image = Image.new("RGB", (16, 16), 0)
        for x in range(4, 12):
            for y in range(4, 12):
                mask_image.putpixel((x, y), (255, 255, 255))
