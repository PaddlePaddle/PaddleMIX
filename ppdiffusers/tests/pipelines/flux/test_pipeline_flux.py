...
from ppdiffusers import AutoencoderKL, FlowMatchEulerDiscreteScheduler, FluxPipeline, FluxTransformer2DModel
...
# FluxIPAdapterTesterMixin, PyramidAttentionBroadcastTesterMixin
class FluxPipelineFastTests(
    unittest.TestCase, PipelineTesterMixin
):
    ...
    # TODO (javey-q): 'Attention' object has no attribute 'fuse_projections'
    ...
    def test_flux_true_cfg(self):
        ...
        paddle.seed(seed=0)
        no_true_cfg_out = pipe(**inputs).images[0]
        inputs["negative_prompt"] = "bad quality"
        inputs["true_cfg_scale"] = 2.0
        true_cfg_out = pipe(**inputs).images[0]
...
class FluxPipelineSlowTests(unittest.TestCase):
    ...
    def get_inputs(self, seed=0):
        ...
        prompt_embeds = paddle.load(
            hf_hub_download(repo_id="diffusers/test-slices", repo_type="dataset", filename="flux/prompt_embeds.pt")
        )
        pooled_prompt_embeds = paddle.load(
            hf_hub_download(
    ...
    def test_flux_inference(self):
        pipe = self.pipeline_class.from_pretrained(
            self.repo_id, paddle_dtype=paddle.float16, text_encoder=None, text_encoder_2=None
        )
