...
class StableDiffusion2PipelineSlowTests(unittest.TestCase):
    ...
    def test_stable_diffusion_default_ddim(self):
        ...
        expected_slice = np.array([0.49492595, 0.47895798, 0.4079, 0.54213667, 0.5321169, 0.48202425, 0.47655687, 0.46329486, 0.4850552 ])
    ...
    def test_stable_diffusion_pndm(self):
        ...
        image_slice = image[0, -3:, -3:, -1].flatten()
        assert image.shape == (1, 512, 512, 3)
        expected_slice = np.array([0.494924, 0.4789574, 0.4079845, 0.5421328, 0.53211, 0.482021, 0.4765542, 0.46329302, 0.48505327])
        assert np.abs(image_slice - expected_slice).max() < 7e-3
    ...
    def test_stable_diffusion_k_lms(self):
        ...
        image_slice = image[0, -3:, -3:, -1].flatten()
        assert image.shape == (1, 512, 512, 3)
        expected_slice = np.array([0.10440376, 0.13114733, 0.1109969,  0.10141572, 0.1144042,  0.07214668, 0.11331889, 0.09693599, 0.10006592])
    ...
    def test_stable_diffusion_text2img_intermediate_state(self):
        ...
        def callback_fn(step: int, timestep: int, latents: paddle.Tensor) -> None:
            ...
            if step == 1:
                ...
                expected_slice = np.array([-0.3865037, -0.45230913, -1.1737096, 0.06332049, -1.1094888,   0.7150976, -1.8258412,   0.19208148,  1.2809916 ])
                assert np.abs(latents_slice.flatten() - expected_slice).max() < 0.05
            elif step == 2:
                latents = latents.detach().cpu().numpy()
                assert latents.shape == (1, 4, 64, 64)
                latents_slice = latents[0, -3:, -3:, -1]
                expected_slice = np.array([0.265153, -0.22492063, -0.792711, -0.5546473, -0.8165231, 0.4034267, -0.7740773, 0.47874892, 1.2858746])
                assert np.abs(latents_slice.flatten() - expected_slice).max() < 0.05
