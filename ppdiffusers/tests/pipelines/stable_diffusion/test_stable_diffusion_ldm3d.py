...
from ppdiffusers.utils import nightly, paddle_device, slow
...
class StableDiffusionLDM3DPipelineFastTests(unittest.TestCase):
    ...
    def test_stable_diffusion_prompt_embeds(self):
        ...
        text_inputs = text_inputs["input_ids"].to(paddle_device)
        ...
        assert np.abs(depth_slice_1.flatten() - depth_slice_2.flatten()).max() < 0.0001
...
class StableDiffusionLDM3DPipelineSlowTests(unittest.TestCase):
    ...
    def get_inputs(self, device, generator_device="cpu", dtype="float32", seed=0):
        generator = paddle.framework.core.default_cpu_generator().manual_seed(seed)
        latents = np.random.RandomState(seed).standard_normal((1, 4, 64, 64))
        latents = paddle.to_tensor(data=latents).to(device=device, dtype=dtype)
        inputs = {
    ...
    def test_ldm3d_stable_diffusion(self):
        ...
        ldm3d_pipe.set_progress_bar_config(disable=None)
        inputs = self.get_inputs(paddle_device)
        output = ldm3d_pipe(**inputs)
        rgb, depth = output.rgb, output.depth
