...
class StableDiffusionInpaintPipelineSlowTests(unittest.TestCase):
    ...
    def test_stable_diffusion_inpaint_strength_test(self):
        ...
        assert image.shape == (1, 512, 512, 3)
        image_slice = image[(0), 253:256, 253:256, (-1)].flatten()
        expected_slice = np.array([0.0021, 0.235, 0.3712, 0.0575, 0.2485, 0.3451, 0.1857, 0.3156, 0.3943])
    ...
    def test_stable_diffusion_simple_inpaint_ddim(self):
        ...
        image_slice = image[(0), 253:256, 253:256, (-1)].flatten()
        assert image.shape == (1, 512, 512, 3)
        expected_slice = np.array([0.5157, 0.6858, 0.6873, 0.4619, 0.6416, 0.6898, 0.3702, 0.596, 0.6935])
...
class StableDiffusionInpaintingPrepareMaskAndMaskedImageTests(unittest.TestCase):
    def test_pil_inputs(self):
        ...
        self.assertTrue(t_image.dtype == "float32")
    ...
    def test_torch_3D_2D_inputs(self):
        height, width = 32, 32
        im_tensor = paddle.randint(low=0, high=255, shape=(3, height, width)).astype("uint8")
        mask_tensor = paddle.randint(low=0, high=255, shape=(height, width)).astype("uint8") > 127.5
        ...
        self.assertTrue((t_mask_tensor == t_mask_np).astype("bool").all())
        self.assertTrue((t_masked_tensor == t_masked_np).astype("bool").all())
        self.assertTrue((t_image_tensor == t_image_np).astype("bool").all())

    def test_torch_3D_3D_inputs(self):
        height, width = 32, 32
        im_tensor = paddle.randint(low=0, high=255, shape=(3, height, width)).astype("uint8")
        mask_tensor = paddle.randint(low=0, high=255, shape=(1, height, width)).astype("uint8") > 127.5
        im_np = im_tensor.numpy().transpose(1, 2, 0)
    ...
    def test_torch_4D_2D_inputs(self):
        height, width = 32, 32
        im_tensor = paddle.randint(low=0, high=255, shape=(1, 3, height, width)).astype("uint8")
        mask_tensor = paddle.randint(low=0, high=255, shape=(height, width)).astype("uint8") > 127.5
    ...
    def test_torch_4D_4D_inputs(self):
        height, width = 32, 32
        im_tensor = paddle.randint(low=0, high=255, shape=(1, 3, height, width)).astype("uint8")
        mask_tensor = paddle.randint(low=0, high=255, shape=(1, 1, height, width)).astype("uint8") > 127.5
        im_np = im_tensor.numpy()[0].transpose(1, 2, 0)
    ...
    def test_torch_batch_4D_3D(self):
        height, width = 32, 32
        im_tensor = paddle.randint(low=0, high=255, shape=(2, 3, height, width)).astype("uint8")
        mask_tensor = paddle.randint(low=0, high=255, shape=(2, height, width)).astype("uint8") > 127.5
    ...
    def test_torch_batch_4D_4D(self):
        height, width = 32, 32
        im_tensor = paddle.randint(low=0, high=255, shape=(2, 3, height, width)).astype("uint8")
        mask_tensor = paddle.randint(low=0, high=255, shape=(2, 1, height, width)).astype("uint8") > 127.5
        im_nps = [im.numpy().transpose(1, 2, 0) for im in im_tensor]
