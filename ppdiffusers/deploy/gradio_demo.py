...
def infer(taskname, image, mask, prompt, steps, height, width, seed, scheduler):
    task_name = taskname
    ...
    if task_name == "text2img":
        ...
        images = fd_pipe.inpaint_legacy(
            prompt=prompt,
            image=np.array(image, dtype=np.float32),
            mask_image=np.array(mask, dtype=np.float32),
            num_inference_steps=steps,
            height=height,
            width=width,
            parse_prompt_type=parse_prompt_type,
        ...
        with gr.Tab("text2img"):
            with gr.Row():
                with gr.Column():
                    ...
                    text2img_prompt = gr.Textbox(label="请输入描述词")
        ...
        with gr.Tab("img2img"):
            with gr.Row():
                with gr.Column():
                    ...
                    img2img_prompt = gr.Textbox(label="请输入描述词")
        ...
        with gr.Tab("inpaint_legacy"):
            with gr.Row():
                with gr.Column():
                    ...
                    inpaint_legacy_img = gr.Image(label="原图")
                    inpaint_legacy_mask = gr.Image(label="重绘mask", image_mode="L")
                    inpaint_legacy_prompt = gr.Textbox(label="请输入正向描述词")
        ...
        with gr.Tab("inpaint"):
            with gr.Row():
                with gr.Column():
                    ...
                    inpaint_img = gr.Image(label="原图")
                    inpaint_mask = gr.Image(label="重绘mask", image_mode="L")
                    inpaint_prompt = gr.Textbox(label="请输入正向描述词")
        ...
        with gr.Tab("controlnet_canny"):
            with gr.Row():
                with gr.Column():
                    ...
                    controlnet_canny_mask = gr.State(value=None)
                    controlnet_canny_prompt = gr.Textbox(label="请输入正向描述词")
