...
from transformer_2d_inference import DitTransformer2DInferenceModel
...
def save_dit_model(
    ...
    pipe = DiTPipeline.from_pretrained(model_path, paddle_dtype="float16")
    ...
    inference_model = DitTransformer2DInferenceModel(**read_json(transformer_config_file), export=True)
    dit_inference_state_dict = pipe.transformer.state_dict()
    inference_model.set_state_dict(dit_inference_state_dict)
    inference_model.to(dtype="float16")
    inference_model.eval()
    # 1. convert transformer
    input_spec = [
        paddle.static.InputSpec(shape=[2, 4, 32, 32], dtype="float16", name="hidden_states"),
